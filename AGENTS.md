...
## 🟣 MANDATORY: USE CONTEXT-MODE (ctx_*) — НЕ сырые Read/Bash/grep на больших данных

**У тебя (Codex, Claude, Gemini/agy) подключён context-mode MCP. ОБЯЗАН его использовать.**
Сырой вывод тулов жрёт твоё контекстное окно и роняет сессию (Lane A codex умер 06-06 на
1.05M токенов ровно потому, что читал 8.5MB логов сырьём вместо ctx). Правило:

- **Анализ/research нескольких файлов, логи, .ips, большие выводы** → `ctx_batch_execute(commands, queries)`
  (гоняет команды, авто-индексит, возвращает только нужные секции) или `ctx_execute_file(path, …)`
  (анализ файла в песочнице — в контекст попадает только то что `console.log`).
- **Повторные вопросы по уже собранному / прошлым сессиям** → `ctx_search(queries: [...])`
  (в т.ч. `sort:"timeline"` — авто-память: решения/ошибки/планы прошлых сессий).
- **Вывести/посчитать/распарсить из данных** → `ctx_execute(language, code)` — в контекст идёт только итог.
- **Сырые `Read`/`Bash cat|grep|tail`/`ListDir`/`Search` на больших данных = ЗАПРЕЩЕНО.**
  Read оставляй ТОЛЬКО для файла который собираешься Edit'ить. Bash — только git/mkdir/rm/mv/навигация.
- **Запись файлов** — нативным Write/Edit (ctx_execute песочницу не персистит).
- На больших логах прогонов: `ctx_execute_file('reports/.../run.log', …)` или
  `ctx_batch_execute([{label, command}], queries=[...])`, НЕ `tail`/`grep` в чат.
...
свой per-lane heartbeat — после КАЖДОГО заметного действия (прогон, фикс, сборка, находка)
дописывай ОДНУ строку в `reports/research/LANE-<X>-PROGRESS.md` (X = твой лайн: A / PE32 / C / D /
E). Формат: `ВРЕМЯ · действие · результат · следующий шаг`. Пример:
`03:20 · HK run + classify · boundary 0x10B44F015→0x1096C0FB4 (next thunk no-unwind) · bulk-CFI`.
Плюс по-прежнему: engine-правки → `docs/ENGINE-CHANGE-JOURNAL.md`, живое состояние текущего бага →
`docs/ACTIVE-INVESTIGATION.md`. Если ты не пишешь heartbeat — координатор тебя «не видит» и считает
лайн застрявшим. Не молчи: пиши строку даже на «начал X», «жду сборку», «упёрся в Y».
...
### Дисциплина координатора (Claude) при выдаче лида Codex'у
Урок: лид, поданный (а) как спекуляция и/или (б) когда Codex УЖЕ нашёл причину —
сбивает его на неверную ветку (downside без upside). Перед отправкой лида:
1. **Сначала проверь текущее состояние Codex** (его сессия / ACTIVE-INVESTIGATION).
   Если он уже назвал конкретную причину/фикс — НЕ слать конкурирующую гипотезу.
   Максимум — подтвердить/уточнить фактом, не уводить.
2. **Шли только проверенное.** Раздели VERIFIED (прочитал лог/код/стек, подтвердил)
   и SPECULATION (догадка о механизме). Спекуляцию либо не слать, либо явно метить
   «гипотеза, проверь — не факт». Никогда не подавать догадку как локализацию.
3. **Evidence > гипотеза координатора.** Codex прав, когда держит свой evidence против
   моего лида. Лид — предложение, не приказ. Если мой лид мимо — это норма, не давить.
...
### 3. docs/ACTIVE-INVESTIGATION.md — живое состояние ТЕКУЩЕГО бага (ОБЯЗАТЕЛЬНО)
**Проблема которую это решает**: после compaction агент терял состояние НЕзакрытого
расследования и заново шёл по уже опровергнутым гипотезам (журнал хранит applied-правки,
а не disprovals живого поиска; restore-файл устаревал).

Правило:
- **На старте сессии И сразу после compaction**: прочитай `docs/ACTIVE-INVESTIGATION.md`
  ПЕРЕД любым build/trace. Это «где мы ПРЯМО СЕЙЧАС».
- Раздел **DISPROVED — БИНДИНГ**: НЕ перепроверяй то, что уже опровергнуто трассой.
  Если хочешь оспорить disproval — нужен НОВЫЙ контр-факт, а не повтор старого шага.
- **Обновляй на КАЖДУЮ находку** (не только на правку кода!): новое CONFIRMED, новое
  DISPROVED, сдвиг boundary, смена next-step. Особенно фиксируй ОПРОВЕРЖЕНИЯ.
- Координатор (Claude) тоже держит его свежим, читая сессию. Любой, кто получил находку —
  пишет её сюда немедленно. Stale запись хуже чем отсутствующая.
- Когда баг закрыт: перенеси итог в ENGINE-CHANGE-JOURNAL (ROOT-FIX) и очисти
  ACTIVE-INVESTIGATION под следующий баг.
