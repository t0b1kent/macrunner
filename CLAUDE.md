...
Как это стоило дня 07.08. Критерием трижды подряд был `macrunner-hb-nullcall-vtable`. Три
прогона по 900 с ждали эту строку и не дождались.

**ПОПРАВКА (найдено позже): печать СУЩЕСТВУЕТ** — `signal_arm64.c:2267`, через `MESSAGE(`.
Я объявил её несуществующей, потому что искал `grep | grep fprintf` и только в `unix/`. Оба
условия поиска были неверны. Внешний агент повторил ту же ошибку независимо — значит она
типовая, а не случайная.

Настоящая причина молчания: печать стоит под **четырьмя вложенными условиями** — перебор шести
регистров, проверка объекта на диапазон и выравнивание, чтение указателя таблицы, пять смещений.
Совпасть должно всё сразу. Маркер не отсутствует, он **труднодостижим** — для критерия одинаково
негодно, но причина другая, и искать надо иначе.

Три проверки, все дешёвые:

1. **Существует ли печать — и искать ПО ВСЕМ способам печати.** Маркеры выводятся не только
   через `fprintf`: в `dlls/ntdll/` их 529 через `fprintf`, **181 через `MESSAGE(` и 125 через
   `ERR(`** — это макросы Wine. Поиск по одному `fprintf` объявит 97 живых маркеров
   несуществующими.
   Правильно: `grep -rnE '(fprintf|MESSAGE|ERR|WARN|TRACE|FIXME)\( *"marker-name' engine/`.
   И искать во ВСЁМ `engine/wine/dlls/ntdll/`, а не только в `unix/` — `signal_arm64.c` есть в
   обоих каталогах, и печать может быть в том, куда не заглянул.
