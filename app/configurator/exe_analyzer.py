#!/usr/bin/env python3
"""
MacRunner — AI Configurator: PE binary analyzer.

Читает Windows .exe и достаёт:
  - архитектура (x86/x86_64/arm64)
  - импортируемые DLL (для определения какой DirectX, какой .NET, etc.)
  - версия Windows subsystem
  - SHA-256 хеш для матчинга с базой профилей

Это вход в profile lookup и AI-генерацию конфига.
"""
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("Usage: exe_analyzer.py <path-to.exe>", file=sys.stderr)
        return 2
    md = analyze(Path(argv[1]))
    print(md.to_json())
    return 0
