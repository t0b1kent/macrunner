...
Verify: Built/deployed/codesigned `ntdll.so` (`e8e0fabee9b97498bcf15c4accdacf410c513853fead276b7d759841b0123c2f`) and used cache root `artifacts/hb-translation-cache/ntdll-e8e0fabee9b97498`. Warm pre-GLM and GLM behavior matched, so GLM DLLs are not the regression. After the memory fix, sample `tree_insert` terms dropped from 540/787 to 130/205; after JIT env caching, latest warm sample `getenv` terms dropped to 17-22. Filtered evidence run `reports/phase4-hollow-knight/laneA-postjit-warm-evidence-glm-try1-120655` classifies as rung 9 (`D3D11_CREATE_DEVICE_MISSING`, `real_factory=2`, self-check PASS). Populate run on the same closure reached raw `macrunner-dxmt-fence` but no `CreateSwapChain`/`Present`.
