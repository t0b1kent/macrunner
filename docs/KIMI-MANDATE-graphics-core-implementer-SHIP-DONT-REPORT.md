...
## GOLDEN ДВИЖОК = ИСПОЛЬЗУЙ КАК ЕСТЬ (не пересобирай)
В твоём worktree УЖЕ лежит полный рабочий x64-движок из golden snapshot (реальные файлы):
`engine/wine/dist-pure-arm64/bin/wine` (arm64), `aarch64-unix/ntdll.so` (HyperBridge/CPU),
вся Wine-дистрибуция. Он ЧИСТЫЙ и РАБОТАЕТ на внешнем диске (golden = x64 FULL_UI_READY).
- **НЕ пересобирай и не правь CPU-движок** — потребляй его как есть, запускай через него.
- Корень WineD3D-fallback виден: рядом `x86_64-windows/d3d11.dll` = Wine-встроенный WineD3D
  (~3.3MB), а DXMT `engine/graphics/dist/dxmt/x86_64-windows/d3d11.dll` (~5.28MB) — твой.
  Задача M1 — чтобы грузился ТВОЙ DXMT (override), а не встроенный WineD3D.
- Ты строишь/итерируешь ТОЛЬКО графику поверх готового движка. Эскалация к Codex нужна
  ТОЛЬКО если игре реально не хватает CPU-опкода/пути в golden HyperBridge (редко — golden
  уже гонял NPP x64 целиком). Тогда одной строкой Claude'у. Иначе — всё сам, автономно.
