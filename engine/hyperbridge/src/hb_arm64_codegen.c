...
static uint32_t emit_direct_mem_addr_with_offset(hb_codegen_buffer_t* buf, const hb_ir_operand_t* op) {
    uint32_t off = 0;
    if (direct_mem_unsigned_offset(op, &off)) {
        emit_ldr_x(buf, 21, 19, (uint32_t)x64_reg_off(op->mem.base));
        return off;
    }
...
static bool emit_native_scalar_mov(hb_codegen_buffer_t* buf, const hb_ir_instr_t* instr) {
    ...
    if (jit_direct_mem_enabled() && is_direct_user_mem_operand(&instr->dst) &&
...
static bool emit_hot_scalar_scan_loop(hb_codegen_buffer_t* buf, const hb_ir_block_t* block) {
    ...
    if (!jit_direct_scalar_scan_enabled() || !block || block->instr_count != 3) return false;
...
static bool emit_epilogue_restore_ret_block(hb_codegen_buffer_t* buf,
    ...
    size_t idx = 0;
    if (!jit_direct_mem_enabled() || !block ||
    ...
    emit_str_x(buf, 21, 19, (uint32_t)x64_reg_off(load1_reg));
    if (maybe_mov) {
        emit_ldr_x(buf, 21, 19, (uint32_t)x64_reg_off(maybe_mov->src1.reg));
        emit_str_x(buf, 21, 19, (uint32_t)x64_reg_off(maybe_mov->dst.reg));
    }
    emit_ldr_x(buf, 21, 20, load2_disp);
    emit_str_x(buf, 21, 19, (uint32_t)x64_reg_off(load2_reg));
    emit_ldr_x(buf, 21, 20, (uint32_t)frame);
    emit_str_x(buf, 21, 19, (uint32_t)x64_reg_off(pop->dst.reg));
...
static uint64_t hb_jit_helper_two_block_loop_budget(void) {
    ...
    uint64_t budget = env && *env ? strtoull(env, NULL, 0) : 16384ULL;
    return budget ? budget : 16384ULL;
}
...
void hb_jit_helper_exec_unity_sort_inner_loop(hb_context_t* ctx,
    ...
    for (uint64_t iter = 0; iter < budget; iter++) {
        ...
        if (r != HB_OK) break;
        rcx = r15;
        r = hb_memory_write_u64(ctx->memory, r14, rax);
        if (r != HB_OK) break;
        r14 = rdi;
...
void hb_jit_helper_exec_unity_freelist_fill_loop(hb_context_t* ctx,
    ...
    for (uint64_t iter = 0; iter < budget; iter++) {
        ...
        r = hb_memory_write_u64(ctx->memory, rcx, r14);
        if (r != HB_OK) { ctx->last_result = r; return; }

        r = hb_memory_read_u64(ctx->memory, rsi + 0x80u, &old_head);
        if (r != HB_OK) { ctx->last_result = r; return; }
        r = hb_memory_write_u64(ctx->memory, rsi + 0x80u, rcx);
        if (r != HB_OK) { ctx->last_result = r; return; }
        ...
        if (old_head) {
            r = hb_memory_write_u64(ctx->memory, old_head, rcx);
        } else {
            r = hb_memory_write_u64(ctx->memory, rsi + 0x40u, rcx);
        }
        if (r != HB_OK) { ctx->last_result = r; return; }
...
static const uint8_t* hb_jit_helper_host_read_span(hb_context_t* ctx, uint64_t addr,
    ...
    if (!region || !region->host_base || !(region->perm & HB_PERM_READ)) return NULL;
...
static hb_result_t hb_jit_helper_read_u16_fast(hb_context_t* ctx, uint64_t addr, uint16_t* out) {
    ...
    (void)available;
    if (span) {
        memcpy(out, span, sizeof(*out));
        return HB_OK;
    }
    return hb_memory_read_u16(ctx->memory, addr, out);
}
...
static hb_result_t hb_jit_helper_read_u32_fast(hb_context_t* ctx, uint64_t addr, uint32_t* out) {
    ...
    return hb_memory_read_u32(ctx->memory, addr, out);
}
