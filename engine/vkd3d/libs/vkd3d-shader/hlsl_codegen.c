...
static bool lower_combined_samples(struct hlsl_ctx *ctx, struct hlsl_ir_node *instr, void *context)
{
    ...
    switch (load->load_type)
    {
        ...
        case HLSL_RESOURCE_COUNTER_INCREMENT:
        case HLSL_RESOURCE_COUNTER_DECREMENT:
            return false;
    ...
    switch (instr->type)
    {
        ...
        case HLSL_IR_RESOURCE_LOAD:
        {
            ...
            if (hlsl_deref_get_regset(ctx, &load->resource) == HLSL_REGSET_UAVS)
            {
                ...
                if (load->load_type == HLSL_RESOURCE_COUNTER_INCREMENT
                        || load->load_type == HLSL_RESOURCE_COUNTER_DECREMENT)
                    load->resource.var->objects_usage[HLSL_REGSET_UAVS][index].uav_atomics = true;
            }
            break;
        }
...
static bool sm4_generate_vsir_instr_uav_counter(struct hlsl_ctx *ctx,
        struct vsir_program *program, const struct hlsl_ir_resource_load *load)
{
    enum vkd3d_shader_opcode opcode = load->load_type == HLSL_RESOURCE_COUNTER_INCREMENT
            ? VSIR_OP_IMM_ATOMIC_ALLOC : VSIR_OP_IMM_ATOMIC_CONSUME;
    const struct hlsl_ir_node *instr = &load->node;
    struct vkd3d_shader_instruction *ins;

    if (hlsl_deref_get_regset(ctx, &load->resource) != HLSL_REGSET_UAVS)
    {
        hlsl_fixme(ctx, &load->node.loc, "Counter method on non-UAV resource.");
        return false;
    }
    ...
    if (!sm4_generate_vsir_init_src_operand_from_deref(ctx, program,
            &ins->src[0], &load->resource, ins->dst[0].write_mask, &instr->loc))
        return false;
        ...
        struct vsir_program *program, const struct hlsl_ir_resource_load *load)
{
    ...
    switch (load->load_type)
    {
        ...
        case HLSL_RESOURCE_COUNTER_INCREMENT:
        case HLSL_RESOURCE_COUNTER_DECREMENT:
            return sm4_generate_vsir_instr_uav_counter(ctx, program, load);
