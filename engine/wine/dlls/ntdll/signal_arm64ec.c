...
unsigned long macrunner_hb_x18_tsd_offset = 0;
...
static void macrunner_hb_init_x18_tls_restore(void)
{
    ...
    base &= ~(unsigned long)7;
    for (i = 0; i < 512; i++)
    {
        if (((void **)base)[i] == (void *)teb)
        {
            macrunner_hb_x18_tsd_offset = (unsigned long)i * sizeof(void *);
...
DEFINE_SYSCALL(NtCreateNamedPipeFile, (HANDLE *handle, ULONG access, OBJECT_ATTRIBUTES *attr, IO_STATUS_BLOCK *io, ULONG sharing, ULONG dispo, ULONG options, const struct __wine_nt_named_pipe_extra *extra))
...
void WINAPI LdrInitializeThunk( CONTEXT *arm_context, ULONG_PTR unk2, ULONG_PTR unk3, ULONG_PTR unk4 )
{
    ...
    ERR( "macrunner-hb-ldr-entry: arm_context=%p unk2=%p unk3=%p unk4=%p\n",
    ...
    ERR( "macrunner-hb-ldr-after-xlat: x0=%p x1=%p rcx=%p rdx=%p rip=%p rsp=%p\n",
    ...
    ERR( "macrunner-hb-ldr-after-loader-init: x0=%p x1=%p rcx=%p rdx=%p rip=%p rsp=%p entry_x64=%u\n",
         (void *)context.X0, (void *)context.X1, (void *)context.AMD64_Context.Rcx,
         (void *)context.AMD64_Context.Rdx, (void *)context.AMD64_Context.Rip,
         (void *)context.AMD64_Context.Rsp,
    ...
    if (macrunner_hb_x64_loader_entry( (void *)(ULONG_PTR)context.X0 ))
    {
        ...
        ERR( "macrunner-hb-ldr-thread-entry: entry=%p arg=%p rip=%p rsp=%p\n",
             params.entry, params.arg, (void *)context.AMD64_Context.Rip,
             (void *)context.AMD64_Context.Rsp );
        status = WINE_UNIX_CALL( unix_macrunner_hb_x64_thread_entry, &params );
        ERR( "macrunner-hb-ldr-thread-return: entry=%p arg=%p status=%lx ret=%s blocks=%s steps=%s\n",
