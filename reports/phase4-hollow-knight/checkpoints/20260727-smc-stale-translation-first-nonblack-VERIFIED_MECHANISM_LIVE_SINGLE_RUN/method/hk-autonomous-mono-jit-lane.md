...
## Prior art — read this before designing your next instrument

`reports/phase4-hollow-knight/HK-MONO-JIT-PRIOR-ART-BRIEF-20260727.md` collects how box64 and
FEX-Emu solved this exact problem class (Mono's JIT stacked on a translator's JIT). It is
external experience, not our measurement — treat every item as `[HYPOTHESIS]` — but it names
three concrete mechanisms and two of them cost NO run slot:

1. **Stale translations from self-modifying code.** FEX fixed SMC handling on their Wine path
   that *"could fix some spurious hangs or incorrect invalidations."* Mono writes fresh
   machine code into executable pages; if our translator does not invalidate cached
   translations on that write, the guest re-executes the previous translation forever — a
   thread that is alive, running, and never advancing, which is exactly our shape. Per the
   Windows-ARM64 xtajit design we are effectively reimplementing, an instruction-cache flush
   is supposed to call back into the translator to flush cached translations.
   **Offline check, do this first: does a guest `NtFlushInstructionCache` /
   `FlushInstructionCache` (and whatever Mono's `__clear_cache` lowers to) actually reach
   HyperBridge's translation-cache invalidation?** A missing callback is a code-reading
   result, not an experiment — and it explains the open mechanism question better than the
   spin does: the spin ends, the thread resumes, and resumes into code that cannot progress.
2. **Block size.** box64 detects `MonoBleedingEdge` and forces `BOX64_DYNAREC_BIGBLOCK=0`;
   bigger blocks span more guest bytes, so they are likelier to contain code Mono rewrites.
3. **Memory ordering.** box64 also forces `BOX64_DYNAREC_STRONGMEM=1` for Mono, and FEX frames
   its Mono work around TSO sensitivity. Mono's `mono_jit_info_table_find_internal` uses
   **hazard pointers** — a lock-free retry loop. Under x86-TSO those loops terminate; if our
   translation drops the implied ordering, a reader can retry forever against a publish it
   never observes. That lands in the same hash-table-lookup neighbourhood as our stack
   capture. We already have `MACRUNNER_HB_SYNC_IMPORT_BARRIER` (presence-based: `=0` turns it
   ON) and `MACRUNNER_HB_JIT_DIRECT_MEM` in-tree, so this is testable without new machinery.

Suggested order: the two offline checks first (they cost no run), then the ordering knob
against the ordinal-200 regression detector, then block size.
