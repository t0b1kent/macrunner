...
STAMP=$(date +%H%M%S)
RUNDIR="$ROOT/reports/phase4-hollow-knight/laneA-INPUT-TEST-manual-language-BISECT-nomem-$STAMP"
mkdir -p "$RUNDIR"
...
# is the right order.
export MACRUNNER_HB_JIT_DIRECT_MEM='0'   # BISECT step2: this one OFF   # FAST-JIT: was 0 (lane's observability profile) -> ON
...
echo "BISECT step2: DIRECT_MEM OFF, the other three ON (STACK proved innocent): oracle template MINUS the two language keys -> save snapshot satisfied AND menu appears"
