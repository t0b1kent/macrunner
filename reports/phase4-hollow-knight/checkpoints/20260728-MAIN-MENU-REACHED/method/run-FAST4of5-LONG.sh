...
STAMP=$(date +%H%M%S)
RUNDIR="$ROOT/reports/phase4-hollow-knight/laneA-INPUT-TEST-manual-language-FAST4of5-LONG-$STAMP"
mkdir -p "$RUNDIR"
...
# is the right order.
export MACRUNNER_HB_JIT_DIRECT_MEM='0'   # the ONE bisected as faulting (c000007b at 0x87ef2785e73)   # FAST-JIT: was 0 (lane's observability profile) -> ON
...
export MACRUNNER_HB_JIT_DIRECT_STACK='1'   # FAST-JIT: was 0 (lane's observability profile) -> ON
...
echo "FAST 4-of-5 @5400s (measured time-to-menu is 2782s; 2400s budgets were cutting runs short): every direct path ON except DIRECT_MEM (bisected as the only faulting one): oracle template MINUS the two language keys -> save snapshot satisfied AND menu appears"
