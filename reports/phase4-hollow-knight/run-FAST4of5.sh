...
STAMP=$(date +%H%M%S)
RUNDIR="$ROOT/reports/phase4-hollow-knight/laneA-INPUT-TEST-manual-language-FAST4of5-$STAMP"
mkdir -p "$RUNDIR"
...
echo "FAST 4-of-5: every direct path ON except DIRECT_MEM (bisected as the only faulting one): oracle template MINUS the two language keys -> save snapshot satisfied AND menu appears"
