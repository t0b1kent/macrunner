...
**Resume checkpoint (2026-05-31 23:14 local):** Hollow Knight remains loader-gated explicit-JIT
...
mask compaction. Current blocker is still throughput/no-window in hot Mono/Unity code. Latest
Hollow Knight run `run-20260531-231311-phase3-memimm-mask-compact-jit/` preserves
fallback/fault/unsupported-zero with cleanup/prune `0`; hot blocks moved rank1 `172 -> 164`, rank2
`236 -> 216`, rank7 `180 -> 172` after skipping redundant immediate masks when the constant already
fits the memory width. NEXT: continue from 64-byte hot evidence, favoring rank3 prologue/local-init
or rank1 multi-block bit/RMW fusion. Do not keep widening immediate/pair paths without new hot
evidence. Keep
