...
| Unity shader/draw probes | PASS | Dynamic `d3dcompiler_47` load, VS/PS/GS compile, GS stream-output creation, shader creation, input layout with per-instance slot, vertex/index/instance/indirect/SO buffers, fullscreen draw, texture-sample draw, sRGB sample/decode draw, present, stream-output readback, and green-pixel readback pass. |
...
| Real Unity present gate | Lane A/C gated | Runtime shader capture and real Unity present are ready to resume once the x64 guest reaches `CreateDXGIFactory` / `D3D11CreateDevice`; post-TSO strict binding evidence `artifacts/dxmt-x64-binding/run-20260604-115855/dxmt-x64-binding.log` still times out before those markers, while owned DXMT substitutes remain green. |
