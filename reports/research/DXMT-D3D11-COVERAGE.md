...
| Phase 6 stability smoke | PASS | Expanded suite after Texture2DArray mip/slice, sRGB sample/decode, typed-buffer + structured-buffer compute, swapchain-effect, present-test, and state-breadth additions: `engine/graphics/scripts/run_dxmt_d3d11_stability_smoke.sh aarch64` ran 2 passes with 60s minimum stability each in `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-113728.outer.log`; both passed `D3DCompile(cs_structured_buffer_5_0)`, `structured_buffer_values=200,201,202,203`, `UnityStateProbe state_reset`, present-test swapchain variants, final readback/frame statistics, and `UnityStabilityProbe`. |
