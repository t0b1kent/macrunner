...
| vkd3d prefix deployment | PASS | `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` verifies `d3d12.dll`, `d3d12core.dll`, `dxgi.dll`, and `winemetal.dll` in isolated aarch64/x64 prefixes byte-match `engine/graphics/dist`; run `artifacts/vkd3d-prefix-sync/run-20260604-125213` also asserts active source selection, PE machine architecture for vkd3d plus DXMT dependency DLLs, `dxgi.dll` factory exports, `d3d12core.dll` exports, and the `d3d12.dll` D3D12 factory/root-signature exports. The same run includes an aarch64 strict-prefix Wine runtime loader gate with `d3d12,d3d12core=n`, verifies copied native `d3d12core.dll` binding, and exercises `D3D12SerializeRootSignature` plus `D3D12CreateRootSignatureDeserializer` (`vkd3d_runtime_rootsig_result=PASS`). |
