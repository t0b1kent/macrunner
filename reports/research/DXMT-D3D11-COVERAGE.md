...
| Phase 6 stability smoke | PASS | Expanded suite after Texture2DArray mip/slice, sRGB sample/decode, typed-buffer compute, swapchain-effect, present-test, and state-breadth additions: `engine/graphics/scripts/run_dxmt_d3d11_stability_smoke.sh aarch64` ran 2 passes with 60s minimum stability each in `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-092236.outer.log`; both passed `UnityStateProbe state_reset`, present-test swapchain variants, final readback/frame statistics, and `UnityStabilityProbe`, with `presented_frames=7193` and `7192`. |
