...
| Phase 6 stability smoke | PASS | Expanded suite after Texture2DArray mip/slice update-copy, sRGB sample/decode, typed-buffer + structured-buffer compute, constant-buffer offsetting, partial constant-buffer update, overlap buffer copy, append-UAV counter copy, swapchain-effect/`ResizeTarget`/resize variants, present-test, and state-breadth additions: `engine/graphics/scripts/run_dxmt_d3d11_stability_smoke.sh aarch64` ran 2 passes with 60s minimum stability each in `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-133145.outer.log`; both per-run logs passed `UnityFeatureProbe result=PASS`, `constant_partial_words=cb000000,cb000003,c0ffee10,c0ffee13,cb000008`, `overlap_copy_words=0c000000,0c000001,0c000000,0c000001,0c000002,0c000003,0c000006,0c000007`, `constant_offset_values=300,301,302,303`, `structured_buffer_values=200,201,202,203`, `append_buffer_count=4`, swapchain `ResizeTarget(60hz)`, `ResizeBuffers(half)` readbacks, `UnityStateProbe state_reset`, present-test variants, final readback/frame statistics, and `UnityStabilityProbe`; the outer summary ends with `stability_smoke_result=PASS`. |
| vkd3d prefix deployment | PASS | `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` verifies `d3d12.dll`, `d3d12core.dll`, `dxgi.dll`, and `winemetal.dll` in isolated aarch64/x64 prefixes byte-match `engine/graphics/dist`; run `artifacts/vkd3d-prefix-sync/run-20260604-131226` also asserts active source selection, PE machine architecture for vkd3d plus DXMT dependency DLLs, `dxgi.dll` factory exports, `d3d12core.dll` exports, and the `d3d12.dll` D3D12 factory/root-signature exports. The same run includes an aarch64 strict-prefix Wine runtime loader gate with `d3d12,d3d12core=n`, verifies copied native `d3d12core.dll` binding, and exercises legacy plus versioned D3D12 root signature semantic roundtrips: `D3D12SerializeRootSignature`, `D3D12CreateRootSignatureDeserializer`, `D3D12SerializeVersionedRootSignature`, and `D3D12CreateVersionedRootSignatureDeserializer` preserve a v1.1 constants + static-sampler signature (`vkd3d_runtime_versioned_rootsig_result=PASS`). |
