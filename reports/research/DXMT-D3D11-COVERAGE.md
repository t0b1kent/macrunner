...
readback, two-pass long-run stability, and vkd3d prefix deployment. The x64
...
| Unity resource probes | PASS | Color texture + SRV/RTV/update/copy/readback, dynamic vertex buffer map + `WRITE_NO_OVERWRITE`, dynamic constant buffer map + `WRITE_NO_OVERWRITE`, default constant-buffer partial `UpdateSubresource` boxed update + staging readback, same-buffer overlap `CopySubresourceRegion` + staging readback, immutable constant buffer, depth DSV, R32_UINT UAV, 3D texture SRV, cubemap SRV, Texture2DArray SRV/RTV, Texture2DArray mip-slice update/copy/readback, and structured buffer SRV/UAV all return success; `artifacts/dxmt-smoke-logs/lane-d-overlap-copy-20260604-132954.outer.log` reports `overlap_copy_words=0c000000,0c000001,0c000000,0c000001,0c000002,0c000003,0c000006,0c000007`, while `artifacts/dxmt-smoke-logs/lane-d-array-mip-copy-20260604-124727.outer.log` verifies `array_slice0_mip1_copy_rgba=68,136,204,255` and `array_slice1_mip1_copy_rgba=68,136,204,255`. |
...
| Unity compute/dispatch probes | PASS | CS `cs_5_0` texture UAV, typed-buffer UAV, structured-buffer UAV, and constant-buffer offset shaders compile/create; direct `Dispatch`, `DispatchIndirect`, `CSSetConstantBuffers1(first=2,count=1)` + `CSGetConstantBuffers1`, UAV-to-staging copy/map, append-UAV hidden counter set plus `CopyStructureCount`, `staging_value=42`, `typed_buffer_values=100,101,102,103`, `structured_buffer_values=200,201,202,203`, `constant_offset_values=300,301,302,303`, and `append_buffer_count=4` all pass in `artifacts/dxmt-smoke-logs/lane-d-cbuffer-offset-20260604-132736.outer.log`. |
| vkd3d HLSL structured UAV store | PASS | Active D3D12 build source is `engine/vkd3d-proton`, which has no Wine `hlsl_codegen.c` structured-store fixme; `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` records this as `structured_store_source=PASS mode=active_source_no_hlsl_codegen` in `artifacts/vkd3d-prefix-sync/run-20260604-111401`. Fallback `engine/vkd3d` and the Wine vendored vkd3d compiler now lower whole-element `RWStructuredBuffer<T>` writes to `VSIR_OP_STORE_STRUCTURED` with a zero byte offset and include the matching TPF `STORE_STRUCTURED` encoder case; rebuilt `d3dcompiler_47.dll`/`wined3d.dll` dist binaries carry no stale structured-store fixme, and the headless smoke compiles `cs_structured_buffer_5_0` with `hr=0x00000000`. |
...
| Phase 6 stability smoke | PASS | Expanded suite after Texture2DArray mip/slice update-copy, sRGB sample/decode, typed-buffer + structured-buffer compute, constant-buffer offsetting, partial constant-buffer update, overlap buffer copy, append-UAV counter copy, query lifecycle pending checks, depth-only `ClearView`, swapchain-effect/`ResizeTarget`/resize variants, present-test, and state-breadth additions ran 2 passes with 60s minimum stability each in `artifacts/dxmt-smoke-logs/lane-d-stability-refresh-20260604-135717.outer.log`; both per-run logs passed `UnityFeatureProbe result=PASS`, `ClearView(depth_full/depth_rect)=PASS`, pipeline/SO/overflow `GetData(... pending) hr=0x00000001`, `UnityComputeProbe result=PASS`, final readback/frame statistics, and `UnityStabilityProbe result=PASS`; the outer summary ends with `exit_code=0`. |
| vkd3d prefix deployment | PASS | `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` verifies `d3d12.dll`, `d3d12core.dll`, `dxgi.dll`, and `winemetal.dll` in isolated aarch64/x64 prefixes byte-match `engine/graphics/dist`; run `artifacts/vkd3d-prefix-sync/run-20260604-135430` also asserts active source selection, PE machine architecture for vkd3d plus DXMT dependency DLLs, `dxgi.dll` factory exports, `d3d12core.dll` exports, and the `d3d12.dll` D3D12 factory/root-signature exports. The same run includes an aarch64 strict-prefix Wine runtime loader gate with `d3d12,d3d12core=n`, verifies copied native `d3d12core.dll` binding, and exercises legacy plus versioned D3D12 root signature semantic roundtrips: `D3D12SerializeRootSignature`, `D3D12CreateRootSignatureDeserializer`, `D3D12SerializeVersionedRootSignature`, and `D3D12CreateVersionedRootSignatureDeserializer` preserve a v1.1 constants + static-sampler signature (`vkd3d_runtime_versioned_rootsig_result=PASS`). The runtime probe now has an opt-in `VKD3D_RUNTIME_PROBE_DEVICE=1` device diagnostic; current evidence `artifacts/vkd3d-prefix-sync/run-20260604-135307/runtime-loader-aarch64-windows.log` shows `D3D12CreateDevice hr=0x80004005` through builtin `winevulkan`, so device bring-up remains backend work, not a native-DLL binding regression. |
...
| Multithread/render stability | Implemented | `ID3D10Multithread` protection, two worker-thread deferred command lists, command batching, and two 60s expanded-suite stability runs pass; `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-133145.outer.log` reports `stability_smoke_result=PASS`, `batch_pass_runs=4`, `multithread_pass_runs=4`, and final green readbacks. |
...
| Dispatch/DispatchIndirect | Implemented | Smoke covers direct and indirect compute dispatch with texture UAV readback value `42` and typed-buffer UAV readback values `100,101,102,103`. |
...
| Phase 4 shaders/draw | PASS | Embedded HLSL VS/PS/CS compiles via `d3dcompiler_47`; shader/input-layout creation succeeds with per-instance input data; direct/indirect draw and dispatch variants pass; expanded state matrix, event/timestamp/occlusion query data, predicate, deferred command list, deferred clear, and deferred draw pass; readback returns `pixel0_bgra=0,255,0,255`, deferred clear returns `255,0,64,255`, deferred draw returns green, and compute returns texture, typed-buffer, structured-buffer, and append-UAV counter readbacks. |
