...
| Phase 6 stability smoke | PASS | Expanded suite after Texture2DArray mip/slice, sRGB sample/decode, typed-buffer compute, and swapchain-effect additions: `engine/graphics/scripts/run_dxmt_d3d11_stability_smoke.sh aarch64` ran 2 passes with 60s minimum stability each in `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-084533.outer.log`; both passed final readback/frame statistics and `UnityStabilityProbe`, with `presented_frames=7198` and `7195`. |
...
| Multithread/render stability | Implemented | `ID3D10Multithread` protection, two worker-thread deferred command lists, command batching, and two 60s expanded-suite stability runs pass; `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-084533.outer.log` presented `7198` and `7195` frames with final green readback. |
