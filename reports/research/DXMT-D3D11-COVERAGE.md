...
| Phase 6 stability smoke | PASS | Expanded suite after Texture2DArray mip/slice, sRGB sample/decode, typed-buffer + structured-buffer compute, append-UAV counter copy, swapchain-effect/`ResizeTarget`/resize variants, present-test, and state-breadth additions: `engine/graphics/scripts/run_dxmt_d3d11_stability_smoke.sh aarch64` ran 2 passes with 60s minimum stability each in `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-120650.outer.log`; both per-run logs passed `structured_buffer_values=200,201,202,203`, `append_buffer_count=4`, swapchain `ResizeTarget(60hz)`, `ResizeBuffers(half)` readbacks, `UnityStateProbe state_reset`, present-test variants, final readback/frame statistics, and `UnityStabilityProbe`. |
