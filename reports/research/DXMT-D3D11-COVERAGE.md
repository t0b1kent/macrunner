...
| vkd3d prefix deployment | PASS | `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` verifies `d3d12.dll`, `d3d12core.dll`, `dxgi.dll`, and `winemetal.dll` in isolated aarch64/x64 prefixes byte-match `engine/graphics/dist`; run `artifacts/vkd3d-prefix-sync/run-20260604-111401` also asserts active source selection, PE machine architecture, plus `D3D12CreateDevice`, root-signature deserializers/serializers, `D3D12EnableExperimentalFeatures`, `D3D12GetDebugInterface`, and `D3D12GetInterface` exports. |
