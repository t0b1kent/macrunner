...
| Unity compute/dispatch probes | PASS | CS `cs_5_0` texture UAV, typed-buffer UAV, and structured-buffer UAV shaders compile/create; direct `Dispatch`, `DispatchIndirect`, UAV-to-staging copy/map, append-UAV hidden counter set plus `CopyStructureCount`, `staging_value=42`, `typed_buffer_values=100,101,102,103`, `structured_buffer_values=200,201,202,203`, and `append_buffer_count=4` all pass in `artifacts/dxmt-smoke-logs/dx11-headless-aarch64.log`. |
...
|---|---:|---|
| Buffers | Implemented | DEFAULT/IMMUTABLE/DYNAMIC/STAGING creation paths, including dynamic vertex and dynamic constant-buffer `WRITE_NO_OVERWRITE` maps plus default constant-buffer partial update/readback. |
