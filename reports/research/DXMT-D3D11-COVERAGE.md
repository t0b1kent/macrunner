...
| Phase 6 stability smoke | PASS | Expanded suite after Texture2DArray mip/slice update-copy, sRGB sample/decode, typed-buffer + structured-buffer compute, append-UAV counter copy, swapchain-effect/`ResizeTarget`/resize variants, present-test, and state-breadth additions: `engine/graphics/scripts/run_dxmt_d3d11_stability_smoke.sh aarch64` ran 2 passes with 60s minimum stability each in `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-125404.outer.log`; both per-run logs passed `array_slice0_mip1_copy_rgba=68,136,204,255`, `array_slice1_mip1_copy_rgba=68,136,204,255`, `structured_buffer_values=200,201,202,203`, `append_buffer_count=4`, swapchain `ResizeTarget(60hz)`, `ResizeBuffers(half)` readbacks, `UnityStateProbe state_reset`, present-test variants, final readback/frame statistics, and `UnityStabilityProbe`. |
...
| Multithread/render stability | Implemented | `ID3D10Multithread` protection, two worker-thread deferred command lists, command batching, and two 60s expanded-suite stability runs pass; `artifacts/dxmt-smoke-logs/lane-d-stability-20260604-125404.outer.log` reports `stability_pass_runs=2`, `batch_pass_runs=2`, `multithread_pass_runs=2`, and final green readbacks. |
