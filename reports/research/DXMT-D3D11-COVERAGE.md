...
| Unity compute/dispatch probes | PASS | CS `cs_5_0` texture UAV and typed-buffer UAV shaders compile/create; direct `Dispatch`, `DispatchIndirect`, UAV-to-staging copy/map, `staging_value=42`, and `typed_buffer_values=100,101,102,103` all pass. |
| vkd3d HLSL structured UAV store | PASS | Active D3D12 build source is `engine/vkd3d-proton`, which has no Wine `hlsl_codegen.c` structured-store fixme; `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` now records this as `structured_store_source=PASS mode=active_source_no_hlsl_codegen` in `artifacts/vkd3d-prefix-sync/run-20260604-111401`. Fallback `engine/vkd3d` remains guarded by `patches/graphics-5-vkd3d-hlsl-structured-store.patch`, which lowers whole-element `RWStructuredBuffer<T>` writes to `VSIR_OP_STORE_STRUCTURED` with zero byte offset and compiles the local autotools object target. |
