...
| Unity resource probes | PASS | Color texture + SRV/RTV/update/copy/readback, dynamic vertex buffer map + `WRITE_NO_OVERWRITE`, dynamic constant buffer map + `WRITE_NO_OVERWRITE`, default constant-buffer partial `UpdateSubresource` boxed update + staging readback, immutable constant buffer, depth DSV, R32_UINT UAV, 3D texture SRV, cubemap SRV, Texture2DArray SRV/RTV, Texture2DArray mip-slice update/copy/readback, and structured buffer SRV/UAV all return success; `artifacts/dxmt-smoke-logs/lane-d-partial-constant-20260604-132341.outer.log` reports `Map(dynamic_constant_no_overwrite) hr=0x00000000` and `constant_partial_words=cb000000,cb000003,c0ffee10,c0ffee13,cb000008`, while `artifacts/dxmt-smoke-logs/lane-d-array-mip-copy-20260604-124727.outer.log` verifies `array_slice0_mip1_copy_rgba=68,136,204,255` and `array_slice1_mip1_copy_rgba=68,136,204,255`. |
...
|---|---:|---|
| Buffers | Implemented | DEFAULT/IMMUTABLE/DYNAMIC/STAGING creation paths, including dynamic vertex and dynamic constant-buffer `WRITE_NO_OVERWRITE` maps, default constant-buffer partial update/readback, and shader-visible constant-buffer offsetting via `CSSetConstantBuffers1`. |
