...
| vkd3d HLSL structured UAV store | Source patch staged | `patches/graphics-5-vkd3d-hlsl-structured-store.patch` lowers whole-element `RWStructuredBuffer<T>` writes to `VSIR_OP_STORE_STRUCTURED` with zero byte offset. Validation: local autotools object target `libs/vkd3d-shader/libvkd3d_shader_la-hlsl_codegen.lo` compiles after configuring with Wine `widl` and Homebrew Bison; full `libvkd3d-shader.la` build is still blocked later by unrelated generated `spirv_grammar.h` plumbing and the installed dist DLLs are not yet rebuilt from this patch. |
...
| vkd3d prefix deployment | PASS | `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` verifies `d3d12.dll`, `d3d12core.dll`, `dxgi.dll`, and `winemetal.dll` in isolated aarch64/x64 prefixes byte-match `engine/graphics/dist`; run `artifacts/vkd3d-prefix-sync/run-20260604-090906` also asserts PE machine architecture plus `D3D12CreateDevice`, root-signature deserializers/serializers, `D3D12EnableExperimentalFeatures`, `D3D12GetDebugInterface`, and `D3D12GetInterface` exports. |
