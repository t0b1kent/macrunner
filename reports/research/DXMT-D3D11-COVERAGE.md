...
| vkd3d prefix deployment | PASS | `engine/graphics/scripts/run_vkd3d_prefix_sync_smoke.sh` verifies `d3d12.dll`, `d3d12core.dll`, `dxgi.dll`, and `winemetal.dll` in isolated aarch64/x64 prefixes byte-match `engine/graphics/dist`; run `artifacts/vkd3d-prefix-sync/run-20260604-124052` also asserts active source selection, PE machine architecture for vkd3d plus DXMT dependency DLLs, `dxgi.dll` factory exports, `d3d12core.dll` exports, and the `d3d12.dll` D3D12 factory/root-signature exports. The same run now includes an aarch64 strict-prefix Wine runtime loader gate for the standalone `d3d12.dll` frontend and verifies those exports resolve from the copied DLL. |
