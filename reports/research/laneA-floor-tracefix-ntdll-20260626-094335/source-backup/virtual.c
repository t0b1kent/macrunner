...
static inline int mprotect_exec( void *base, size_t size, int unix_prot )
{
    ...
    if (!rc && macrunner_x64_guest && macrunner_hb_trace_host_exec())
    {
        ...
        if (kr == KERN_SUCCESS && (info.protection & VM_PROT_EXECUTE))
            ERR( "macrunner-host-exec-still-exec: range %p-%p requested %#x final %#x vm_base %p "
...
static void macrunner_hb_register_x64_guest_range( void *base, SIZE_T size )
{
    ...
    {
        ERR( "MacRunner x64 guest range table full, cannot register %p-%p\n",
             base, (char *)base + size );
        return;
    }
                                    ...
                                    BOOL host_executable )
{
    ...
    {
        /* code sections need a private mapping on macOS so they can be made executable.
         * Non-executable read-only sections may be shared session/data mappings; mapping
         * them private gives clients a stale zero-filled snapshot instead of wineserver updates. */
#if defined(__linux__) || defined(__APPLE__)
        if (executable || (vprot & VPROT_EXEC)) flags |= MAP_PRIVATE;
        else
        {
            flags |= MAP_SHARED;
            prot &= ~PROT_WRITE;
...
void virtual_init(void)
{
    ...
    {
        TRACE( "MacRunner HyperBridge clamping host addr space limit from %p to %p\n",
               host_addr_space_limit, (void *)0x0000088000000000 );
        host_addr_space_limit = (void *)0x0000088000000000;
    }
#endif
