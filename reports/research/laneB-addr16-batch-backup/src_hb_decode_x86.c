...
static hb_result_t parse_addr16_ea(hb_dec_t* d, uint8_t mod, uint8_t rm,
                                   int* base, int* index, uint8_t* scale,
                                   int64_t* disp) {
    *base = -1;
    *index = -1;
    *scale = 1;
    *disp = 0;
    ...
    switch (rm) {
        case 0: *base = HB_REG_X86_EBX; *index = HB_REG_X86_ESI; break;
        case 1: *base = HB_REG_X86_EBX; *index = HB_REG_X86_EDI; break;
        case 2: *base = HB_REG_X86_EBP; *index = HB_REG_X86_ESI; break;
        case 3: *base = HB_REG_X86_EBP; *index = HB_REG_X86_EDI; break;
        case 4: *base = HB_REG_X86_ESI; break;
        case 5: *base = HB_REG_X86_EDI; break;
        case 6:
            if (mod == 0) {
                if (!can_read(d, 2)) return HB_ERR_DECODE_FAILED;
                *disp = (uint16_t)read_s16(d);
                return HB_OK;
            }
            *base = HB_REG_X86_EBP;
            break;
        case 7: *base = HB_REG_X86_EBX; break;
    }
...
static hb_result_t parse_modrm(hb_dec_t* d, uint8_t modrm,
    ...
    if (out->address32_prefix) {
        hb_result_t r = parse_addr16_ea(d, mod, rm, &base, &index, &scale, &disp);
        if (r != HB_OK) return r;
    } else if (rm == 4) {
        if (!can_read(d, 1)) return HB_ERR_DECODE_FAILED;
...
static hb_result_t decode_x87(hb_dec_t* d, uint8_t opcode, hb_decoded_t* out) {
    ...
    if (mod == 3) {
        ...
        if (opcode == 0xdb) {
            ...
            else if (modrm >= 0xc0 && modrm <= 0xc7) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_AE; } /* FCMOVNB */
            else if (modrm >= 0xc8 && modrm <= 0xcf) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_NE; } /* FCMOVNE */
            else if (modrm >= 0xd0 && modrm <= 0xd7) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_A; }  /* FCMOVNBE */
            else if (modrm >= 0xd8 && modrm <= 0xdf) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_NP; } /* FCMOVNU */
            else if (modrm >= 0xe8 && modrm <= 0xef) out->opcode = HB_INS_X87_FUCOMI;
            else if (modrm >= 0xf0 && modrm <= 0xf7) out->opcode = HB_INS_X87_FCOMI;
            else return HB_ERR_UNSUPPORTED_OPCODE;
            if (out->opcode == HB_INS_X87_FCMOV) {
        ...
        if (opcode == 0xda) {
            ...
            if (modrm >= 0xc0 && modrm <= 0xc7) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_B; }       /* FCMOVB */
            else if (modrm >= 0xc8 && modrm <= 0xcf) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_E; }  /* FCMOVE */
            else if (modrm >= 0xd0 && modrm <= 0xd7) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_BE; } /* FCMOVBE */
            else if (modrm >= 0xd8 && modrm <= 0xdf) { out->opcode = HB_INS_X87_FCMOV; out->cond = HB_COND_P; }  /* FCMOVU */
            else if (modrm == 0xe9) {
...
static hb_result_t decode_one(hb_dec_t* d, hb_decoded_t* out) {
    ...
    while (can_read(d, 1)) {
            ...
            b == 0xf0 || b == 0xf2 || b == 0xf3)
        {
            ...
            else if (b == 0x67) { address16 = true; out->address32_prefix = true; }
    ...
    bool addr16_modrm_supported =
        ((opcode <= 0x3D) && ((opcode & 7) <= 5)) ||
        (opcode >= 0x69 && opcode <= 0x6B) ||
        (opcode >= 0x80 && opcode <= 0x8D) ||
    ...
    if ((operand16 && !((opcode <= 0x3D) && ((opcode & 7) <= 5)) &&
        ...
        (address16 && !suppress_addr16 && !addr16_modrm_supported) ||
