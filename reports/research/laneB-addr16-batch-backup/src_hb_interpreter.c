...
static uint64_t resolve_addr(hb_context_t* ctx, const hb_ir_operand_t* op) {
    ...
    if (ctx->mode == HB_MODE_32BIT) {
        if (op->mem.addr32) {
            uint32_t base16 = (uint16_t)base;
            uint32_t index16 = (uint16_t)index;
            return (uint16_t)(base16 + index16 * op->mem.scale + (uint32_t)op->mem.disp);
        }
...
static hb_result_t exec_instr(hb_context_t* ctx, const hb_ir_instr_t* instr) {
    ...
    switch (instr->op) {
        ...
        case HB_IR_CMOVcc: {
            ...
            if (r != HB_OK) return r;
            if (instr->dst.type == HB_OP_NONE) {
                if (instr->src1.type != HB_OP_IMM || instr->src1.imm < 0 || instr->src1.imm > 7) {
                    return HB_ERR_INTERNAL;
                }
                if (value) {
                    double src = 0.0;
                    r = hb_x87_st_f64(&ctx->regs.x86.x87, (unsigned)instr->src1.imm, &src);
                    if (r != HB_OK) return r;
                    return hb_x87_set_st_f64(&ctx->regs.x86.x87, 0, src);
                }
                return HB_OK;
            }
            if (instr->dst.type != HB_OP_REG) return HB_ERR_INTERNAL;
        ...
        case HB_IR_LODS: {
            ...
            while (count > 0) {
                ...
                r = mem_read(ctx, rsi, &value, size);
                if (r != HB_OK) return r;
                /* LODS writes the accumulator part selected by operand size:
                 * AL/AX/EAX/RAX. Byte/word forms preserve the remaining bits;
                 * in 64-bit mode, the dword form writes EAX and zero-extends. */
                value = trunc_to_size(value, size);
                if (mode32) {
                    uint32_t eax = ctx->regs.x86.eax;
                    if (size == HB_SIZE_8) {
                        eax = (eax & ~0xffu) | (uint32_t)value;
                    } else if (size == HB_SIZE_16) {
                        eax = (eax & ~0xffffu) | (uint32_t)value;
                    } else {
                        eax = (uint32_t)value;
                    }
                    ctx->regs.x86.eax = eax;
                } else {
                    uint64_t rax = ctx->regs.x64.rax;
                    if (size == HB_SIZE_8) {
                        rax = (rax & ~0xffULL) | value;
                    } else if (size == HB_SIZE_16) {
                        rax = (rax & ~0xffffULL) | value;
                    } else if (size == HB_SIZE_32) {
                        rax = (uint32_t)value;
                    } else {
                        rax = value;
