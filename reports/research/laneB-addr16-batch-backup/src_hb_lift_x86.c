...
hb_result_t hb_lift_x86(const hb_decoded_t* dec, hb_ir_builder_t* b) {
    ...
    switch (dec->opcode) {
        ...
        case HB_INS_X87_FCMOV:
            /* FCMOVcc ST, ST(i): reuse CMOVcc with dst=none as the x87
             * sentinel and src1.imm carrying the logical ST(i) index. */
            emit(b, hb_ir_emit_cmovcc(b, cc_from_dec(dec->cond),
                                      hb_ir_none(), operand_from_dec(dec, 1)), dec);
            return HB_OK;
        case HB_INS_PUSHA: {
