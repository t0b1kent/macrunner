...
static int run_x86_interp_bytes(hb_context_t* ctx, const uint8_t* code, size_t len,
                                uint32_t code_base, hb_exec_result_t* exec) {
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X86, code, len, code_base);
    hb_ir_func_t* func = NULL;
    if (!ctx || !code || !exec || !dec) return 0;
    if (hb_lift_func_x86(dec, &func) != HB_OK) {
    ...
    if (hb_memory_guest32_map(ctx->memory, code_base, 4096,
                              HB_PERM_READ | HB_PERM_WRITE | HB_PERM_EXEC) != HB_OK) {
        hb_ir_func_destroy(func);
        return 0;
    }
    if (hb_memory_write(ctx->memory, code_base, code, len) != HB_OK) {
        hb_ir_func_destroy(func);
        return 0;
    }
    ctx->pc = code_base;
    ctx->regs.x86.eip = code_base;
    hb_result_t r = hb_runtime_run(ctx, func, HB_BACKEND_INTERP, exec);
    hb_ir_func_destroy(func);
    return r == HB_OK && exec->result == HB_OK && !exec->faulted;
}
...
TEST(decode_x64_0f_system_segment_family) {
    ...
    uint8_t random_mov_rsi_cr1[] = {0x0F, 0x20, 0x8E, 0x68, 0xAA, 0x83};
    uint8_t random_mov_dr3_rbx[] = {0x0F, 0x23, 0x1B, 0x00};
    uint8_t random_mov_r10_dr2[] = {0x41, 0x0F, 0x21, 0x52};
    ...
    ASSERT(hb_decode_x64(random_mov_rsi_cr1, sizeof(random_mov_rsi_cr1), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_MOV_CR && d.len == 3);
    ASSERT(d.op1.is_reg && d.op1.reg == HB_REG_RSI && d.op1.size == 8);
    ASSERT(d.op2.is_imm && d.op2.imm == 1);

    ASSERT(hb_decode_x64(random_mov_dr3_rbx, sizeof(random_mov_dr3_rbx), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_MOV_DR && d.len == 3);
    ASSERT(d.op1.is_imm && d.op1.imm == 3);
    ASSERT(d.op2.is_reg && d.op2.reg == HB_REG_RBX && d.op2.size == 8);

    ASSERT(hb_decode_x64(random_mov_r10_dr2, sizeof(random_mov_r10_dr2), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_MOV_DR && d.len == 4);
    ASSERT(d.op1.is_reg && d.op1.reg == HB_REG_R10 && d.op1.size == 8);
...
TEST(decode_x64_prefix_pop_hlt_family) {
    ...
    uint8_t rex_before_segment_mov_imm[] = {0x49, 0x64, 0xBB, 0x59, 0x63, 0x6E, 0x4A, 0x84};
    ...
    ASSERT(hb_decode_x64(rex_before_segment_mov_imm, sizeof(rex_before_segment_mov_imm), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_MOV && d.len == 7);
    ASSERT(d.op1.is_reg && d.op1.reg == HB_REG_RBX && d.op1.size == 4);
    ASSERT(d.op2.is_imm && d.op2.imm == 0x4a6e6359);
...
TEST(decode_x64_0f_unsupported_family_lengths) {
    ...
    uint8_t mmx_psrad_imm[] = {0x0F, 0x72, 0xE1, 0x21};
    uint8_t vmread[] = {0x0F, 0x78, 0xC0};
    uint8_t lgs_random[] = {0x0F, 0xB5, 0x29, 0xBB};
    ...
    uint8_t xbegin_random[] = {0xC7, 0xF8, 0xD9, 0x6B, 0x81, 0xFD};
    hb_decoded_t d;
    ...
    ASSERT(hb_decode_x64(mmx_psrad_imm, sizeof(mmx_psrad_imm), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_MMX && d.len == 4 && d.op2.imm == 0x21);

    ASSERT(hb_decode_x64(vmread, sizeof(vmread), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SYS && d.len == 3);

    ASSERT(hb_decode_x64(lgs_random, sizeof(lgs_random), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SYS && d.len == 3);
    ...
    ASSERT(d.opcode == HB_INS_POPCNT && d.len == 4);

    ASSERT(hb_decode_x64(xbegin_random, sizeof(xbegin_random), 0x1000, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SYS && d.len == 6);
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, xbegin_random, sizeof(xbegin_random), 0x1000);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ASSERT(func != NULL && func->has_unsupported);
    hb_ir_func_destroy(func);
    tests_passed++;
}
...
TEST(interp_x86_unicorn_diff_regressions_rcl_rcr) {
    const uint32_t code_base = 0x00403000u;
    uint8_t rcl_eax_cl[] = {0xd3, 0xd0};
    hb_exec_result_t exec;
    hb_context_t* ctx = hb_context_create(HB_ARCH_X86, HB_BACKEND_INTERP);
    ASSERT(ctx != NULL);

    ctx->regs.x86.eax = 0x70001000u;
    ctx->regs.x86.ecx = 0x0000000du;
    ctx->flags.cf = false;
    ASSERT(run_x86_interp_bytes(ctx, rcl_eax_cl, sizeof(rcl_eax_cl), code_base, &exec));
    ASSERT(ctx->regs.x86.eax == 0x02000700u);
    ASSERT(ctx->regs.x86.eip == code_base + sizeof(rcl_eax_cl));
...
TEST(interp_x86_unicorn_diff_regressions_sse2) {
    const uint32_t code_base = 0x00404000u;
    uint8_t movmskps[] = {0x0f, 0x50, 0xc0};
    uint8_t shufps[] = {0x0f, 0xc6, 0xc1, 0x1b};
    uint8_t pmullw[] = {0x66, 0x0f, 0xd5, 0xc1};
    hb_exec_result_t exec;
    hb_context_t* ctx = hb_context_create(HB_ARCH_X86, HB_BACKEND_INTERP);
    ASSERT(ctx != NULL);

    uint32_t mov_lanes[4] = {0x80000000u, 0x00000000u, 0x80000000u, 0x00000000u};
    memcpy(ctx->regs.x86.xmm[0], mov_lanes, sizeof(mov_lanes));
    ASSERT(run_x86_interp_bytes(ctx, movmskps, sizeof(movmskps), code_base, &exec));
    ASSERT(ctx->regs.x86.eax == 0x5u);

    hb_context_destroy(ctx);
    ctx = hb_context_create(HB_ARCH_X86, HB_BACKEND_INTERP);
    ASSERT(ctx != NULL);
    uint32_t dst_lanes[4] = {0x11111111u, 0x22222222u, 0x33333333u, 0x44444444u};
    uint32_t src_lanes[4] = {0xaaaaaaaau, 0xbbbbbbbbu, 0xccccccccu, 0xddddddddu};
    uint32_t expected_shuf[4] = {0x44444444u, 0x33333333u, 0xbbbbbbbbu, 0xaaaaaaaau};
    memcpy(ctx->regs.x86.xmm[0], dst_lanes, sizeof(dst_lanes));
    memcpy(ctx->regs.x86.xmm[1], src_lanes, sizeof(src_lanes));
    ASSERT(run_x86_interp_bytes(ctx, shufps, sizeof(shufps), code_base, &exec));
    ASSERT(memcmp(ctx->regs.x86.xmm[0], expected_shuf, sizeof(expected_shuf)) == 0);

    hb_context_destroy(ctx);
    ctx = hb_context_create(HB_ARCH_X86, HB_BACKEND_INTERP);
    ASSERT(ctx != NULL);
    uint16_t mul_a[8] = {2, 3, 4, 5, 6, 7, 8, 9};
    uint16_t mul_b[8] = {10, 11, 12, 13, 14, 15, 16, 17};
    uint16_t expected_mul[8] = {20, 33, 48, 65, 84, 105, 128, 153};
    memcpy(ctx->regs.x86.xmm[0], mul_a, sizeof(mul_a));
    memcpy(ctx->regs.x86.xmm[1], mul_b, sizeof(mul_b));
    ASSERT(run_x86_interp_bytes(ctx, pmullw, sizeof(pmullw), code_base, &exec));
    ASSERT(memcmp(ctx->regs.x86.xmm[0], expected_mul, sizeof(expected_mul)) == 0);
...
TEST(interp_x86_unicorn_diff_regressions_x87_constants) {
    const uint32_t code_base = 0x00405000u;
    uint8_t fld1_fnstsw[] = {0xd9, 0xe8, 0xdf, 0xe0};
    uint8_t fldz_fnstsw[] = {0xd9, 0xee, 0xdf, 0xe0};
    hb_exec_result_t exec;

    hb_context_t* ctx = hb_context_create(HB_ARCH_X86, HB_BACKEND_INTERP);
    ASSERT(ctx != NULL);
    ctx->regs.x86.eax = 0x70001000u;
    ASSERT(run_x86_interp_bytes(ctx, fld1_fnstsw, sizeof(fld1_fnstsw), code_base, &exec));
    ASSERT(ctx->regs.x86.eax == 0x70003800u);
    ASSERT(ctx->regs.x86.x87.status_word == 0x3800u);
    ASSERT(ctx->regs.x86.x87.tag_word == 0x3fffu);
    hb_context_destroy(ctx);

    ctx = hb_context_create(HB_ARCH_X86, HB_BACKEND_INTERP);
    ASSERT(ctx != NULL);
    ctx->regs.x86.eax = 0x70001000u;
    ASSERT(run_x86_interp_bytes(ctx, fldz_fnstsw, sizeof(fldz_fnstsw), code_base, &exec));
    ASSERT(ctx->regs.x86.eax == 0x70003800u);
    ASSERT(ctx->regs.x86.x87.status_word == 0x3800u);
...
TEST(x87_fxam_class_condition_codes) {
    hb_x87_state_t x87;
    const uint16_t cc_mask = (uint16_t)((1u << 8) | (1u << 9) | (1u << 10) | (1u << 14));
    const uint16_t c0 = (uint16_t)(1u << 8);
    const uint16_t c1 = (uint16_t)(1u << 9);
    const uint16_t c2 = (uint16_t)(1u << 10);
    const uint16_t c3 = (uint16_t)(1u << 14);

    hb_x87_reset(&x87);
    ASSERT(hb_x87_push_f64(&x87, 1.25) == HB_OK);
    ASSERT(hb_x87_fxam(&x87) == HB_OK);
    ASSERT((x87.status_word & cc_mask) == c2);

    hb_x87_reset(&x87);
    ASSERT(hb_x87_push_f64(&x87, -2.0) == HB_OK);
    ASSERT(hb_x87_fxam(&x87) == HB_OK);
    ASSERT((x87.status_word & cc_mask) == (uint16_t)(c1 | c2));

    hb_x87_reset(&x87);
    ASSERT(hb_x87_push_f64(&x87, 0.0) == HB_OK);
    ASSERT(hb_x87_fxam(&x87) == HB_OK);
    ASSERT((x87.status_word & cc_mask) == c3);

    hb_x87_reset(&x87);
    ASSERT(hb_x87_push_f64(&x87, INFINITY) == HB_OK);
    ASSERT(hb_x87_fxam(&x87) == HB_OK);
    ASSERT((x87.status_word & cc_mask) == (uint16_t)(c0 | c2));

    hb_x87_reset(&x87);
    ASSERT(hb_x87_push_f64(&x87, NAN) == HB_OK);
    ASSERT(hb_x87_fxam(&x87) == HB_OK);
    ASSERT((x87.status_word & cc_mask) == c0);

    hb_x87_reset(&x87);
    ASSERT(hb_x87_fxam(&x87) == HB_OK);
    ASSERT((x87.status_word & cc_mask) == (uint16_t)(c0 | c3));

    tests_passed++;
}

TEST(x87_fprem_condition_codes) {
    hb_x87_state_t x87;
    const uint16_t cc_mask = (uint16_t)((1u << 8) | (1u << 9) | (1u << 10) | (1u << 14));
    const uint16_t c0 = (uint16_t)(1u << 8);
    const uint16_t c1 = (uint16_t)(1u << 9);
    ...
    hb_x87_reset(&x87);
    ASSERT(hb_x87_push_f64(&x87, 2.0) == HB_OK);
    ASSERT(hb_x87_push_f64(&x87, 7.0) == HB_OK);
    ASSERT(hb_x87_fprem(&x87) == HB_OK);
    ASSERT(hb_x87_st_f64(&x87, 0, &value) == HB_OK);
    ASSERT(value == 1.0);
    ASSERT((x87.status_word & cc_mask) == (uint16_t)(c1 | c3));

    hb_x87_reset(&x87);
    ASSERT(hb_x87_push_f64(&x87, 2.0) == HB_OK);
    ASSERT(hb_x87_push_f64(&x87, 7.0) == HB_OK);
    ASSERT(hb_x87_fprem1(&x87) == HB_OK);
    ASSERT(hb_x87_st_f64(&x87, 0, &value) == HB_OK);
    ASSERT(value == -1.0);
    ASSERT((x87.status_word & cc_mask) == c0);
...
TEST(interp_x64_x87_fld_tbyte_memory_form) {
    struct {
        uint8_t code[6];
        uint8_t mem[10];
    } block = {
        {0xdb, 0x2d, 0x00, 0x00, 0x00, 0x00},
        {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0xff, 0x3f}
    };
    uint64_t base = (uint64_t)(uintptr_t)&block;
    hb_decoded_t d;
    ASSERT(hb_decode_x64(block.code, sizeof(block.code), base, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_X87_FLD);
    ASSERT(d.len == sizeof(block.code));
    ASSERT(d.op1.is_mem && d.op1.size == 10 && d.op1.mem.rip_relative);
...
TEST(interp_x86_x87_fcmov_condition_family) {
    struct {
        uint8_t code[2];
        bool cf, zf, pf;
        double expected;
    } cases[] = {
        {{0xda, 0xc1}, true,  false, false, 5.0}, /* FCMOVB  ST, ST(1) */
        {{0xdb, 0xc1}, false, false, false, 5.0}, /* FCMOVNB ST, ST(1) */
        {{0xda, 0xc9}, false, true,  false, 5.0}, /* FCMOVE  ST, ST(1) */
        {{0xdb, 0xc9}, false, true,  false, 2.0}, /* FCMOVNE not taken */
        {{0xda, 0xd1}, true,  false, false, 5.0}, /* FCMOVBE ST, ST(1) */
        {{0xdb, 0xd1}, false, false, false, 5.0}, /* FCMOVNBE ST, ST(1) */
        {{0xda, 0xd9}, false, false, true,  5.0}, /* FCMOVU  ST, ST(1) */
        {{0xdb, 0xd9}, false, false, false, 5.0}, /* FCMOVNU ST, ST(1) */
    };
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        double got = 0.0;
        ASSERT(ctx != NULL);
        ASSERT(hb_x87_push_f64(&ctx->regs.x86.x87, 5.0) == HB_OK);
        ASSERT(hb_x87_push_f64(&ctx->regs.x86.x87, 2.0) == HB_OK);
        hb_lazy_flags_clear(ctx);
        ctx->flags.cf = cases[i].cf;
        ctx->flags.zf = cases[i].zf;
        ctx->flags.pf = cases[i].pf;

        ASSERT(run_x86_interp_bytes(ctx, cases[i].code, sizeof(cases[i].code),
                                    0x1000, &exec));
        ASSERT(hb_x87_st_f64(&ctx->regs.x86.x87, 0, &got) == HB_OK);
        ASSERT(test_double_bits(got) == test_double_bits(cases[i].expected));
        hb_context_destroy(ctx);
    }
...
TEST(interp_x86_lodsb_preserves_eax_high) {
    /* LODSB writes AL only and advances ESI by 1. */
    const uint32_t code_base = 0x00407000u;
...
TEST(interp_x86_addr16_lea_bx_si) {
    /* 0x67 in 32-bit mode switches ModRM memory addressing to the 16-bit
     * table. 67 8D 00 is LEA EAX,[BX+SI], not LEA EAX,[EAX]. */
    const uint32_t code_base = 0x0040C000u;
    uint8_t code[] = {0x67, 0x8d, 0x00};
    ...
    ctx->pc = code_base;
    ctx->regs.x86.eip = code_base;
    ctx->regs.x86.ebx = 0x00001234u;
    ctx->regs.x86.esi = 0x00000056u;
    ctx->regs.x86.eax = 0xDEADBEEFu;

    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x86.eax == 0x0000128Au);
    ASSERT(ctx->regs.x86.eip == code_base + sizeof(code));
...
TEST(interp_x64_lodsq_overwrites_rax) {
    uint8_t code[] = {0x48, 0xad}; /* lodsq */
    uint64_t base = (uint64_t)(uintptr_t)code;
    ...
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.rax == 0x0102030405060708ULL);
    ASSERT(ctx->regs.x64.rsi == addr + 8);
...
TEST(interp_x64_lodsw_preserves_rax_high) {
    ...
    ASSERT(hb_memory_map(ctx->memory, addr, sizeof(data), HB_PERM_READ) == HB_OK);
    ctx->pc = base;
    ctx->regs.x64.rax = 0xfeedfacecafebabeULL;
    ctx->regs.x64.rsi = addr;

    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.rax == 0xfeedfacecafe1234ULL);
...
TEST(jit_x64_native_stack_push_pop_family) {
    ...
    char* saved_stack = save_env_var("MACRUNNER_HB_JIT_DIRECT_STACK");
    setenv("MACRUNNER_HB_JIT_DIRECT_MEM", "1", 1);
    setenv("MACRUNNER_HB_JIT_DIRECT_STACK", "1", 1);
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ...
    restore_env_var("MACRUNNER_HB_JIT_DIRECT_STACK", saved_stack);
...
TEST(jit_x64_direct_stack_ret_without_direct_mem) {
    uint64_t stack[1] = {0x5566778899aabbccULL};
    hb_ir_func_t* func = hb_ir_func_create(0x4410, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* blk = hb_ir_block_create(0, 0x4410);
    ASSERT(blk != NULL);
    hb_ir_cfg_add_block(func->cfg, blk);
    func->cfg->entry = blk;
    ...
    hb_ir_instr_t* ret = hb_ir_emit_ret(b);
    ASSERT(ret != NULL);
    ret->guest_addr = 0x4410; ret->guest_len = 1;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ASSERT(ctx->pc == stack[0]);
    ASSERT(ctx->regs.x64.rsp == (uint64_t)(uintptr_t)&stack[1]);
...
TEST(jit_x64_mid_block_control_transfer_stops_fallthrough) {
    uint64_t jit_stack[8] = {0};
    uint64_t interp_stack[8] = {0};
    const uint64_t entry = 0x4a00;
    const uint64_t call_pc = 0x4a08;
    const uint64_t target = 0x9900;
    const uint64_t ret_pc = call_pc + 2;

    hb_ir_func_t* func = hb_ir_func_create(entry, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* blk = hb_ir_block_create(0, entry);
    ASSERT(blk != NULL);
    hb_ir_cfg_add_block(func->cfg, blk);
    func->cfg->entry = blk;
    ...
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, blk);
    hb_ir_emit_mov(b, hb_ir_reg(HB_REG_RBX, HB_SIZE_64), hb_ir_imm(0x1111, HB_SIZE_64));
    hb_ir_instr_t* call = hb_ir_emit_call(b, 0);
    ASSERT(call != NULL);
    call->src1 = hb_ir_reg(HB_REG_RAX, HB_SIZE_64);
    call->guest_addr = call_pc;
    call->guest_len = 2;
    hb_ir_emit_mov(b, hb_ir_reg(HB_REG_RBX, HB_SIZE_64), hb_ir_imm(0x2222, HB_SIZE_64));
    hb_ir_builder_destroy(b);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)jit_stack, sizeof(jit_stack),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = entry;
    ctx->regs.x64.rip = entry;
    ctx->regs.x64.rax = target;
    ctx->regs.x64.rsp = (uint64_t)(uintptr_t)&jit_stack[4];
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == target);
    ASSERT(ctx->regs.x64.rip == target);
    ASSERT(ctx->regs.x64.rbx == 0x1111);
    ASSERT(ctx->regs.x64.rsp == (uint64_t)(uintptr_t)&jit_stack[3]);
    ASSERT(jit_stack[3] == ret_pc);
    hb_context_destroy(ctx);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)interp_stack, sizeof(interp_stack),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = entry;
    ctx->regs.x64.rip = entry;
    ctx->regs.x64.rax = target;
    ctx->regs.x64.rsp = (uint64_t)(uintptr_t)&interp_stack[4];
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == target);
    ASSERT(ctx->regs.x64.rip == target);
    ASSERT(ctx->regs.x64.rbx == 0x1111);
    ASSERT(ctx->regs.x64.rsp == (uint64_t)(uintptr_t)&interp_stack[3]);
    ASSERT(interp_stack[3] == ret_pc);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    tests_passed++;
}
...
TEST(jit_x64_xfg_dispatch_call_shortcuts_guard_jmp_rax) {
    uint8_t mem[512] __attribute__((aligned(16)));
    uint8_t* code = mem;
    uint8_t* stub = mem + 128;
    uint8_t* impl = mem + 256;
    uint8_t* stack = mem + 384;
    uint64_t base = (uint64_t)(uintptr_t)code;
    uint64_t stub_addr = (uint64_t)(uintptr_t)stub;
    uint64_t impl_addr = (uint64_t)(uintptr_t)impl;
    uint64_t target = 0x123456789abcdef0ULL;
    uint64_t hash = 0x85f13e9656da4870ULL;
    uint64_t ret_addr = 0;
    memset(mem, 0x90, sizeof(mem));

    code[0] = 0x49; code[1] = 0xba;          /* movabs hash, %r10 */
    memcpy(code + 2, &hash, sizeof(hash));
    code[10] = 0xff; code[11] = 0x15;        /* callq *0x8(%rip) */
    int32_t call_disp = 8;
    memcpy(code + 12, &call_disp, sizeof(call_disp));
    memcpy(code + 24, &stub_addr, sizeof(stub_addr));

    stub[0] = 0xff; stub[1] = 0x25;          /* jmpq *0x2(%rip) */
    int32_t stub_disp = 2;
    memcpy(stub + 2, &stub_disp, sizeof(stub_disp));
    memcpy(stub + 8, &impl_addr, sizeof(impl_addr));
    impl[0] = 0xff; impl[1] = 0xe0;          /* jmpq *%rax */

    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, code, 32, base);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)mem, sizeof(mem),
                         HB_PERM_READ | HB_PERM_WRITE | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    ctx->regs.x64.rsp = (uint64_t)(uintptr_t)(stack + 64);
    ctx->regs.x64.rax = target;
    ...
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == target);
    ASSERT(ctx->regs.x64.r10 == hash);
    ASSERT(ctx->regs.x64.rsp == (uint64_t)(uintptr_t)(stack + 56));
    ASSERT(hb_memory_read_u64(ctx->memory, ctx->regs.x64.rsp, &ret_addr) == HB_OK);
    ASSERT(ret_addr == base + 16);
...
TEST(jit_x64_xfg_dispatch_call_fallback_preserves_normal_target) {
    uint8_t mem[512] __attribute__((aligned(16)));
    uint8_t* code = mem;
    uint8_t* stub = mem + 128;
    uint8_t* stack = mem + 384;
    ...
    code[0] = 0x49; code[1] = 0xba;
    memcpy(code + 2, &hash, sizeof(hash));
    code[10] = 0xff; code[11] = 0x15;
    int32_t call_disp = 8;
    memcpy(code + 12, &call_disp, sizeof(call_disp));
    memcpy(code + 24, &stub_addr, sizeof(stub_addr));
    stub[0] = 0xc3;                           /* not an XFG dispatch stub */
...
TEST(jit_x64_helper_copy_scan_counted_loop_promotes_cache_pair) {
    uint8_t src[3] = {'a', 'b', 0};
    uint8_t dst[3] = {0xaa, 0xbb, 0xcc};
    hb_ir_func_t* body_func = hb_ir_func_create(0x3b80, 0);
    hb_ir_func_t* guard_func = hb_ir_func_create(0x3b8e, 0);
    ASSERT(body_func && guard_func);
    hb_ir_block_t* body = hb_ir_block_create(0, 0x3b80);
    hb_ir_block_t* guard = hb_ir_block_create(0, 0x3b8e);
    ASSERT(body && guard);
    hb_ir_cfg_add_block(body_func->cfg, body);
    hb_ir_cfg_add_block(guard_func->cfg, guard);
    body_func->cfg->entry = body;
    ...
    hb_ir_instr_t* test = hb_ir_emit_test(b, hb_ir_reg(HB_REG_RAX, HB_SIZE_8),
                                          hb_ir_reg(HB_REG_RAX, HB_SIZE_8));
    hb_ir_instr_t* body_jcc = hb_ir_emit_jcc(b, HB_CC_E, 0x3ba0);
    ASSERT(load && store && add && test && body_jcc);
    load->guest_addr = 0x3b80; load->guest_len = 4;
    store->guest_addr = 0x3b84; store->guest_len = 3;
    add->guest_addr = 0x3b87; add->guest_len = 3;
    test->guest_addr = 0x3b8a; test->guest_len = 2;
    body_jcc->guest_addr = 0x3b8c; body_jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    hb_ir_instr_t* sub = hb_ir_emit_binop(b, HB_IR_SUB, hb_ir_reg(HB_REG_RDX, HB_SIZE_64),
                                          hb_ir_reg(HB_REG_RDX, HB_SIZE_64),
                                          hb_ir_imm(1, HB_SIZE_64));
    hb_ir_instr_t* guard_jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x3b80);
    ASSERT(sub && guard_jcc);
    sub->guest_addr = 0x3b8e; sub->guest_len = 4;
    guard_jcc->guest_addr = 0x3b92; guard_jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(hb_jit_runtime_run(rt, body_func, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == 0x3b8e);

    ASSERT(hb_jit_runtime_run(rt, guard_func, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == 0x3b80);
...
TEST(jit_x64_helper_byte_compare_loop_promotes_cache_pair) {
    uint8_t lhs_text[4] = {'a', 'b', 'c', 0};
    uint8_t rhs_text[4] = {'a', 'b', 'c', 0};
    hb_ir_func_t* cmp_func = hb_ir_func_create(0x3d00, 0);
    hb_ir_func_t* back_func = hb_ir_func_create(0x3d0c, 0);
    ASSERT(cmp_func && back_func);
    hb_ir_block_t* cmp_block = hb_ir_block_create(0, 0x3d00);
    hb_ir_block_t* back_block = hb_ir_block_create(0, 0x3d0c);
    ...
    hb_ir_instr_t* sub = hb_ir_emit_binop(b, HB_IR_SUB, hb_ir_reg(HB_REG_RDX, HB_SIZE_32),
                                          hb_ir_reg(HB_REG_RDX, HB_SIZE_32),
                                          hb_ir_reg(HB_REG_RCX, HB_SIZE_32));
    hb_ir_instr_t* cmp_jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x3d30);
    ASSERT(lhs && rhs && sub && cmp_jcc);
    lhs->guest_addr = 0x3d00; lhs->guest_len = 3;
    rhs->guest_addr = 0x3d03; rhs->guest_len = 5;
    sub->guest_addr = 0x3d08; sub->guest_len = 2;
    cmp_jcc->guest_addr = 0x3d0a; cmp_jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    hb_ir_instr_t* test = hb_ir_emit_test(b, hb_ir_reg(HB_REG_RCX, HB_SIZE_32),
                                          hb_ir_reg(HB_REG_RCX, HB_SIZE_32));
    hb_ir_instr_t* back_jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x3d00);
    ASSERT(inc && test && back_jcc);
    inc->guest_addr = 0x3d0c; inc->guest_len = 3;
    test->guest_addr = 0x3d0f; test->guest_len = 2;
    back_jcc->guest_addr = 0x3d11; back_jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == 0x3d0c);

    ASSERT(hb_jit_runtime_run(rt, back_func, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == 0x3d00);

    ASSERT(hb_jit_runtime_run(rt, cmp_func, &out) == HB_OK);
    restore_env_var("MACRUNNER_HB_JIT_DIRECT_MEM", saved);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ASSERT(ctx->regs.x64.rax == (uint64_t)(uintptr_t)(lhs_text + 4));
    ASSERT(ctx->regs.x64.rcx == 0);
    ASSERT(ctx->regs.x64.rdx == 0);
...
TEST(jit_x64_helper_unity_string_bsearch_loop_promotes_block) {
    uint8_t ant[] = "ant";
    uint8_t bee[] = "bee";
    uint8_t cat[] = "cat";
    uint8_t dog[] = "dog";
    ...
    uint8_t key_eel[] = "eel";
    uint64_t table[8] = {
        (uint64_t)(uintptr_t)ant, 0,
        (uint64_t)(uintptr_t)bee, 0,
        (uint64_t)(uintptr_t)cat, 0,
        (uint64_t)(uintptr_t)dog, 0
    };
    const uint64_t base = 0x6c4451;
    hb_ir_func_t* func = hb_ir_func_create(base, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* block = hb_ir_block_create(0, base);
    ASSERT(block != NULL);
    ...
    hb_ir_instr_t* mov_r10 = hb_ir_emit_mov(b, hb_ir_reg(HB_REG_R10, HB_SIZE_64),
                                            hb_ir_reg(HB_REG_R9, HB_SIZE_64));
    hb_ir_instr_t* mov_r8 = hb_ir_emit_mov(b, hb_ir_reg(HB_REG_R8, HB_SIZE_64),
                                           hb_ir_reg(HB_REG_RDI, HB_SIZE_64));
    hb_ir_instr_t* shr = hb_ir_emit_binop(b, HB_IR_SHR, hb_ir_reg(HB_REG_R10, HB_SIZE_64),
                                          hb_ir_reg(HB_REG_R10, HB_SIZE_64),
                                          hb_ir_imm(1, HB_SIZE_64));
    hb_ir_instr_t* mov_r11 = hb_ir_emit_mov(b, hb_ir_reg(HB_REG_R11, HB_SIZE_64),
                                            hb_ir_reg(HB_REG_R10, HB_SIZE_64));
    hb_ir_instr_t* add_r11 = hb_ir_emit_binop(b, HB_IR_ADD, hb_ir_reg(HB_REG_R11, HB_SIZE_64),
                                              hb_ir_reg(HB_REG_R11, HB_SIZE_64),
                                              hb_ir_reg(HB_REG_R11, HB_SIZE_64));
    hb_ir_instr_t* load = hb_ir_emit_load(b, hb_ir_reg(HB_REG_RAX, HB_SIZE_64),
                                          hb_ir_mem(HB_REG_RBX, HB_REG_R11, 8, 0, HB_SIZE_64));
    hb_ir_instr_t* sub_r8 = hb_ir_emit_binop(b, HB_IR_SUB, hb_ir_reg(HB_REG_R8, HB_SIZE_64),
                                             hb_ir_reg(HB_REG_R8, HB_SIZE_64),
                                             hb_ir_reg(HB_REG_RAX, HB_SIZE_64));
    hb_ir_instr_t* nop = hb_ir_emit(b, HB_IR_NOP);
    hb_ir_instr_t* lhs = hb_ir_emit_unop(b, HB_IR_ZERO_EXTEND,
                                         hb_ir_reg(HB_REG_RDX, HB_SIZE_32),
                                         hb_ir_mem(HB_REG_RAX, HB_REG_COUNT, 1, 0, HB_SIZE_8));
    hb_ir_instr_t* rhs = hb_ir_emit_unop(b, HB_IR_ZERO_EXTEND,
                                         hb_ir_reg(HB_REG_RCX, HB_SIZE_32),
                                         hb_ir_mem(HB_REG_RAX, HB_REG_R8, 1, 0, HB_SIZE_8));
    hb_ir_instr_t* sub_cmp = hb_ir_emit_binop(b, HB_IR_SUB, hb_ir_reg(HB_REG_RDX, HB_SIZE_32),
                                              hb_ir_reg(HB_REG_RDX, HB_SIZE_32),
                                              hb_ir_reg(HB_REG_RCX, HB_SIZE_32));
    hb_ir_instr_t* jne = hb_ir_emit_jcc(b, HB_CC_NE, base + 0x32);
    ASSERT(mov_r10 && mov_r8 && shr && mov_r11 && add_r11 && load && sub_r8 &&
           nop && lhs && rhs && sub_cmp && jne);
    hb_ir_instr_t* instrs[] = {mov_r10, mov_r8, shr, mov_r11, add_r11, load,
                               sub_r8, nop, lhs, rhs, sub_cmp, jne};
    for (size_t n = 0; n < sizeof(instrs) / sizeof(instrs[0]); n++) {
        instrs[n]->guest_addr = base + n;
        instrs[n]->guest_len = 1;
    }
    hb_ir_builder_destroy(b);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)ant, sizeof(ant), HB_PERM_READ) == HB_OK);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)bee, sizeof(bee), HB_PERM_READ) == HB_OK);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)cat, sizeof(cat), HB_PERM_READ) == HB_OK);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)dog, sizeof(dog), HB_PERM_READ) == HB_OK);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)key_cat, sizeof(key_cat), HB_PERM_READ) == HB_OK);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)key_eel, sizeof(key_eel), HB_PERM_READ) == HB_OK);
    ...
    ctx->regs.x64.rdi = (uint64_t)(uintptr_t)key_cat;
    ctx->regs.x64.r9 = 4;
    ASSERT(hb_jit_runtime_run(rt, func, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(ctx->pc, base + 0x55);
    ASSERT_EQ(ctx->regs.x64.rbx, (uint64_t)(uintptr_t)&table[4]);
    ASSERT_EQ(ctx->regs.x64.r9, 0);

    bool greater = true;
    ASSERT(hb_flags_eval_cond(ctx, HB_CC_G, &greater) == HB_OK);
    ASSERT(!greater);

    ctx->pc = base;
    ctx->regs.x64.rbx = (uint64_t)(uintptr_t)table;
    ctx->regs.x64.rdi = (uint64_t)(uintptr_t)key_eel;
    ctx->regs.x64.r9 = 4;
    ASSERT(hb_jit_runtime_run(rt, func, &out) == HB_OK);
    restore_env_var("MACRUNNER_HB_JIT_DIRECT_MEM", saved);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(ctx->pc, base + 0x55);
    ASSERT_EQ(ctx->regs.x64.rbx, (uint64_t)(uintptr_t)&table[8]);
    ASSERT_EQ(ctx->regs.x64.r9, 0);
...
TEST(jit_x64_helper_load_cmp_jcc_block) {
    ...
    ASSERT(func != NULL);
    hb_ir_block_t* block = hb_ir_block_create(0, 0x3e00);
    ASSERT(block != NULL);
    hb_ir_cfg_add_block(func->cfg, block);
    func->cfg->entry = block;
    ...
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, block);
    hb_ir_instr_t* load = hb_ir_emit_load(b, hb_ir_reg(HB_REG_RAX, HB_SIZE_32),
                                          hb_ir_mem(HB_REG_RDX, HB_REG_COUNT, 1, 0, HB_SIZE_32));
    hb_ir_instr_t* cmp = hb_ir_emit_cmp(b, hb_ir_mem(HB_REG_RCX, HB_REG_COUNT, 1, 0, HB_SIZE_32),
                                        hb_ir_reg(HB_REG_RAX, HB_SIZE_32));
    hb_ir_instr_t* jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x3e20);
    ASSERT(load && cmp && jcc);
    load->guest_addr = 0x3e00; load->guest_len = 2;
    cmp->guest_addr = 0x3e02; cmp->guest_len = 2;
    jcc->guest_addr = 0x3e04; jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&lhs, sizeof(lhs), HB_PERM_READ) == HB_OK);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&rhs, sizeof(rhs), HB_PERM_READ) == HB_OK);
    ctx->pc = 0x3e00;
    ctx->regs.x64.rdx = (uint64_t)(uintptr_t)&lhs;
    ctx->regs.x64.rcx = (uint64_t)(uintptr_t)&rhs;
    ...
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ASSERT(ctx->regs.x64.rax == lhs);
    ASSERT(ctx->pc == 0x3e06);
    bool equal = false;
    ASSERT(hb_flags_eval_cond(ctx, HB_CC_E, &equal) == HB_OK);
    ASSERT(equal);
...
TEST(jit_x64_helper_cmp_setcc_ret_block) {
    uint64_t stack[1] = {0x3f80};
    hb_ir_func_t* func = hb_ir_func_create(0x3ef0, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* block = hb_ir_block_create(0, 0x3ef0);
    ASSERT(block != NULL);
    hb_ir_cfg_add_block(func->cfg, block);
    func->cfg->entry = block;
    ...
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, block);
    hb_ir_instr_t* cmp = hb_ir_emit_cmp(b, hb_ir_reg(HB_REG_RCX, HB_SIZE_64),
                                        hb_ir_reg(HB_REG_RDX, HB_SIZE_64));
    hb_ir_instr_t* setcc = hb_ir_emit_setcc(b, HB_CC_B, hb_ir_reg(HB_REG_RAX, HB_SIZE_8));
    hb_ir_instr_t* ret = hb_ir_emit_ret(b);
    ASSERT(cmp && setcc && ret);
    cmp->guest_addr = 0x3ef0; cmp->guest_len = 3;
    setcc->guest_addr = 0x3ef3; setcc->guest_len = 3;
    ret->guest_addr = 0x3ef6; ret->guest_len = 1;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(ctx->pc == stack[0]);
    ASSERT(ctx->regs.x64.rsp == (uint64_t)(uintptr_t)&stack[1]);
    ASSERT(ctx->regs.x64.rax == 0x1122334455667701ULL);
...
TEST(jit_x64_helper_store_stride_self_loop) {
    uint32_t values[3] = {0, 0, 0};
    hb_ir_func_t* func = hb_ir_func_create(0x3f00, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* block = hb_ir_block_create(0, 0x3f00);
    ASSERT(block != NULL);
    hb_ir_cfg_add_block(func->cfg, block);
    func->cfg->entry = block;
    ...
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, block);
    hb_ir_instr_t* store = hb_ir_emit_store(b, hb_ir_mem(HB_REG_R8, HB_REG_COUNT, 1, 0, HB_SIZE_32),
                                            hb_ir_imm(-1, HB_SIZE_32));
    hb_ir_instr_t* add = hb_ir_emit_binop(b, HB_IR_ADD, hb_ir_reg(HB_REG_R8, HB_SIZE_64),
                                          hb_ir_reg(HB_REG_R8, HB_SIZE_64),
                                          hb_ir_imm(4, HB_SIZE_64));
    hb_ir_instr_t* cmp = hb_ir_emit_cmp(b, hb_ir_reg(HB_REG_R8, HB_SIZE_64),
                                        hb_ir_reg(HB_REG_RAX, HB_SIZE_64));
    hb_ir_instr_t* jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x3f00);
    ASSERT(store && add && cmp && jcc);
    store->guest_addr = 0x3f00; store->guest_len = 7;
    add->guest_addr = 0x3f07; add->guest_len = 4;
    cmp->guest_addr = 0x3f0b; cmp->guest_len = 3;
    jcc->guest_addr = 0x3f0e; jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)values, sizeof(values),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x3f00;
    ctx->regs.x64.r8 = (uint64_t)(uintptr_t)values;
    ctx->regs.x64.rax = (uint64_t)(uintptr_t)(values + 3);
    ...
    restore_env_var("MACRUNNER_HB_JIT_DIRECT_MEM", saved);
    ASSERT(out.result == HB_OK);
    ASSERT(values[0] == UINT32_MAX && values[1] == UINT32_MAX && values[2] == UINT32_MAX);
    ASSERT(ctx->regs.x64.r8 == (uint64_t)(uintptr_t)(values + 3));
    ASSERT(ctx->pc == 0x3f10);
...
TEST(jit_x64_helper_null_qword_scan_loop_promotes_cache_triplet) {
    uint64_t values[3] = {0, 0, 0};
    hb_ir_func_t* func = hb_ir_func_create(0x4100, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* load_block = hb_ir_block_create(0, 0x4100);
    hb_ir_block_t* dec_block = hb_ir_block_create(1, 0x4109);
    hb_ir_block_t* test_block = hb_ir_block_create(2, 0x4110);
    ASSERT(load_block && dec_block && test_block);
    hb_ir_cfg_add_block(func->cfg, load_block);
    hb_ir_cfg_add_block(func->cfg, dec_block);
    hb_ir_cfg_add_block(func->cfg, test_block);
    func->cfg->entry = load_block;

    hb_ir_builder_t* b = hb_ir_builder_create(func);
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, load_block);
    hb_ir_instr_t* load = hb_ir_emit_load(b, hb_ir_reg(HB_REG_RCX, HB_SIZE_64),
                                          hb_ir_mem(HB_REG_RDX, HB_REG_RAX, 8, 0, HB_SIZE_64));
    hb_ir_instr_t* test_value = hb_ir_emit_test(b, hb_ir_reg(HB_REG_RCX, HB_SIZE_64),
                                                hb_ir_reg(HB_REG_RCX, HB_SIZE_64));
    hb_ir_instr_t* value_jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x4120);
    ASSERT(load && test_value && value_jcc);
    load->guest_addr = 0x4100; load->guest_len = 4;
    test_value->guest_addr = 0x4104; test_value->guest_len = 3;
    value_jcc->guest_addr = 0x4107; value_jcc->guest_len = 2;

    hb_ir_builder_set_block(b, dec_block);
    hb_ir_instr_t* dec = hb_ir_emit_binop(b, HB_IR_SUB, hb_ir_reg(HB_REG_RAX, HB_SIZE_64),
                                          hb_ir_reg(HB_REG_RAX, HB_SIZE_64),
                                          hb_ir_imm(1, HB_SIZE_64));
    hb_ir_instr_t* jmp = hb_ir_emit_jmp(b, 0x4110);
    ASSERT(dec && jmp);
    dec->guest_addr = 0x4109; dec->guest_len = 3;
    jmp->guest_addr = 0x410c; jmp->guest_len = 2;

    hb_ir_builder_set_block(b, test_block);
    hb_ir_instr_t* test_index = hb_ir_emit_test(b, hb_ir_reg(HB_REG_RAX, HB_SIZE_64),
                                                hb_ir_reg(HB_REG_RAX, HB_SIZE_64));
    hb_ir_instr_t* back = hb_ir_emit_jcc(b, HB_CC_NS, 0x4100);
    ASSERT(test_index && back);
    test_index->guest_addr = 0x4110; test_index->guest_len = 3;
    back->guest_addr = 0x4113; back->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)values, sizeof(values),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x4100;
    ctx->regs.x64.rdx = (uint64_t)(uintptr_t)values;
    ctx->regs.x64.rax = 2;
    ...
    restore_env_var("MACRUNNER_HB_JIT_DIRECT_MEM", saved);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == 0x4115);
    ASSERT(ctx->regs.x64.rax == UINT64_MAX);
    ASSERT(ctx->regs.x64.rcx == 0);
...
TEST(jit_x64_helper_i32_less_tiebreaker_comparator_promotes_triplet) {
    int32_t values[2] = {1, 2};
    uint64_t stack[3] = {0x4300, 0x4308, 0x4310};
    hb_ir_func_t* func = hb_ir_func_create(0x4200, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* entry = hb_ir_block_create(0, 0x4200);
    hb_ir_block_t* equal = hb_ir_block_create(1, 0x4206);
    hb_ir_block_t* less = hb_ir_block_create(2, 0x4210);
    ASSERT(entry && equal && less);
    hb_ir_cfg_add_block(func->cfg, entry);
    hb_ir_cfg_add_block(func->cfg, equal);
    hb_ir_cfg_add_block(func->cfg, less);
    ...
    hb_ir_instr_t* load = hb_ir_emit_load(b, hb_ir_reg(HB_REG_RAX, HB_SIZE_32),
                                          hb_ir_mem(HB_REG_RDX, HB_REG_COUNT, 1, 0, HB_SIZE_32));
    hb_ir_instr_t* cmp_entry = hb_ir_emit_cmp(b, hb_ir_mem(HB_REG_RCX, HB_REG_COUNT, 1, 0, HB_SIZE_32),
                                              hb_ir_reg(HB_REG_RAX, HB_SIZE_32));
    hb_ir_instr_t* jne = hb_ir_emit_jcc(b, HB_CC_NE, 0x4210);
    ASSERT(load && cmp_entry && jne);
    load->guest_addr = 0x4200; load->guest_len = 2;
    cmp_entry->guest_addr = 0x4202; cmp_entry->guest_len = 2;
    jne->guest_addr = 0x4204; jne->guest_len = 2;

    hb_ir_builder_set_block(b, equal);
    hb_ir_instr_t* cmp_ptr = hb_ir_emit_cmp(b, hb_ir_reg(HB_REG_RCX, HB_SIZE_64),
                                            hb_ir_reg(HB_REG_RDX, HB_SIZE_64));
    hb_ir_instr_t* setb = hb_ir_emit_setcc(b, HB_CC_B, hb_ir_reg(HB_REG_RAX, HB_SIZE_8));
    hb_ir_instr_t* ret_equal = hb_ir_emit_ret(b);
    ASSERT(cmp_ptr && setb && ret_equal);
    cmp_ptr->guest_addr = 0x4206; cmp_ptr->guest_len = 3;
    setb->guest_addr = 0x4209; setb->guest_len = 3;
    ret_equal->guest_addr = 0x420c; ret_equal->guest_len = 1;

    hb_ir_builder_set_block(b, less);
    hb_ir_instr_t* setl = hb_ir_emit_setcc(b, HB_CC_L, hb_ir_reg(HB_REG_RAX, HB_SIZE_8));
    hb_ir_instr_t* ret_less = hb_ir_emit_ret(b);
    ASSERT(setl && ret_less);
    setl->guest_addr = 0x4210; setl->guest_len = 3;
    ret_less->guest_addr = 0x4213; ret_less->guest_len = 1;
    hb_ir_builder_destroy(b);
    ...
    hb_exec_result_t out;
    ctx->pc = 0x4200;
    ctx->regs.x64.rcx = (uint64_t)(uintptr_t)&values[0];
    ctx->regs.x64.rdx = (uint64_t)(uintptr_t)&values[1];
    ctx->regs.x64.rax = 0x1122334455667700ULL;
    ctx->regs.x64.rsp = (uint64_t)(uintptr_t)&stack[0];
    ASSERT(hb_jit_runtime_run(rt, func, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == stack[0]);
    ASSERT((ctx->regs.x64.rax & 0xff) == 1);

    values[0] = 2;
    ctx->pc = 0x4200;
    ctx->regs.x64.rax = 0x1122334455667700ULL;
    ctx->regs.x64.rsp = (uint64_t)(uintptr_t)&stack[1];
    ASSERT(hb_jit_runtime_run(rt, func, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->pc == stack[1]);
    ASSERT((ctx->regs.x64.rax & 0xff) == 1);
...
TEST(jit_x64_native_arith_rmw_memory_family) {
    char* saved = save_env_var("MACRUNNER_HB_JIT_DIRECT_MEM");
    setenv("MACRUNNER_HB_JIT_DIRECT_MEM", "1", 1);

    uint8_t byte = 0xff;
    hb_ir_func_t* func = hb_ir_func_create(0x4700, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* blk = hb_ir_block_create(0, 0x4700);
    ASSERT(blk != NULL);
    hb_ir_cfg_add_block(func->cfg, blk);
    func->cfg->entry = blk;
    hb_ir_builder_t* b = hb_ir_builder_create(func);
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, blk);
    hb_ir_operand_t byte_mem = hb_ir_mem(HB_REG_RBX, HB_REG_COUNT, 1, 0, HB_SIZE_8);
    hb_ir_instr_t* add_byte = hb_ir_emit_binop(b, HB_IR_ADD, byte_mem, byte_mem,
                                               hb_ir_imm(1, HB_SIZE_8));
    ASSERT(add_byte != NULL);
    add_byte->guest_addr = 0x4700; add_byte->guest_len = 3;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&byte, sizeof(byte),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x4700;
    ctx->regs.x64.rbx = (uint64_t)(uintptr_t)&byte;
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ...
    uint64_t qword = 41;
    uint16_t word = 3;
    func = hb_ir_func_create(0x4800, 0);
    ASSERT(func != NULL);
    blk = hb_ir_block_create(0, 0x4800);
    ASSERT(blk != NULL);
    hb_ir_cfg_add_block(func->cfg, blk);
    func->cfg->entry = blk;
    ...
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, blk);
    hb_ir_operand_t qword_mem = hb_ir_mem(HB_REG_R15, HB_REG_COUNT, 1, 0, HB_SIZE_64);
    hb_ir_operand_t word_mem = hb_ir_mem(HB_REG_RSI, HB_REG_COUNT, 1, 0, HB_SIZE_16);
    hb_ir_instr_t* add_qword = hb_ir_emit_binop(b, HB_IR_ADD, qword_mem, qword_mem,
                                                hb_ir_imm(1, HB_SIZE_64));
    hb_ir_instr_t* sub_word = hb_ir_emit_binop(b, HB_IR_SUB, word_mem, word_mem,
                                               hb_ir_reg(HB_REG_RCX, HB_SIZE_16));
    ASSERT(add_qword && sub_word);
    add_qword->guest_addr = 0x4800; add_qword->guest_len = 3;
    sub_word->guest_addr = 0x4803; sub_word->guest_len = 3;
    hb_ir_builder_destroy(b);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&qword, sizeof(qword),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&word, sizeof(word),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x4800;
    ctx->regs.x64.r15 = (uint64_t)(uintptr_t)&qword;
    ctx->regs.x64.rsi = (uint64_t)(uintptr_t)&word;
    ctx->regs.x64.rcx = 5;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
...
TEST(jit_x64_native_arith_rmw_dead_flags_test_jcc) {
    uint64_t counter = 41;
    hb_ir_func_t* func = hb_ir_func_create(0x4900, 0);
    ASSERT(func != NULL);
    hb_ir_block_t* blk = hb_ir_block_create(0, 0x4900);
    ASSERT(blk != NULL);
    hb_ir_cfg_add_block(func->cfg, blk);
    func->cfg->entry = blk;
    hb_ir_builder_t* b = hb_ir_builder_create(func);
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, blk);
    hb_ir_operand_t mem = hb_ir_mem(HB_REG_R15, HB_REG_COUNT, 1, 0, HB_SIZE_64);
    hb_ir_instr_t* add = hb_ir_emit_binop(b, HB_IR_ADD, mem, mem, hb_ir_imm(1, HB_SIZE_64));
    hb_ir_instr_t* test = hb_ir_emit_test(b, hb_ir_reg(HB_REG_RBX, HB_SIZE_64),
                                          hb_ir_reg(HB_REG_RBX, HB_SIZE_64));
    hb_ir_instr_t* jcc = hb_ir_emit_jcc(b, HB_CC_E, 0x4910);
    ASSERT(add && test && jcc);
    add->guest_addr = 0x4900; add->guest_len = 3;
    test->guest_addr = 0x4903; test->guest_len = 3;
    jcc->guest_addr = 0x4906; jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&counter, sizeof(counter),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x4900;
    ctx->regs.x64.r15 = (uint64_t)(uintptr_t)&counter;
    ctx->regs.x64.rbx = 0;
...
TEST(jit_x64_native_mem_reg_flags_jcc_pair) {
    uint8_t byte = 0x10;
    uint16_t word = 0x00f0;
    char* saved = save_env_var("MACRUNNER_HB_JIT_DIRECT_MEM");
    setenv("MACRUNNER_HB_JIT_DIRECT_MEM", "1", 1);
    ...
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, blk);
    hb_ir_instr_t* test = hb_ir_emit_test(b, hb_ir_mem(HB_REG_RAX, HB_REG_RDX, 1, 0x19, HB_SIZE_8),
                                          hb_ir_reg(HB_REG_RDI, HB_SIZE_8));
    hb_ir_instr_t* jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x4310);
    ASSERT(test && jcc);
    test->guest_addr = 0x4300; test->guest_len = 5;
    jcc->guest_addr = 0x4305; jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&byte, sizeof(byte),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x4300;
    ctx->regs.x64.rax = (uint64_t)(uintptr_t)&byte - 0x19;
    ctx->regs.x64.rdx = 0;
    ctx->regs.x64.rdi = 0x10;
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ASSERT(ctx->pc == 0x4310);
    bool equal = true;
    ASSERT(hb_flags_eval_cond(ctx, HB_CC_E, &equal) == HB_OK);
    ASSERT(!equal);
    ...
    ASSERT(code_buf != NULL && cg != NULL);
    ASSERT(hb_arm64_codegen_block(cg, blk, code_buf) == HB_OK);
    ASSERT(code_buf->size <= 196);
    hb_arm64_codegen_destroy(cg);
    hb_codegen_buffer_destroy(code_buf);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    func = hb_ir_func_create(0x4400, 0);
    ASSERT(func != NULL);
    blk = hb_ir_block_create(0, 0x4400);
    ASSERT(blk != NULL);
    hb_ir_cfg_add_block(func->cfg, blk);
    func->cfg->entry = blk;
    b = hb_ir_builder_create(func);
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, blk);
    hb_ir_instr_t* cmp = hb_ir_emit_cmp(b, hb_ir_mem(HB_REG_RBX, HB_REG_COUNT, 1, 0, HB_SIZE_16),
                                        hb_ir_reg(HB_REG_RCX, HB_SIZE_16));
    jcc = hb_ir_emit_jcc(b, HB_CC_NE, 0x4410);
    ASSERT(cmp && jcc);
    cmp->guest_addr = 0x4400; cmp->guest_len = 4;
    jcc->guest_addr = 0x4404; jcc->guest_len = 2;
    hb_ir_builder_destroy(b);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&word, sizeof(word),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x4400;
    ctx->regs.x64.rbx = (uint64_t)(uintptr_t)&word;
    ctx->regs.x64.rcx = 0x00f0;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ASSERT(ctx->pc == 0x4406);
    equal = false;
    ASSERT(hb_flags_eval_cond(ctx, HB_CC_E, &equal) == HB_OK);
    ASSERT(equal);
...
TEST(jit_x64_native_flags_setcc_sequence_family) {
    char* saved = save_env_var("MACRUNNER_HB_JIT_DIRECT_MEM");
    setenv("MACRUNNER_HB_JIT_DIRECT_MEM", "1", 1);
    ...
    ASSERT(b != NULL);
    hb_ir_builder_set_block(b, blk);
    hb_ir_instr_t* test = hb_ir_emit_test(b, hb_ir_reg(HB_REG_RBP, HB_SIZE_8),
                                          hb_ir_reg(HB_REG_RBP, HB_SIZE_8));
    hb_ir_instr_t* mov = hb_ir_emit_mov(b, hb_ir_reg(HB_REG_RSI, HB_SIZE_8),
                                        hb_ir_imm(0x22, HB_SIZE_8));
    hb_ir_instr_t* setcc = hb_ir_emit_setcc(b, HB_CC_E, hb_ir_reg(HB_REG_RBP, HB_SIZE_8));
    ASSERT(test && mov && setcc);
    test->guest_addr = 0x4500; test->guest_len = 3;
    mov->guest_addr = 0x4503; mov->guest_len = 3;
    setcc->guest_addr = 0x4506; setcc->guest_len = 4;
    hb_ir_builder_destroy(b);

    hb_context_t* ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_JIT);
    ASSERT(ctx != NULL);
    ctx->pc = 0x4500;
    ctx->regs.x64.rbp = 0x1122334455667700ULL;
    ctx->regs.x64.rsi = 0xaabbccddeeff0000ULL;
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ASSERT(ctx->regs.x64.rbp == 0x1122334455667701ULL);
    ASSERT(ctx->regs.x64.rsi == 0xaabbccddeeff0022ULL);
    bool equal = false;
    ASSERT(hb_flags_eval_cond(ctx, HB_CC_E, &equal) == HB_OK);
    ASSERT(equal);
    ...
    hb_ir_instr_t* cmp = hb_ir_emit_cmp(b, hb_ir_reg(HB_REG_RAX, HB_SIZE_32),
                                        hb_ir_imm(1, HB_SIZE_32));
    setcc = hb_ir_emit_setcc(b, HB_CC_NE,
                             hb_ir_mem(HB_REG_RBX, HB_REG_COUNT, 1, 0, HB_SIZE_8));
    ASSERT(cmp && setcc);
    cmp->guest_addr = 0x4600; cmp->guest_len = 3;
    setcc->guest_addr = 0x4603; setcc->guest_len = 3;
    hb_ir_builder_destroy(b);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&flag, sizeof(flag),
                         HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = 0x4600;
    ctx->regs.x64.rax = 2;
    ctx->regs.x64.rbx = (uint64_t)(uintptr_t)&flag;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_JIT, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT_EQ(out.blocks_executed, 1);
    ASSERT(flag == 1);
...
static hb_result_t make_unicorn_diff_regression_ctx(const uint8_t* code, size_t len,
                                                    hb_ir_func_t** out_func,
                                                    hb_context_t** out_ctx) {
    uint64_t base = (uint64_t)(uintptr_t)code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, code, len, base);
    ...
    hb_decoder_destroy(dec);
    if (r != HB_OK) return r;
    if (!*out_func) return HB_ERR_INTERNAL;
    hb_context_t* ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    ...
    r = hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code, len,
...
TEST(interp_x64_unicorn_diff_regressions_scalar_flags) {
    hb_exec_result_t out;
    hb_ir_func_t* func = NULL;
    hb_context_t* ctx = NULL;

    uint8_t imul_r8_r9[] = {0x4d, 0x0f, 0xaf, 0xc1}; /* imul %r9,%r8 */
    hb_decoded_t d;
    ASSERT(hb_decode_x64(imul_r8_r9, sizeof(imul_r8_r9),
                         (uint64_t)(uintptr_t)imul_r8_r9, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_IMUL);
    ASSERT(make_unicorn_diff_regression_ctx(imul_r8_r9, sizeof(imul_r8_r9), &func, &ctx) == HB_OK);
    ctx->regs.x64.r8 = 0x4000000000000000ULL;
    ctx->regs.x64.r9 = 4;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.r8 == 0);
    ASSERT(ctx->flags.cf == true && ctx->flags.of == true);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t popcnt_r8_r9[] = {0xf3, 0x4d, 0x0f, 0xb8, 0xc1}; /* popcnt %r9,%r8 */
    ASSERT(hb_decode_x64(popcnt_r8_r9, sizeof(popcnt_r8_r9),
                         (uint64_t)(uintptr_t)popcnt_r8_r9, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_POPCNT);
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(popcnt_r8_r9, sizeof(popcnt_r8_r9), &func, &ctx) == HB_OK);
    ctx->regs.x64.r9 = 0xf0f0000000000001ULL;
    ctx->flags.cf = ctx->flags.pf = ctx->flags.af = true;
    ctx->flags.sf = ctx->flags.of = true;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.r8 == 9);
    ASSERT(!ctx->flags.cf && !ctx->flags.pf && !ctx->flags.af);
    ASSERT(!ctx->flags.zf && !ctx->flags.sf && !ctx->flags.of);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t rcl_r8_1[] = {0x49, 0xd1, 0xd0}; /* rcl %r8,1 */
    ASSERT(hb_decode_x64(rcl_r8_1, sizeof(rcl_r8_1),
                         (uint64_t)(uintptr_t)rcl_r8_1, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_RCL);
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(rcl_r8_1, sizeof(rcl_r8_1), &func, &ctx) == HB_OK);
    ctx->regs.x64.r8 = 0x8000000000000000ULL;
    ctx->flags.cf = true;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.r8 == 1);
    ASSERT(ctx->flags.cf == true && ctx->flags.of == true);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t rcr_r8_1[] = {0x49, 0xd1, 0xd8}; /* rcr %r8,1 */
    ASSERT(hb_decode_x64(rcr_r8_1, sizeof(rcr_r8_1),
                         (uint64_t)(uintptr_t)rcr_r8_1, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_RCR);
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(rcr_r8_1, sizeof(rcr_r8_1), &func, &ctx) == HB_OK);
    ctx->regs.x64.r8 = 1;
    ctx->flags.cf = true;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.r8 == 0x8000000000000000ULL);
    ASSERT(ctx->flags.cf == true && ctx->flags.of == true);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_fma3_f16c_semantics) {
    typedef struct { uint8_t code[5]; int opcode; uint8_t bytes; } fma_decode_case_t;
    static const fma_decode_case_t fma_cases[] = {
        {{0xc4,0xe2,0x75,0x98,0xc2}, HB_INS_VFMADD132PS, 32},
        {{0xc4,0xe2,0xf5,0x98,0xc2}, HB_INS_VFMADD132PD, 32},
        {{0xc4,0xe2,0x71,0x99,0xc2}, HB_INS_VFMADD132SS, 16},
        {{0xc4,0xe2,0xf1,0x99,0xc2}, HB_INS_VFMADD132SD, 16},
        {{0xc4,0xe2,0x75,0xa8,0xc2}, HB_INS_VFMADD213PS, 32},
        {{0xc4,0xe2,0xf5,0xa8,0xc2}, HB_INS_VFMADD213PD, 32},
        {{0xc4,0xe2,0x71,0xa9,0xc2}, HB_INS_VFMADD213SS, 16},
        {{0xc4,0xe2,0xf1,0xa9,0xc2}, HB_INS_VFMADD213SD, 16},
        {{0xc4,0xe2,0x75,0xb8,0xc2}, HB_INS_VFMADD231PS, 32},
        {{0xc4,0xe2,0xf5,0xb8,0xc2}, HB_INS_VFMADD231PD, 32},
        {{0xc4,0xe2,0x71,0xb9,0xc2}, HB_INS_VFMADD231SS, 16},
        {{0xc4,0xe2,0xf1,0xb9,0xc2}, HB_INS_VFMADD231SD, 16},
        {{0xc4,0xe2,0x75,0x9a,0xc2}, HB_INS_VFMSUB132PS, 32},
        {{0xc4,0xe2,0xf5,0x9a,0xc2}, HB_INS_VFMSUB132PD, 32},
        {{0xc4,0xe2,0x71,0x9b,0xc2}, HB_INS_VFMSUB132SS, 16},
        {{0xc4,0xe2,0xf1,0x9b,0xc2}, HB_INS_VFMSUB132SD, 16},
        {{0xc4,0xe2,0x75,0xaa,0xc2}, HB_INS_VFMSUB213PS, 32},
        {{0xc4,0xe2,0xf5,0xaa,0xc2}, HB_INS_VFMSUB213PD, 32},
        {{0xc4,0xe2,0x71,0xab,0xc2}, HB_INS_VFMSUB213SS, 16},
        {{0xc4,0xe2,0xf1,0xab,0xc2}, HB_INS_VFMSUB213SD, 16},
        {{0xc4,0xe2,0x75,0xba,0xc2}, HB_INS_VFMSUB231PS, 32},
        {{0xc4,0xe2,0xf5,0xba,0xc2}, HB_INS_VFMSUB231PD, 32},
        {{0xc4,0xe2,0x71,0xbb,0xc2}, HB_INS_VFMSUB231SS, 16},
        {{0xc4,0xe2,0xf1,0xbb,0xc2}, HB_INS_VFMSUB231SD, 16},
    };
    hb_decoded_t d;
    for (size_t i = 0; i < sizeof(fma_cases) / sizeof(fma_cases[0]); i++) {
        ASSERT(hb_decode_x64(fma_cases[i].code, sizeof(fma_cases[i].code),
                             (uint64_t)(uintptr_t)fma_cases[i].code, &d) == HB_OK);
        ASSERT((int)d.opcode == fma_cases[i].opcode);
        ASSERT(d.op1.reg == HB_REG_XMM0 && d.op2.reg == HB_REG_XMM1 && d.op3.reg == HB_REG_XMM2);
        ASSERT(d.op1.size == fma_cases[i].bytes);
    }

    hb_exec_result_t out;
    hb_ir_func_t* func = NULL;
    hb_context_t* ctx = NULL;
    uint64_t data_addr = 0x70000000ULL;

    uint8_t fma132ps_mem[] = {0xc4,0xe2,0x75,0x98,0x00}; /* vfmadd132ps ymm0,ymm1,[rax] */
    ASSERT(make_unicorn_diff_regression_ctx(fma132ps_mem, sizeof(fma132ps_mem), &func, &ctx) == HB_OK);
    ASSERT(hb_memory_map_private(ctx->memory, data_addr, 0x1000, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->regs.x64.rax = data_addr;
    uint32_t fma_dst_lo[4] = {test_float_bits(1.0f), test_float_bits(2.0f), test_float_bits(3.0f), test_float_bits(4.0f)};
    uint32_t fma_dst_hi[4] = {test_float_bits(5.0f), test_float_bits(6.0f), test_float_bits(7.0f), test_float_bits(8.0f)};
    uint32_t fma_src1_lo[4] = {test_float_bits(100.0f), test_float_bits(200.0f), test_float_bits(300.0f), test_float_bits(400.0f)};
    uint32_t fma_src1_hi[4] = {test_float_bits(500.0f), test_float_bits(600.0f), test_float_bits(700.0f), test_float_bits(800.0f)};
    uint32_t fma_mem[8] = {test_float_bits(10.0f), test_float_bits(20.0f), test_float_bits(30.0f), test_float_bits(40.0f),
                           test_float_bits(50.0f), test_float_bits(60.0f), test_float_bits(70.0f), test_float_bits(80.0f)};
    memcpy(ctx->regs.x64.xmm[0], fma_dst_lo, 16);
    memcpy(ctx->ymm_hi[0], fma_dst_hi, 16);
    memcpy(ctx->regs.x64.xmm[1], fma_src1_lo, 16);
    memcpy(ctx->ymm_hi[1], fma_src1_hi, 16);
    ASSERT(hb_memory_write(ctx->memory, data_addr, fma_mem, sizeof(fma_mem)) == HB_OK);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint32_t expected_fma_ps_lo[4] = {test_float_bits(110.0f), test_float_bits(240.0f), test_float_bits(390.0f), test_float_bits(560.0f)};
    uint32_t expected_fma_ps_hi[4] = {test_float_bits(750.0f), test_float_bits(960.0f), test_float_bits(1190.0f), test_float_bits(1440.0f)};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_fma_ps_lo, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[0], expected_fma_ps_hi, 16) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vfmsub231pd[] = {0xc4,0xe2,0xf5,0xba,0xc2}; /* vfmsub231pd ymm0,ymm1,ymm2 */
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(vfmsub231pd, sizeof(vfmsub231pd), &func, &ctx) == HB_OK);
    uint64_t pd_dst_lo[2] = {test_double_bits(1.0), test_double_bits(2.0)};
    uint64_t pd_dst_hi[2] = {test_double_bits(3.0), test_double_bits(4.0)};
    uint64_t pd_s1_lo[2] = {test_double_bits(10.0), test_double_bits(20.0)};
    uint64_t pd_s1_hi[2] = {test_double_bits(30.0), test_double_bits(40.0)};
    uint64_t pd_s2_lo[2] = {test_double_bits(2.0), test_double_bits(3.0)};
    uint64_t pd_s2_hi[2] = {test_double_bits(4.0), test_double_bits(5.0)};
    memcpy(ctx->regs.x64.xmm[0], pd_dst_lo, 16);
    memcpy(ctx->ymm_hi[0], pd_dst_hi, 16);
    memcpy(ctx->regs.x64.xmm[1], pd_s1_lo, 16);
    memcpy(ctx->ymm_hi[1], pd_s1_hi, 16);
    memcpy(ctx->regs.x64.xmm[2], pd_s2_lo, 16);
    memcpy(ctx->ymm_hi[2], pd_s2_hi, 16);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint64_t expected_pd_lo[2] = {test_double_bits(19.0), test_double_bits(58.0)};
    uint64_t expected_pd_hi[2] = {test_double_bits(117.0), test_double_bits(196.0)};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_pd_lo, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[0], expected_pd_hi, 16) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vfmadd213ss[] = {0xc4,0xe2,0x71,0xa9,0xc2}; /* vfmadd213ss xmm0,xmm1,xmm2 */
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(vfmadd213ss, sizeof(vfmadd213ss), &func, &ctx) == HB_OK);
    uint32_t ss_dst[4] = {test_float_bits(3.0f), test_float_bits(30.0f), test_float_bits(31.0f), test_float_bits(32.0f)};
    uint32_t ss_s1[4] = {test_float_bits(4.0f), test_float_bits(40.0f), test_float_bits(41.0f), test_float_bits(42.0f)};
    uint32_t ss_s2[4] = {test_float_bits(5.0f), test_float_bits(50.0f), test_float_bits(51.0f), test_float_bits(52.0f)};
    memset(ctx->ymm_hi[0], 0xa5, 16);
    memcpy(ctx->regs.x64.xmm[0], ss_dst, 16);
    memcpy(ctx->regs.x64.xmm[1], ss_s1, 16);
    memcpy(ctx->regs.x64.xmm[2], ss_s2, 16);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint32_t expected_ss[4] = {test_float_bits(17.0f), test_float_bits(40.0f), test_float_bits(41.0f), test_float_bits(42.0f)};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_ss, 16) == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vfmsub132sd[] = {0xc4,0xe2,0xf1,0x9b,0xc2}; /* vfmsub132sd xmm0,xmm1,xmm2 */
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(vfmsub132sd, sizeof(vfmsub132sd), &func, &ctx) == HB_OK);
    uint64_t sd_dst[2] = {test_double_bits(10.0), test_double_bits(30.0)};
    uint64_t sd_s1[2] = {test_double_bits(4.0), test_double_bits(40.0)};
    uint64_t sd_s2[2] = {test_double_bits(3.0), test_double_bits(50.0)};
    memcpy(ctx->regs.x64.xmm[0], sd_dst, 16);
    memcpy(ctx->regs.x64.xmm[1], sd_s1, 16);
    memcpy(ctx->regs.x64.xmm[2], sd_s2, 16);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint64_t expected_sd[2] = {test_double_bits(26.0), test_double_bits(40.0)};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_sd, 16) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vcvtph2ps_reg[] = {0xc4,0xe2,0x79,0x13,0xc1};
    ASSERT(hb_decode_x64(vcvtph2ps_reg, sizeof(vcvtph2ps_reg), (uint64_t)(uintptr_t)vcvtph2ps_reg, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VCVTPH2PS && d.op1.size == 16 && d.op2.size == 8);
    uint8_t vcvtps2ph_reg[] = {0xc4,0xe3,0x7d,0x1d,0xc1,0x00};
    ASSERT(hb_decode_x64(vcvtps2ph_reg, sizeof(vcvtps2ph_reg), (uint64_t)(uintptr_t)vcvtps2ph_reg, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VCVTPS2PH && d.op1.size == 16 && d.op2.size == 32);

    uint8_t vcvtph2ps_mem[] = {0xc4,0xe2,0x7d,0x13,0x00}; /* vcvtph2ps ymm0,[rax] */
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(vcvtph2ps_mem, sizeof(vcvtph2ps_mem), &func, &ctx) == HB_OK);
    ASSERT(hb_memory_map_private(ctx->memory, data_addr, 0x1000, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->regs.x64.rax = data_addr;
    uint16_t halves[8] = {0x3c00, 0xc000, 0x3800, 0x7c00, 0x0000, 0x8000, 0x7e00, 0x3555};
    ASSERT(hb_memory_write(ctx->memory, data_addr, halves, sizeof(halves)) == HB_OK);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint32_t expected_ph2ps_lo[4] = {test_float_bits(1.0f), test_float_bits(-2.0f), test_float_bits(0.5f), 0x7f800000u};
    uint32_t expected_ph2ps_hi[4] = {test_float_bits(0.0f), 0x80000000u, 0x7fc00000u, 0x3eaaa000u};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_ph2ps_lo, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[0], expected_ph2ps_hi, 16) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vcvtps2ph_mem[] = {0xc4,0xe3,0x7d,0x1d,0x00,0x00}; /* vcvtps2ph [rax],ymm0,0 */
    func = NULL;
    ASSERT(make_unicorn_diff_regression_ctx(vcvtps2ph_mem, sizeof(vcvtps2ph_mem), &func, &ctx) == HB_OK);
    ASSERT(hb_memory_map_private(ctx->memory, data_addr, 0x1000, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->regs.x64.rax = data_addr;
    uint32_t floats_lo[4] = {test_float_bits(1.0f), test_float_bits(-2.0f), test_float_bits(0.5f), 0x7f800000u};
    uint32_t floats_hi[4] = {test_float_bits(0.0f), 0x80000000u, 0x7fc00000u, 0x3eaaa000u};
    memcpy(ctx->regs.x64.xmm[0], floats_lo, 16);
    memcpy(ctx->ymm_hi[0], floats_hi, 16);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint16_t got_halves[8] = {0};
    uint16_t expected_halves[8] = {0x3c00, 0xc000, 0x3800, 0x7c00, 0x0000, 0x8000, 0x7e00, 0x3555};
    ASSERT(hb_memory_read(ctx->memory, data_addr, got_halves, sizeof(got_halves)) == HB_OK);
    ASSERT(memcmp(got_halves, expected_halves, sizeof(expected_halves)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_vex_avx_and_sse41_packed_vector_semantics) {
    uint8_t vadd_vpxor[] = {
        0xc5, 0xf4, 0x58, 0xc2, /* vaddps ymm0, ymm1, ymm2 */
        0xc5, 0xf1, 0xef, 0xda  /* vpxor  xmm3, xmm1, xmm2 */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(vadd_vpxor, 4, (uint64_t)(uintptr_t)vadd_vpxor, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_ADDPS && d.op1.reg == HB_REG_XMM0);
    ASSERT(d.op2.reg == HB_REG_XMM1 && d.op3.reg == HB_REG_XMM2);
    ASSERT(d.op1.size == 32 && d.op2.size == 32 && d.op3.size == 32);
    ASSERT(hb_decode_x64(vadd_vpxor + 4, 4, (uint64_t)(uintptr_t)(vadd_vpxor + 4), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PXOR && d.op1.reg == HB_REG_XMM3);
    ASSERT(d.op2.reg == HB_REG_XMM1 && d.op3.reg == HB_REG_XMM2);

    uint64_t base = (uint64_t)(uintptr_t)vadd_vpxor;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, vadd_vpxor, sizeof(vadd_vpxor), base);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)vadd_vpxor, sizeof(vadd_vpxor),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;

    uint32_t ymm1_lo[4] = {test_float_bits(1.0f), test_float_bits(2.0f), test_float_bits(3.0f), test_float_bits(4.0f)};
    uint32_t ymm1_hi[4] = {test_float_bits(5.0f), test_float_bits(6.0f), test_float_bits(7.0f), test_float_bits(8.0f)};
    uint32_t ymm2_lo[4] = {test_float_bits(10.0f), test_float_bits(20.0f), test_float_bits(30.0f), test_float_bits(40.0f)};
    uint32_t ymm2_hi[4] = {test_float_bits(50.0f), test_float_bits(60.0f), test_float_bits(70.0f), test_float_bits(80.0f)};
    memcpy(ctx->regs.x64.xmm[1], ymm1_lo, sizeof(ymm1_lo));
    memcpy(ctx->ymm_hi[1], ymm1_hi, sizeof(ymm1_hi));
    memcpy(ctx->regs.x64.xmm[2], ymm2_lo, sizeof(ymm2_lo));
    memcpy(ctx->ymm_hi[2], ymm2_hi, sizeof(ymm2_hi));

    hb_exec_result_t run_out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint32_t expected_vadd_lo[4] = {test_float_bits(11.0f), test_float_bits(22.0f), test_float_bits(33.0f), test_float_bits(44.0f)};
    uint32_t expected_vadd_hi[4] = {test_float_bits(55.0f), test_float_bits(66.0f), test_float_bits(77.0f), test_float_bits(88.0f)};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_vadd_lo, sizeof(expected_vadd_lo)) == 0);
    ASSERT(memcmp(ctx->ymm_hi[0], expected_vadd_hi, sizeof(expected_vadd_hi)) == 0);
    ASSERT(ctx->regs.x64.xmm[3][0] == (ctx->regs.x64.xmm[1][0] ^ ctx->regs.x64.xmm[2][0]));
    ASSERT(ctx->regs.x64.xmm[3][1] == (ctx->regs.x64.xmm[1][1] ^ ctx->regs.x64.xmm[2][1]));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t avx2_packed[] = {
        0xc5, 0xf5, 0xd8, 0xc2, /* vpsubusb ymm0, ymm1, ymm2 */
        0xc5, 0xf5, 0xf6, 0xda, /* vpsadbw  ymm3, ymm1, ymm2 */
        0xc5, 0xd5, 0xf4, 0xe6  /* vpmuludq ymm4, ymm5, ymm6 */
    };
    ASSERT(hb_decode_x64(avx2_packed, 4, (uint64_t)(uintptr_t)avx2_packed, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PSUBUSB && d.op1.size == 32 && d.op2.reg == HB_REG_XMM1);
    ASSERT(hb_decode_x64(avx2_packed + 8, 4, (uint64_t)(uintptr_t)(avx2_packed + 8), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PMULUDQ && d.op1.reg == HB_REG_XMM4 && d.op2.reg == HB_REG_XMM5);
    base = (uint64_t)(uintptr_t)avx2_packed;
    dec = hb_decoder_create(HB_ARCH_X64, avx2_packed, sizeof(avx2_packed), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)avx2_packed, sizeof(avx2_packed),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint8_t avx2_lhs[32], avx2_rhs[32];
    uint32_t mul_lhs[8] = {2, 0x11111111u, 0xffffffffu, 0x22222222u, 7, 8, 9, 10};
    uint32_t mul_rhs[8] = {3, 0x33333333u, 2, 0x44444444u, 11, 12, 13, 14};
    for (int i = 0; i < 32; i++) {
        avx2_lhs[i] = (uint8_t)(i * 7 + 3);
        avx2_rhs[i] = (uint8_t)(100 - i * 2);
    }
    memcpy(ctx->regs.x64.xmm[1], avx2_lhs, 16);
    memcpy(ctx->ymm_hi[1], avx2_lhs + 16, 16);
    memcpy(ctx->regs.x64.xmm[2], avx2_rhs, 16);
    memcpy(ctx->ymm_hi[2], avx2_rhs + 16, 16);
    memcpy(ctx->regs.x64.xmm[5], mul_lhs, 16);
    memcpy(ctx->ymm_hi[5], mul_lhs + 4, 16);
    memcpy(ctx->regs.x64.xmm[6], mul_rhs, 16);
    memcpy(ctx->ymm_hi[6], mul_rhs + 4, 16);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint8_t expected_vpsubusb[32];
    uint64_t expected_vpsadbw[4] = {0, 0, 0, 0};
    uint64_t expected_vpmuludq[4] = {
        6, 0x00000001fffffffeULL, 77, 117
    };
    for (int i = 0; i < 32; i++) {
        expected_vpsubusb[i] = avx2_lhs[i] > avx2_rhs[i] ? (uint8_t)(avx2_lhs[i] - avx2_rhs[i]) : 0;
        int diff = (int)avx2_lhs[i] - (int)avx2_rhs[i];
        expected_vpsadbw[i / 8] += (uint64_t)(diff < 0 ? -diff : diff);
    }
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_vpsubusb, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[0], expected_vpsubusb + 16, 16) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[3], expected_vpsadbw, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[3], expected_vpsadbw + 2, 16) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[4], expected_vpmuludq, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[4], expected_vpmuludq + 2, 16) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vzero[] = {0xc5, 0xf8, 0x77};
    ASSERT(hb_decode_x64(vzero, sizeof(vzero), (uint64_t)(uintptr_t)vzero, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VZEROUPPER);
    base = (uint64_t)(uintptr_t)vzero;
    dec = hb_decoder_create(HB_ARCH_X64, vzero, sizeof(vzero), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)vzero, sizeof(vzero),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    memset(ctx->ymm_hi, 0xa5, sizeof(ctx->ymm_hi));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    for (size_t i = 0; i < 16; i++) {
        ASSERT(ctx->ymm_hi[i][0] == 0);
        ASSERT(ctx->ymm_hi[i][1] == 0);
    }
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t sse41[] = {
        0x66, 0x0f, 0x3a, 0x0e, 0xc1, 0xaa, /* pblendw xmm0, xmm1, 0xaa */
        0x66, 0x0f, 0x3a, 0x0f, 0xd3, 0x04, /* palignr xmm2, xmm3, 4 */
        0x66, 0x0f, 0x38, 0x1c, 0xe5,       /* pabsb   xmm4, xmm5 */
        0x66, 0x0f, 0x38, 0x39, 0xf7        /* pminsd  xmm6, xmm7 */
    };
    base = (uint64_t)(uintptr_t)sse41;
    dec = hb_decoder_create(HB_ARCH_X64, sse41, sizeof(sse41), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)sse41, sizeof(sse41),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint16_t blend_dst[8] = {100, 101, 102, 103, 104, 105, 106, 107};
    uint16_t blend_src[8] = {200, 201, 202, 203, 204, 205, 206, 207};
    uint8_t align_dst[16], align_src[16];
    int8_t pabs_src[16] = {-1, -2, -128, 0, 1, 2, 127, -127, 5, -5, 10, -10, 64, -64, 30, -30};
    int32_t min_dst[4] = {5, -10, 100, -100};
    int32_t min_src[4] = {4, -20, 200, -50};
    for (int i = 0; i < 16; i++) {
        align_dst[i] = (uint8_t)(0xd0 + i);
        align_src[i] = (uint8_t)(0x30 + i);
    }
    memcpy(ctx->regs.x64.xmm[0], blend_dst, sizeof(blend_dst));
    memcpy(ctx->regs.x64.xmm[1], blend_src, sizeof(blend_src));
    memcpy(ctx->regs.x64.xmm[2], align_dst, sizeof(align_dst));
    memcpy(ctx->regs.x64.xmm[3], align_src, sizeof(align_src));
    memcpy(ctx->regs.x64.xmm[5], pabs_src, sizeof(pabs_src));
    memcpy(ctx->regs.x64.xmm[6], min_dst, sizeof(min_dst));
    memcpy(ctx->regs.x64.xmm[7], min_src, sizeof(min_src));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint16_t expected_blend[8] = {100, 201, 102, 203, 104, 205, 106, 207};
    uint8_t expected_align[16] = {0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3a, 0x3b,
                                  0x3c, 0x3d, 0x3e, 0x3f, 0xd0, 0xd1, 0xd2, 0xd3};
    uint8_t expected_abs[16] = {1, 2, 0x80, 0, 1, 2, 127, 127, 5, 5, 10, 10, 64, 64, 30, 30};
    int32_t expected_min[4] = {4, -20, 100, -100};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_blend, sizeof(expected_blend)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[2], expected_align, sizeof(expected_align)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[4], expected_abs, sizeof(expected_abs)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[6], expected_min, sizeof(expected_min)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
static void test_set_ymm_bytes(hb_context_t* ctx, int reg, const void* bytes32) {
    const uint8_t* bytes = (const uint8_t*)bytes32;
    memcpy(ctx->regs.x64.xmm[reg], bytes, 16);
    memcpy(ctx->ymm_hi[reg], bytes + 16, 16);
}

static bool test_ymm_bytes_equal(hb_context_t* ctx, int reg, const void* bytes32) {
    const uint8_t* bytes = (const uint8_t*)bytes32;
    return memcmp(ctx->regs.x64.xmm[reg], bytes, 16) == 0 &&
           memcmp(ctx->ymm_hi[reg], bytes + 16, 16) == 0;
}

static void test_set_zmm_bytes(hb_context_t* ctx, int reg, const void* bytes64) {
    const uint8_t* bytes = (const uint8_t*)bytes64;
    memcpy(ctx->regs.x64.xmm[reg], bytes, 16);
    memcpy(ctx->ymm_hi[reg], bytes + 16, 16);
    memcpy(ctx->zmm_hi[reg], bytes + 32, 32);
}

static bool test_zmm_bytes_equal(hb_context_t* ctx, int reg, const void* bytes64) {
    const uint8_t* bytes = (const uint8_t*)bytes64;
    return memcmp(ctx->regs.x64.xmm[reg], bytes, 16) == 0 &&
           memcmp(ctx->ymm_hi[reg], bytes + 16, 16) == 0 &&
           memcmp(ctx->zmm_hi[reg], bytes + 32, 32) == 0;
}

static void test_expect_punpck(uint8_t* out, const uint8_t* lhs, const uint8_t* rhs,
                               size_t bytes, unsigned lane, bool high) {
    memset(out, 0, bytes);
    for (size_t base = 0; base < bytes; base += 16) {
        ...
        for (unsigned i = 0; i < lanes; i++) {
            ...
            memcpy(out + pos, lhs + off, lane);
            pos += lane;
            memcpy(out + pos, rhs + off, lane);
            pos += lane;
        }
    }
}

static void test_expect_pack_bytes(uint8_t* out, const int16_t* lhs, const int16_t* rhs,
                                   size_t bytes, bool unsigned_sat) {
    memset(out, 0, bytes);
    for (size_t base = 0; base < bytes; base += 16) {
        unsigned word_base = (unsigned)(base / 2);
        for (unsigned i = 0; i < 8; i++) {
            int16_t v = lhs[word_base + i];
            out[base + i] = unsigned_sat
                ? (uint8_t)(v < 0 ? 0 : (v > 255 ? 255 : v))
                : (uint8_t)(v > 127 ? 127 : (v < -128 ? -128 : v));
        }
        for (unsigned i = 0; i < 8; i++) {
            int16_t v = rhs[word_base + i];
            out[base + 8 + i] = unsigned_sat
                ? (uint8_t)(v < 0 ? 0 : (v > 255 ? 255 : v))
                : (uint8_t)(v > 127 ? 127 : (v < -128 ? -128 : v));
        }
    }
...
static void test_expect_packssdw(int16_t* out, const int32_t* lhs, const int32_t* rhs,
                                 size_t bytes) {
    memset(out, 0, bytes);
    for (size_t base = 0; base < bytes; base += 16) {
        unsigned dword_base = (unsigned)(base / 4);
        unsigned word_base = (unsigned)(base / 2);
        for (unsigned i = 0; i < 4; i++) {
            int32_t v = lhs[dword_base + i];
            out[word_base + i] = v > 32767 ? 32767 : (v < -32768 ? -32768 : (int16_t)v);
        }
        for (unsigned i = 0; i < 4; i++) {
            int32_t v = rhs[dword_base + i];
            out[word_base + 4 + i] = v > 32767 ? 32767 : (v < -32768 ? -32768 : (int16_t)v);
        }
    }
}

TEST(interp_x64_avx2_packed_integer_256_family_semantics) {
    hb_decoded_t d;
    hb_exec_result_t run_out;
    ...
    uint8_t shifts[] = {
        ...
        0xc5, 0xf5, 0xf3, 0xf2  /* vpsllq ymm6, ymm1, xmm2 */
    };
    ASSERT(hb_decode_x64(shifts, 4, (uint64_t)(uintptr_t)shifts, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PSRLW && d.op1.size == 32 && d.op3.size == 16);
    uint64_t base = (uint64_t)(uintptr_t)shifts;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, shifts, sizeof(shifts), base);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)shifts, sizeof(shifts),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint16_t shift_words[16] = {0xffff, 0x8000, 0x0008, 0x00f0, 0x1234, 0xaaaa, 0x0001, 0x0100,
                                0x7fff, 0x4000, 0x2222, 0x3333, 0x4444, 0x5555, 0x6666, 0x7777};
    int32_t shift_dwords[8] = {-16, 0x70000000, 16, -1, -32, 0x40000000, 0x12345678, -123456};
    uint64_t count3 = 3, count4 = 4;
    test_set_ymm_bytes(ctx, HB_REG_XMM1 - HB_REG_XMM0, shift_words);
    memcpy(ctx->regs.x64.xmm[2], &count3, sizeof(count3));
    test_set_ymm_bytes(ctx, HB_REG_XMM4 - HB_REG_XMM0, shift_dwords);
    memcpy(ctx->regs.x64.xmm[5], &count4, sizeof(count4));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint16_t expect_psrlw[16];
    int32_t expect_psrad[8];
    uint64_t qsrc[4], expect_psllq[4];
    memcpy(qsrc, shift_words, sizeof(qsrc));
    for (unsigned i = 0; i < 16; i++) expect_psrlw[i] = (uint16_t)(shift_words[i] >> 3);
    for (unsigned i = 0; i < 8; i++) expect_psrad[i] = shift_dwords[i] >> 4;
    for (unsigned i = 0; i < 4; i++) expect_psllq[i] = qsrc[i] << 3;
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_psrlw));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect_psrad));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expect_psllq));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t sat_avg[] = {
        0xc5, 0xf5, 0xdc, 0xc2, /* vpaddusb ymm0, ymm1, ymm2 */
        0xc5, 0xdd, 0xed, 0xdd, /* vpaddsw  ymm3, ymm4, ymm5 */
        0xc5, 0xf5, 0xe0, 0xf2  /* vpavgb   ymm6, ymm1, ymm2 */
    };
    base = (uint64_t)(uintptr_t)sat_avg;
    dec = hb_decoder_create(HB_ARCH_X64, sat_avg, sizeof(sat_avg), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)sat_avg, sizeof(sat_avg),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint8_t bytes_a[32], bytes_b[32], expect_paddusb[32], expect_pavgb[32];
    int16_t words_a[16], words_b[16], expect_paddsw[16];
    for (unsigned i = 0; i < 32; i++) {
        bytes_a[i] = (uint8_t)(i * 9 + 11);
        bytes_b[i] = (uint8_t)(240 - i * 5);
        unsigned sum = (unsigned)bytes_a[i] + (unsigned)bytes_b[i];
        expect_paddusb[i] = (uint8_t)(sum > 255 ? 255 : sum);
        expect_pavgb[i] = (uint8_t)((sum + 1u) >> 1);
    }
    for (unsigned i = 0; i < 16; i++) {
        words_a[i] = (int16_t)(i % 3 == 0 ? 30000 : (int)i * 1000 - 8000);
        words_b[i] = (int16_t)(i % 4 == 0 ? 10000 : 9000 - (int)i * 700);
        int32_t sum = (int32_t)words_a[i] + (int32_t)words_b[i];
        expect_paddsw[i] = sum > 32767 ? 32767 : (sum < -32768 ? -32768 : (int16_t)sum);
    }
    test_set_ymm_bytes(ctx, 1, bytes_a);
    test_set_ymm_bytes(ctx, 2, bytes_b);
    test_set_ymm_bytes(ctx, 4, words_a);
    test_set_ymm_bytes(ctx, 5, words_b);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_paddusb));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect_paddsw));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expect_pavgb));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    uint8_t muls[] = {
        ...
        0xc5, 0xf5, 0xf5, 0xf2  /* vpmaddwd ymm6, ymm1, ymm2 */
    };
    base = (uint64_t)(uintptr_t)muls;
    dec = hb_decoder_create(HB_ARCH_X64, muls, sizeof(muls), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)muls, sizeof(muls),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    int16_t mul_a[16], mul_b[16];
    uint16_t umul_a[16], umul_b[16], expect_pmulhuw[16], expect_pmullw[16];
    int32_t expect_pmaddwd[8];
    for (unsigned i = 0; i < 16; i++) {
        mul_a[i] = (int16_t)(i * 321 - 2000);
        mul_b[i] = (int16_t)(1500 - (int)i * 77);
        int32_t product = (int32_t)mul_a[i] * (int32_t)mul_b[i];
        expect_pmullw[i] = (uint16_t)product;
        umul_a[i] = (uint16_t)(40000u + i * 123u);
        umul_b[i] = (uint16_t)(30000u + i * 211u);
        expect_pmulhuw[i] = (uint16_t)(((uint32_t)umul_a[i] * (uint32_t)umul_b[i]) >> 16);
    }
    for (unsigned i = 0; i < 8; i++) {
        expect_pmaddwd[i] = (int32_t)mul_a[i * 2] * (int32_t)mul_b[i * 2] +
                            (int32_t)mul_a[i * 2 + 1] * (int32_t)mul_b[i * 2 + 1];
    }
    test_set_ymm_bytes(ctx, 1, mul_a);
    test_set_ymm_bytes(ctx, 2, mul_b);
    test_set_ymm_bytes(ctx, 4, umul_a);
    test_set_ymm_bytes(ctx, 5, umul_b);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_pmullw));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect_pmulhuw));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expect_pmaddwd));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    uint8_t compares[] = {
        ...
        0xc5, 0xf5, 0x76, 0xf2  /* vpcmpeqd ymm6, ymm1, ymm2 */
    };
    base = (uint64_t)(uintptr_t)compares;
    dec = hb_decoder_create(HB_ARCH_X64, compares, sizeof(compares), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)compares, sizeof(compares),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint8_t cmp_a[32], cmp_b[32], expect_pcmpeqb[32], expect_pcmpeqd[32];
    int16_t cmpw_a[16], cmpw_b[16];
    uint16_t expect_pcmpgtw[16];
    for (unsigned i = 0; i < 32; i++) {
        cmp_a[i] = (uint8_t)(i + (i % 5 == 0 ? 7 : 0));
        cmp_b[i] = (uint8_t)(i + (i % 5 == 0 ? 7 : 1));
        expect_pcmpeqb[i] = cmp_a[i] == cmp_b[i] ? 0xff : 0;
    }
    for (unsigned i = 0; i < 8; i++) {
        uint32_t a, b;
        memcpy(&a, cmp_a + i * 4, sizeof(a));
        memcpy(&b, cmp_b + i * 4, sizeof(b));
        memset(expect_pcmpeqd + i * 4, a == b ? 0xff : 0, 4);
    }
    for (unsigned i = 0; i < 16; i++) {
        cmpw_a[i] = (int16_t)((int)i * 300 - 2000);
        cmpw_b[i] = (int16_t)(i % 2 ? cmpw_a[i] + 1 : cmpw_a[i] - 1);
        expect_pcmpgtw[i] = cmpw_a[i] > cmpw_b[i] ? 0xffff : 0;
    }
    test_set_ymm_bytes(ctx, 1, cmp_a);
    test_set_ymm_bytes(ctx, 2, cmp_b);
    test_set_ymm_bytes(ctx, 4, cmpw_a);
    test_set_ymm_bytes(ctx, 5, cmpw_b);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_pcmpeqb));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect_pcmpgtw));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expect_pcmpeqd));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_avx2_unpack_pack_family_semantics) {
    hb_decoded_t d;
    hb_exec_result_t run_out;

    uint8_t xmm_code[] = {
        0xc5, 0xf1, 0x60, 0xc2, /* vpunpcklbw xmm0, xmm1, xmm2 */
        0xc5, 0xd1, 0x63, 0xde  /* vpacksswb  xmm3, xmm5, xmm6 */
    };
    ASSERT(hb_decode_x64(xmm_code, 4, (uint64_t)(uintptr_t)xmm_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PUNPCKLBW && d.op1.size == 16 && d.op2.reg == HB_REG_XMM1);
    ASSERT(hb_decode_x64(xmm_code + 4, 4, (uint64_t)(uintptr_t)(xmm_code + 4), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PACKSSWB && d.op1.size == 16 && d.op2.reg == HB_REG_XMM5);
    uint64_t base = (uint64_t)(uintptr_t)xmm_code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, xmm_code, sizeof(xmm_code), base);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)xmm_code, sizeof(xmm_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint8_t lhs16[16], rhs16[16], expected16[16];
    int16_t pack_lhs16[8] = {-200, -129, -128, -1, 0, 1, 127, 200};
    int16_t pack_rhs16[8] = {-300, -5, 5, 126, 127, 128, 500, -127};
    for (unsigned i = 0; i < 16; i++) {
        lhs16[i] = (uint8_t)(0x10 + i);
        rhs16[i] = (uint8_t)(0xa0 + i);
    }
    memcpy(ctx->regs.x64.xmm[1], lhs16, sizeof(lhs16));
    memcpy(ctx->regs.x64.xmm[2], rhs16, sizeof(rhs16));
    memcpy(ctx->regs.x64.xmm[5], pack_lhs16, sizeof(pack_lhs16));
    memcpy(ctx->regs.x64.xmm[6], pack_rhs16, sizeof(pack_rhs16));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    test_expect_punpck(expected16, lhs16, rhs16, 16, 1, false);
    uint8_t expected_pack16[16];
    test_expect_pack_bytes(expected_pack16, pack_lhs16, pack_rhs16, 16, false);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected16, sizeof(expected16)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[3], expected_pack16, sizeof(expected_pack16)) == 0);

    uint8_t legacy_code[] = {
        0x66, 0x0f, 0x60, 0xc2, /* punpcklbw xmm0, xmm2 */
        0x66, 0x0f, 0x63, 0xde  /* packsswb  xmm3, xmm6 */
    };
    hb_context_t* legacy = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    ASSERT(legacy != NULL);
    legacy->memory = hb_memory_create(0);
    ASSERT(legacy->memory != NULL);
    ASSERT(hb_memory_map(legacy->memory, (hb_gva_t)(uintptr_t)legacy_code, sizeof(legacy_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    legacy->pc = (uint64_t)(uintptr_t)legacy_code;
    memcpy(legacy->regs.x64.xmm[0], lhs16, sizeof(lhs16));
    memcpy(legacy->regs.x64.xmm[2], rhs16, sizeof(rhs16));
    memcpy(legacy->regs.x64.xmm[3], pack_lhs16, sizeof(pack_lhs16));
    memcpy(legacy->regs.x64.xmm[6], pack_rhs16, sizeof(pack_rhs16));
    hb_decoder_t* legacy_dec = hb_decoder_create(HB_ARCH_X64, legacy_code, sizeof(legacy_code), legacy->pc);
    hb_ir_func_t* legacy_func = NULL;
    ASSERT(legacy_dec != NULL);
    ASSERT(hb_lift_func_x64(legacy_dec, &legacy_func) == HB_OK);
    hb_decoder_destroy(legacy_dec);
    ASSERT(hb_runtime_run(legacy, legacy_func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], legacy->regs.x64.xmm[0], sizeof(expected16)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[3], legacy->regs.x64.xmm[3], sizeof(expected_pack16)) == 0);
    hb_context_destroy(legacy);
    hb_ir_func_destroy(legacy_func);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    base = (uint64_t)(uintptr_t)ymm_code;
    dec = hb_decoder_create(HB_ARCH_X64, ymm_code, sizeof(ymm_code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)ymm_code, sizeof(ymm_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint8_t lhs32[32], rhs32[32], expected32[32], expected_pack32[32];
    int16_t pack_lhs32[16], pack_rhs32[16];
    for (unsigned i = 0; i < 32; i++) {
        lhs32[i] = (uint8_t)(0x20 + i);
        rhs32[i] = (uint8_t)(0xc0 + i);
    }
    for (unsigned i = 0; i < 16; i++) {
        pack_lhs32[i] = (int16_t)((int)i * 40 - 120);
        pack_rhs32[i] = (int16_t)(300 - (int)i * 25);
    }
    test_set_ymm_bytes(ctx, 1, lhs32);
    test_set_ymm_bytes(ctx, 2, rhs32);
    test_set_ymm_bytes(ctx, 4, pack_lhs32);
    test_set_ymm_bytes(ctx, 5, pack_rhs32);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    test_expect_punpck(expected32, lhs32, rhs32, 32, 2, true);
    test_expect_pack_bytes(expected_pack32, pack_lhs32, pack_rhs32, 32, true);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expected32));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expected_pack32));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xf5, 0x62, 0x05, 0x00, 0x00, 0x00, 0x00},
        {0}
    };
    for (unsigned i = 0; i < 32; i++) mem_unpck.mem[i] = (uint8_t)(0x80 + i);
    base = (uint64_t)(uintptr_t)&mem_unpck;
    dec = hb_decoder_create(HB_ARCH_X64, mem_unpck.code, sizeof(mem_unpck.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&mem_unpck, sizeof(mem_unpck),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    test_set_ymm_bytes(ctx, 1, lhs32);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    test_expect_punpck(expected32, lhs32, mem_unpck.mem, 32, 4, false);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expected32));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xdd, 0x6b, 0x1d, 0x00, 0x00, 0x00, 0x00},
        {0}
    };
    int32_t pack_lhs_dword[8], expected_pack_dword_src[8];
    int16_t expected_packssdw[16];
    for (unsigned i = 0; i < 8; i++) {
        pack_lhs_dword[i] = (int32_t)i * 20000 - 50000;
        mem_pack.mem[i] = 50000 - (int32_t)i * 15000;
        expected_pack_dword_src[i] = mem_pack.mem[i];
    }
    base = (uint64_t)(uintptr_t)&mem_pack;
    dec = hb_decoder_create(HB_ARCH_X64, mem_pack.code, sizeof(mem_pack.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&mem_pack, sizeof(mem_pack),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    test_set_ymm_bytes(ctx, 4, pack_lhs_dword);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    test_expect_packssdw(expected_packssdw, pack_lhs_dword, expected_pack_dword_src, 32);
    ASSERT(test_ymm_bytes_equal(ctx, 3, expected_packssdw));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_vex_movdqa_movdqu_load_store_semantics) {
    hb_decoded_t d;
    hb_exec_result_t run_out;

    uint8_t reg_code[] = {
        0xc5, 0xfa, 0x6f, 0xc1, /* vmovdqu xmm0, xmm1 */
        0xc5, 0xfd, 0x6f, 0xd1  /* vmovdqa ymm2, ymm1 */
    };
    ASSERT(hb_decode_x64(reg_code, 4, (uint64_t)(uintptr_t)reg_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SSE_MOV && d.op1.size == 16 && d.op2.size == 16);
    ASSERT(hb_decode_x64(reg_code + 4, 4, (uint64_t)(uintptr_t)(reg_code + 4), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SSE_MOV && d.op1.size == 32 && d.op2.size == 32);
    uint64_t base = (uint64_t)(uintptr_t)reg_code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, reg_code, sizeof(reg_code), base);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ...
    ctx->pc = base;
    uint8_t src32[32];
    for (unsigned i = 0; i < 32; i++) src32[i] = (uint8_t)(0x40 + i);
    test_set_ymm_bytes(ctx, 1, src32);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], src32, 16) == 0);
    ASSERT(test_ymm_bytes_equal(ctx, 2, src32));

    uint8_t legacy_code[] = {0xf3, 0x0f, 0x6f, 0xc1}; /* movdqu xmm0, xmm1 */
    hb_context_t* legacy = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    ASSERT(legacy != NULL);
    legacy->memory = hb_memory_create(0);
    ASSERT(legacy->memory != NULL);
    ASSERT(hb_memory_map(legacy->memory, (hb_gva_t)(uintptr_t)legacy_code, sizeof(legacy_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    legacy->pc = (uint64_t)(uintptr_t)legacy_code;
    memcpy(legacy->regs.x64.xmm[1], src32, 16);
    hb_decoder_t* legacy_dec = hb_decoder_create(HB_ARCH_X64, legacy_code, sizeof(legacy_code), legacy->pc);
    hb_ir_func_t* legacy_func = NULL;
    ASSERT(legacy_dec != NULL);
    ASSERT(hb_lift_func_x64(legacy_dec, &legacy_func) == HB_OK);
    ...
    ASSERT(hb_runtime_run(legacy, legacy_func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], legacy->regs.x64.xmm[0], 16) == 0);
    hb_context_destroy(legacy);
    hb_ir_func_destroy(legacy_func);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xfd, 0x6f, 0x05, 0x00, 0x00, 0x00, 0x00},
        {0}
    };
    for (unsigned i = 0; i < 32; i++) load_case.mem[i] = (uint8_t)(0x90 + i);
    base = (uint64_t)(uintptr_t)&load_case;
    dec = hb_decoder_create(HB_ARCH_X64, load_case.code, sizeof(load_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&load_case, sizeof(load_case),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, load_case.mem));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xfd, 0x7f, 0x05, 0x00, 0x00, 0x00, 0x00},
        {0}
    };
    base = (uint64_t)(uintptr_t)&store_case;
    dec = hb_decoder_create(HB_ARCH_X64, store_case.code, sizeof(store_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&store_case, sizeof(store_case),
                         HB_PERM_READ | HB_PERM_WRITE | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    test_set_ymm_bytes(ctx, 0, src32);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(memcmp(store_case.out, src32, sizeof(src32)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_vex_packed_mov_float_load_store_semantics) {
    hb_decoded_t d;
    hb_exec_result_t run_out;
    ...
    uint8_t reg_code[] = {
        ...
        0xc5, 0xfd, 0x28, 0xd9  /* vmovapd ymm3, ymm1 */
    };
    ASSERT(hb_decode_x64(reg_code, 4, (uint64_t)(uintptr_t)reg_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SSE_MOV && d.op1.size == 16 && d.op2.size == 16);
    ASSERT(hb_decode_x64(reg_code + 4, 4, (uint64_t)(uintptr_t)(reg_code + 4), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SSE_MOV && d.op1.size == 32 && d.op2.size == 32);
    ASSERT(hb_decode_x64(reg_code + 8, 4, (uint64_t)(uintptr_t)(reg_code + 8), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SSE_MOV && d.op1.size == 32 && d.op2.size == 32);
    ...
    ctx->pc = base;
    uint8_t src32[32];
    for (unsigned i = 0; i < 32; i++) src32[i] = (uint8_t)(0x20 + i);
    test_set_ymm_bytes(ctx, 1, src32);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], src32, 16) == 0);
    ASSERT(test_ymm_bytes_equal(ctx, 2, src32));
    ASSERT(test_ymm_bytes_equal(ctx, 3, src32));

    uint8_t legacy_code[] = {0x0f, 0x10, 0xc1}; /* movups xmm0, xmm1 */
    hb_context_t* legacy = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    ASSERT(legacy != NULL);
    legacy->memory = hb_memory_create(0);
    ASSERT(legacy->memory != NULL);
    ...
    struct {
        ...
        {0xc5, 0xfc, 0x28, 0x05, 0x00, 0x00, 0x00, 0x00},
        {0}
    };
    for (unsigned i = 0; i < 32; i++) load_case.mem[i] = (uint8_t)(0x70 + i);
    base = (uint64_t)(uintptr_t)&load_case;
    dec = hb_decoder_create(HB_ARCH_X64, load_case.code, sizeof(load_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ...
    struct {
        ...
        {0xc5, 0xfc, 0x11, 0x05, 0x00, 0x00, 0x00, 0x00},
        {0}
    };
    base = (uint64_t)(uintptr_t)&store_case;
    dec = hb_decoder_create(HB_ARCH_X64, store_case.code, sizeof(store_case.code), base);
...
TEST(interp_x64_vex_packed_float_pd_and_sqrt_semantics) {
    hb_exec_result_t out;
    uint8_t code[] = {
        0xc5, 0xf5, 0x5c, 0xc2, /* vsubpd ymm0, ymm1, ymm2 */
        0xc5, 0xf5, 0x59, 0xda, /* vmulpd ymm3, ymm1, ymm2 */
        0xc5, 0xf5, 0x5e, 0xe2, /* vdivpd ymm4, ymm1, ymm2 */
        0xc5, 0xf5, 0x5d, 0xea, /* vminpd ymm5, ymm1, ymm2 */
        0xc5, 0xf5, 0x5f, 0xf2  /* vmaxpd ymm6, ymm1, ymm2 */
    };
    uint64_t base = (uint64_t)(uintptr_t)code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, code, sizeof(code), base);
    hb_ir_func_t* func = NULL;
    ...
    uint64_t lhs_bits[4] = {test_double_bits(10.0), test_double_bits(-20.0),
                            test_double_bits(40.0), test_double_bits(1.5)};
    uint64_t rhs_bits[4] = {test_double_bits(1.0), test_double_bits(2.0),
                            test_double_bits(-4.0), test_double_bits(0.5)};
    test_set_ymm_bytes(ctx, 1, (const uint8_t*)lhs_bits);
    test_set_ymm_bytes(ctx, 2, (const uint8_t*)rhs_bits);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);

    uint64_t expect_sub[4] = {test_double_bits(9.0), test_double_bits(-22.0),
                              test_double_bits(44.0), test_double_bits(1.0)};
    uint64_t expect_mul[4] = {test_double_bits(10.0), test_double_bits(-40.0),
                              test_double_bits(-160.0), test_double_bits(0.75)};
    uint64_t expect_div[4] = {test_double_bits(10.0), test_double_bits(-10.0),
                              test_double_bits(-10.0), test_double_bits(3.0)};
    uint64_t expect_min[4] = {test_double_bits(1.0), test_double_bits(-20.0),
                              test_double_bits(-4.0), test_double_bits(0.5)};
    uint64_t expect_max[4] = {test_double_bits(10.0), test_double_bits(2.0),
                              test_double_bits(40.0), test_double_bits(1.5)};
    ASSERT(test_ymm_bytes_equal(ctx, 0, (const uint8_t*)expect_sub));
    ASSERT(test_ymm_bytes_equal(ctx, 3, (const uint8_t*)expect_mul));
    ASSERT(test_ymm_bytes_equal(ctx, 4, (const uint8_t*)expect_div));
    ASSERT(test_ymm_bytes_equal(ctx, 5, (const uint8_t*)expect_min));
    ASSERT(test_ymm_bytes_equal(ctx, 6, (const uint8_t*)expect_max));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vex_xmm[] = {0xc5, 0xf1, 0x5c, 0xc2}; /* vsubpd xmm0, xmm1, xmm2 */
    uint8_t legacy_xmm[] = {0x66, 0x0f, 0x5c, 0xc2}; /* subpd xmm0, xmm2 */
    hb_context_t* vex_ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    hb_context_t* legacy_ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    ASSERT(vex_ctx != NULL && legacy_ctx != NULL);
    vex_ctx->memory = hb_memory_create(0);
    legacy_ctx->memory = hb_memory_create(0);
    ASSERT(vex_ctx->memory != NULL && legacy_ctx->memory != NULL);
    ASSERT(hb_memory_map(vex_ctx->memory, (hb_gva_t)(uintptr_t)vex_xmm, sizeof(vex_xmm),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ASSERT(hb_memory_map(legacy_ctx->memory, (hb_gva_t)(uintptr_t)legacy_xmm, sizeof(legacy_xmm),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    vex_ctx->pc = (uint64_t)(uintptr_t)vex_xmm;
    legacy_ctx->pc = (uint64_t)(uintptr_t)legacy_xmm;
    memcpy(vex_ctx->regs.x64.xmm[1], lhs_bits, 16);
    memcpy(vex_ctx->regs.x64.xmm[2], rhs_bits, 16);
    memcpy(legacy_ctx->regs.x64.xmm[0], lhs_bits, 16);
    memcpy(legacy_ctx->regs.x64.xmm[2], rhs_bits, 16);
    hb_decoder_t* vex_dec = hb_decoder_create(HB_ARCH_X64, vex_xmm, sizeof(vex_xmm), vex_ctx->pc);
    hb_decoder_t* leg_dec = hb_decoder_create(HB_ARCH_X64, legacy_xmm, sizeof(legacy_xmm), legacy_ctx->pc);
    hb_ir_func_t* vex_func = NULL;
    hb_ir_func_t* leg_func = NULL;
    ASSERT(vex_dec != NULL && leg_dec != NULL);
    ASSERT(hb_lift_func_x64(vex_dec, &vex_func) == HB_OK);
    ASSERT(hb_lift_func_x64(leg_dec, &leg_func) == HB_OK);
    hb_decoder_destroy(vex_dec);
    hb_decoder_destroy(leg_dec);
    ASSERT(hb_runtime_run(vex_ctx, vex_func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(hb_runtime_run(legacy_ctx, leg_func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(memcmp(vex_ctx->regs.x64.xmm[0], legacy_ctx->regs.x64.xmm[0], 16) == 0);
    hb_context_destroy(vex_ctx);
    hb_context_destroy(legacy_ctx);
    hb_ir_func_destroy(vex_func);
    hb_ir_func_destroy(leg_func);

    struct {
        uint8_t code[8];
        uint64_t mem[4];
    } mem_case = {
        {0xc5, 0xf5, 0x5c, 0x05, 0x00, 0x00, 0x00, 0x00},
        {test_double_bits(2.0), test_double_bits(4.0),
         test_double_bits(6.0), test_double_bits(8.0)}
    };
    base = (uint64_t)(uintptr_t)&mem_case;
    dec = hb_decoder_create(HB_ARCH_X64, mem_case.code, sizeof(mem_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&mem_case, sizeof(mem_case),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    test_set_ymm_bytes(ctx, 1, (const uint8_t*)lhs_bits);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint64_t expect_mem_sub[4] = {test_double_bits(8.0), test_double_bits(-24.0),
                                  test_double_bits(34.0), test_double_bits(-6.5)};
    ASSERT(test_ymm_bytes_equal(ctx, 0, (const uint8_t*)expect_mem_sub));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    base = (uint64_t)(uintptr_t)sqrt_code;
    dec = hb_decoder_create(HB_ARCH_X64, sqrt_code, sizeof(sqrt_code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)sqrt_code, sizeof(sqrt_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint32_t ps_bits[8] = {
        test_float_bits(1.0f), test_float_bits(4.0f), test_float_bits(9.0f), test_float_bits(16.0f),
        test_float_bits(25.0f), test_float_bits(36.0f), test_float_bits(49.0f), test_float_bits(64.0f)
    };
    uint64_t pd_bits[4] = {test_double_bits(4.0), test_double_bits(9.0),
                           test_double_bits(16.0), test_double_bits(25.0)};
    test_set_ymm_bytes(ctx, 1, (const uint8_t*)ps_bits);
    test_set_ymm_bytes(ctx, 3, (const uint8_t*)pd_bits);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint32_t expect_ps[8] = {
        test_float_bits(1.0f), test_float_bits(2.0f), test_float_bits(3.0f), test_float_bits(4.0f),
        test_float_bits(5.0f), test_float_bits(6.0f), test_float_bits(7.0f), test_float_bits(8.0f)
    };
    uint64_t expect_pd[4] = {test_double_bits(2.0), test_double_bits(3.0),
                             test_double_bits(4.0), test_double_bits(5.0)};
    ASSERT(test_ymm_bytes_equal(ctx, 0, (const uint8_t*)expect_ps));
    ASSERT(test_ymm_bytes_equal(ctx, 2, (const uint8_t*)expect_pd));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_vex_scalar_sqrt_semantics) {
    uint8_t code[] = {
        0xc5, 0xf2, 0x51, 0xc2, /* vsqrtss xmm0, xmm1, xmm2 */
        0xc5, 0xdb, 0x51, 0xdd  /* vsqrtsd xmm3, xmm4, xmm5 */
    };
    uint64_t base = (uint64_t)(uintptr_t)code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, code, sizeof(code), base);
    hb_ir_func_t* func = NULL;
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code, sizeof(code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    ctx->regs.x64.xmm[1][0] = 0xaaaaaaaa00000000ULL | test_float_bits(99.0f);
    ctx->regs.x64.xmm[1][1] = 0xbbbbbbbbbbbbbbbbULL;
    ctx->regs.x64.xmm[2][0] = test_float_bits(16.0f);
    ctx->regs.x64.xmm[4][0] = test_double_bits(123.0);
    ctx->regs.x64.xmm[4][1] = 0xccccccccccccccccULL;
    ctx->regs.x64.xmm[5][0] = test_double_bits(81.0);
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT((uint32_t)ctx->regs.x64.xmm[0][0] == test_float_bits(4.0f));
    ASSERT((ctx->regs.x64.xmm[0][0] >> 32) == 0xaaaaaaaaULL);
    ASSERT(ctx->regs.x64.xmm[0][1] == 0xbbbbbbbbbbbbbbbbULL);
    ASSERT(ctx->regs.x64.xmm[3][0] == test_double_bits(9.0));
    ASSERT(ctx->regs.x64.xmm[3][1] == 0xccccccccccccccccULL);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xf2, 0x51, 0x05, 0x00, 0x00, 0x00, 0x00},
        0
    };
    mem_case.mem = test_float_bits(25.0f);
    base = (uint64_t)(uintptr_t)&mem_case;
    dec = hb_decoder_create(HB_ARCH_X64, mem_case.code, sizeof(mem_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&mem_case, sizeof(mem_case),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    ctx->regs.x64.xmm[1][0] = 0xdddddddd00000000ULL | test_float_bits(1.0f);
    ctx->regs.x64.xmm[1][1] = 0xeeeeeeeeeeeeeeeeULL;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT((uint32_t)ctx->regs.x64.xmm[0][0] == test_float_bits(5.0f));
    ASSERT((ctx->regs.x64.xmm[0][0] >> 32) == 0xddddddddULL);
    ASSERT(ctx->regs.x64.xmm[0][1] == 0xeeeeeeeeeeeeeeeeULL);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_vex_scalar_mov_arith_and_upper_zero_semantics) {
    hb_exec_result_t out;
    uint8_t mov_code[] = {
        0xc5, 0xf2, 0x10, 0xc2, /* vmovss xmm0, xmm1, xmm2 */
        0xc5, 0xdb, 0x10, 0xdd  /* vmovsd xmm3, xmm4, xmm5 */
    };
    uint64_t base = (uint64_t)(uintptr_t)mov_code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, mov_code, sizeof(mov_code), base);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)mov_code, sizeof(mov_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint32_t ss_merge[4] = {test_float_bits(1.0f), 0xaaaaaaaaU, 0xbbbbbbbbU, 0xccccccccU};
    uint32_t ss_rhs[4] = {test_float_bits(-4.5f), 0, 0, 0};
    uint64_t sd_merge[2] = {test_double_bits(2.0), 0xddddddddddddddddULL};
    uint64_t sd_rhs[2] = {test_double_bits(81.0), 0};
    memcpy(ctx->regs.x64.xmm[1], ss_merge, sizeof(ss_merge));
    memcpy(ctx->regs.x64.xmm[2], ss_rhs, sizeof(ss_rhs));
    memcpy(ctx->regs.x64.xmm[4], sd_merge, sizeof(sd_merge));
    memcpy(ctx->regs.x64.xmm[5], sd_rhs, sizeof(sd_rhs));
    memset(ctx->ymm_hi[0], 0x5a, sizeof(ctx->ymm_hi[0]));
    memset(ctx->ymm_hi[3], 0xa5, sizeof(ctx->ymm_hi[3]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT((uint32_t)ctx->regs.x64.xmm[0][0] == test_float_bits(-4.5f));
    ASSERT(memcmp((uint8_t*)ctx->regs.x64.xmm[0] + 4, (uint8_t*)ss_merge + 4, 12) == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    ASSERT(ctx->regs.x64.xmm[3][0] == test_double_bits(81.0));
    ASSERT(ctx->regs.x64.xmm[3][1] == sd_merge[1]);
    ASSERT(ctx->ymm_hi[3][0] == 0 && ctx->ymm_hi[3][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xfa, 0x10, 0x05, 0x00, 0x00, 0x00, 0x00},
        0
    };
    load_case.mem = test_float_bits(6.25f);
    base = (uint64_t)(uintptr_t)&load_case;
    dec = hb_decoder_create(HB_ARCH_X64, load_case.code, sizeof(load_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&load_case, sizeof(load_case),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    memset(ctx->regs.x64.xmm[0], 0x77, sizeof(ctx->regs.x64.xmm[0]));
    memset(ctx->ymm_hi[0], 0x88, sizeof(ctx->ymm_hi[0]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT((uint32_t)ctx->regs.x64.xmm[0][0] == test_float_bits(6.25f));
    ASSERT((ctx->regs.x64.xmm[0][0] >> 32) == 0);
    ASSERT(ctx->regs.x64.xmm[0][1] == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xfa, 0x11, 0x05, 0x00, 0x00, 0x00, 0x00},
        0
    };
    base = (uint64_t)(uintptr_t)&store_case;
    dec = hb_decoder_create(HB_ARCH_X64, store_case.code, sizeof(store_case.code), base);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&store_case, sizeof(store_case),
                         HB_PERM_READ | HB_PERM_WRITE | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    ctx->regs.x64.xmm[0][0] = test_float_bits(-13.0f);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(store_case.out_mem == test_float_bits(-13.0f));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    load_sd_case.mem = test_double_bits(-42.5);
    base = (uint64_t)(uintptr_t)&load_sd_case;
    dec = hb_decoder_create(HB_ARCH_X64, load_sd_case.code, sizeof(load_sd_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&load_sd_case, sizeof(load_sd_case),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    memset(ctx->regs.x64.xmm[0], 0x44, sizeof(ctx->regs.x64.xmm[0]));
    memset(ctx->ymm_hi[0], 0x55, sizeof(ctx->ymm_hi[0]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.xmm[0][0] == test_double_bits(-42.5));
    ASSERT(ctx->regs.x64.xmm[0][1] == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xfb, 0x11, 0x05, 0x00, 0x00, 0x00, 0x00},
        0
    };
    base = (uint64_t)(uintptr_t)&store_sd_case;
    dec = hb_decoder_create(HB_ARCH_X64, store_sd_case.code, sizeof(store_sd_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)&store_sd_case, sizeof(store_sd_case),
                         HB_PERM_READ | HB_PERM_WRITE | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    ctx->regs.x64.xmm[0][0] = test_double_bits(123.25);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(store_sd_case.out_mem == test_double_bits(123.25));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t ss_code[] = {
        0xc5, 0xf2, 0x58, 0xc2, /* vaddss xmm0, xmm1, xmm2 */
        0xc5, 0xf2, 0x5c, 0xda, /* vsubss xmm3, xmm1, xmm2 */
        0xc5, 0xf2, 0x59, 0xe2, /* vmulss xmm4, xmm1, xmm2 */
        0xc5, 0xf2, 0x5e, 0xea, /* vdivss xmm5, xmm1, xmm2 */
        0xc5, 0xf2, 0x5d, 0xf2, /* vminss xmm6, xmm1, xmm2 */
        0xc5, 0xf2, 0x5f, 0xfa  /* vmaxss xmm7, xmm1, xmm2 */
    };
    base = (uint64_t)(uintptr_t)ss_code;
    dec = hb_decoder_create(HB_ARCH_X64, ss_code, sizeof(ss_code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)ss_code, sizeof(ss_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint32_t ss_lhs[4] = {test_float_bits(9.0f), 0x10101010U, 0x20202020U, 0x30303030U};
    uint32_t ss_rhs2[4] = {test_float_bits(3.0f), 0, 0, 0};
    memcpy(ctx->regs.x64.xmm[1], ss_lhs, sizeof(ss_lhs));
    memcpy(ctx->regs.x64.xmm[2], ss_rhs2, sizeof(ss_rhs2));
    for (unsigned r = 0; r < 8; r++) memset(ctx->ymm_hi[r], 0xff, sizeof(ctx->ymm_hi[r]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint32_t ss_expected[6] = {test_float_bits(12.0f), test_float_bits(6.0f),
                               test_float_bits(27.0f), test_float_bits(3.0f),
                               test_float_bits(3.0f), test_float_bits(9.0f)};
    int ss_regs[6] = {0, 3, 4, 5, 6, 7};
    for (unsigned i = 0; i < 6; i++) {
        int reg = ss_regs[i];
        ASSERT((uint32_t)ctx->regs.x64.xmm[reg][0] == ss_expected[i]);
        ASSERT(memcmp((uint8_t*)ctx->regs.x64.xmm[reg] + 4, (uint8_t*)ss_lhs + 4, 12) == 0);
        ASSERT(ctx->ymm_hi[reg][0] == 0 && ctx->ymm_hi[reg][1] == 0);
    }
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t ss_edge_code[] = {
        0xc5, 0xf2, 0x58, 0xc2, /* vaddss xmm0, xmm1, xmm2: +Inf + -Inf => NaN */
        0xc5, 0xda, 0x5e, 0xdd  /* vdivss xmm3, xmm4, xmm5: -0 / +3 => -0 */
    };
    base = (uint64_t)(uintptr_t)ss_edge_code;
    dec = hb_decoder_create(HB_ARCH_X64, ss_edge_code, sizeof(ss_edge_code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)ss_edge_code, sizeof(ss_edge_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint32_t ss_inf_merge[4] = {0x7f800000U, 0x11111111U, 0x22222222U, 0x33333333U};
    uint32_t ss_neg_inf[4] = {0xff800000U, 0, 0, 0};
    uint32_t ss_neg_zero[4] = {0x80000000U, 0x44444444U, 0x55555555U, 0x66666666U};
    uint32_t ss_three[4] = {test_float_bits(3.0f), 0, 0, 0};
    memcpy(ctx->regs.x64.xmm[1], ss_inf_merge, sizeof(ss_inf_merge));
    memcpy(ctx->regs.x64.xmm[2], ss_neg_inf, sizeof(ss_neg_inf));
    memcpy(ctx->regs.x64.xmm[4], ss_neg_zero, sizeof(ss_neg_zero));
    memcpy(ctx->regs.x64.xmm[5], ss_three, sizeof(ss_three));
    memset(ctx->ymm_hi[0], 0x9a, sizeof(ctx->ymm_hi[0]));
    memset(ctx->ymm_hi[3], 0x9b, sizeof(ctx->ymm_hi[3]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint32_t ss_nan = (uint32_t)ctx->regs.x64.xmm[0][0];
    ASSERT((ss_nan & 0x7f800000U) == 0x7f800000U && (ss_nan & 0x007fffffU) != 0);
    ASSERT(memcmp((uint8_t*)ctx->regs.x64.xmm[0] + 4, (uint8_t*)ss_inf_merge + 4, 12) == 0);
    ASSERT((uint32_t)ctx->regs.x64.xmm[3][0] == 0x80000000U);
    ASSERT(memcmp((uint8_t*)ctx->regs.x64.xmm[3] + 4, (uint8_t*)ss_neg_zero + 4, 12) == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    ASSERT(ctx->ymm_hi[3][0] == 0 && ctx->ymm_hi[3][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    uint8_t sd_code[] = {
        ...
        0xc5, 0xf3, 0x5c, 0xda, /* vsubsd xmm3, xmm1, xmm2 */
        0xc5, 0xf3, 0x59, 0xe2, /* vmulsd xmm4, xmm1, xmm2 */
        0xc5, 0xf3, 0x5e, 0xea, /* vdivsd xmm5, xmm1, xmm2 */
        0xc5, 0xf3, 0x5d, 0xf2, /* vminsd xmm6, xmm1, xmm2 */
        0xc5, 0xf3, 0x5f, 0xfa  /* vmaxsd xmm7, xmm1, xmm2 */
    };
    base = (uint64_t)(uintptr_t)sd_code;
    dec = hb_decoder_create(HB_ARCH_X64, sd_code, sizeof(sd_code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)sd_code, sizeof(sd_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint64_t sd_lhs[2] = {test_double_bits(8.0), 0x5151515151515151ULL};
    uint64_t sd_rhs2[2] = {test_double_bits(2.0), 0};
    memcpy(ctx->regs.x64.xmm[1], sd_lhs, sizeof(sd_lhs));
    memcpy(ctx->regs.x64.xmm[2], sd_rhs2, sizeof(sd_rhs2));
    for (unsigned r = 0; r < 8; r++) memset(ctx->ymm_hi[r], 0xee, sizeof(ctx->ymm_hi[r]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint64_t sd_expected[6] = {test_double_bits(10.0), test_double_bits(6.0),
                               test_double_bits(16.0), test_double_bits(4.0),
                               test_double_bits(2.0), test_double_bits(8.0)};
    for (unsigned i = 0; i < 6; i++) {
        int reg = ss_regs[i];
        ASSERT(ctx->regs.x64.xmm[reg][0] == sd_expected[i]);
        ASSERT(ctx->regs.x64.xmm[reg][1] == sd_lhs[1]);
        ASSERT(ctx->ymm_hi[reg][0] == 0 && ctx->ymm_hi[reg][1] == 0);
    }
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t sd_edge_code[] = {
        0xc5, 0xf3, 0x58, 0xc2, /* vaddsd xmm0, xmm1, xmm2: +Inf + -Inf => NaN */
        0xc5, 0xdb, 0x5c, 0xdd  /* vsubsd xmm3, xmm4, xmm5: -0 - +0 => -0 */
    };
    base = (uint64_t)(uintptr_t)sd_edge_code;
    dec = hb_decoder_create(HB_ARCH_X64, sd_edge_code, sizeof(sd_edge_code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)sd_edge_code, sizeof(sd_edge_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint64_t sd_inf_merge[2] = {0x7ff0000000000000ULL, 0x1111222233334444ULL};
    uint64_t sd_neg_inf[2] = {0xfff0000000000000ULL, 0};
    uint64_t sd_neg_zero[2] = {0x8000000000000000ULL, 0x5555666677778888ULL};
    uint64_t sd_pos_zero[2] = {0, 0};
    memcpy(ctx->regs.x64.xmm[1], sd_inf_merge, sizeof(sd_inf_merge));
    memcpy(ctx->regs.x64.xmm[2], sd_neg_inf, sizeof(sd_neg_inf));
    memcpy(ctx->regs.x64.xmm[4], sd_neg_zero, sizeof(sd_neg_zero));
    memcpy(ctx->regs.x64.xmm[5], sd_pos_zero, sizeof(sd_pos_zero));
    memset(ctx->ymm_hi[0], 0x8a, sizeof(ctx->ymm_hi[0]));
    memset(ctx->ymm_hi[3], 0x8b, sizeof(ctx->ymm_hi[3]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint64_t sd_nan = ctx->regs.x64.xmm[0][0];
    ASSERT((sd_nan & 0x7ff0000000000000ULL) == 0x7ff0000000000000ULL &&
           (sd_nan & 0x000fffffffffffffULL) != 0);
    ASSERT(ctx->regs.x64.xmm[0][1] == sd_inf_merge[1]);
    ASSERT(ctx->regs.x64.xmm[3][0] == 0x8000000000000000ULL);
    ASSERT(ctx->regs.x64.xmm[3][1] == sd_neg_zero[1]);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    ASSERT(ctx->ymm_hi[3][0] == 0 && ctx->ymm_hi[3][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_vex128_upper_zero_audit) {
    uint8_t code[] = {
        0xc5, 0xf0, 0x58, 0xc2,       /* vaddps xmm0, xmm1, xmm2 */
        0xc5, 0xf1, 0xef, 0xda,       /* vpxor xmm3, xmm1, xmm2 */
        0xc5, 0xf1, 0xfc, 0xe2,       /* vpaddb xmm4, xmm1, xmm2 */
        0xc5, 0xf1, 0x60, 0xea,       /* vpunpcklbw xmm5, xmm1, xmm2 */
        0xc4, 0xe2, 0x71, 0x40, 0xf2, /* vpmulld xmm6, xmm1, xmm2 */
        0xc5, 0xf8, 0x10, 0xf9        /* vmovups xmm7, xmm1 */
    };
    uint64_t base = (uint64_t)(uintptr_t)code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, code, sizeof(code), base);
    hb_ir_func_t* func = NULL;
    ...
    for (unsigned i = 0; i < 32; i++) {
        lhs[i] = (uint8_t)(i + 1);
        rhs[i] = (uint8_t)(0x80 + i);
    }
    test_set_ymm_bytes(ctx, 1, lhs);
    test_set_ymm_bytes(ctx, 2, rhs);
    for (unsigned r = 0; r < 8; r++) memset(ctx->ymm_hi[r], 0xab, sizeof(ctx->ymm_hi[r]));
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    for (unsigned r = 0; r < 8; r++) {
        if (r == 1 || r == 2) continue;
        ASSERT(ctx->ymm_hi[r][0] == 0 && ctx->ymm_hi[r][1] == 0);
    }
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    tests_passed++;
...
TEST(interp_x64_vex_bitwise_pd_family_semantics) {
    hb_exec_result_t run_out;
    uint8_t code[] = {
        0xc5, 0xf5, 0x54, 0xc2, /* vandpd  ymm0, ymm1, ymm2 */
        0xc5, 0xf5, 0x55, 0xda, /* vandnpd ymm3, ymm1, ymm2 */
        0xc5, 0xf5, 0x56, 0xe2, /* vorpd   ymm4, ymm1, ymm2 */
        0xc5, 0xf5, 0x57, 0xea  /* vxorpd  ymm5, ymm1, ymm2 */
    };
    uint64_t base = (uint64_t)(uintptr_t)code;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, code, sizeof(code), base);
    hb_ir_func_t* func = NULL;
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code, sizeof(code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint8_t lhs[32], rhs[32], expect_and[32], expect_andn[32], expect_or[32], expect_xor[32];
    for (unsigned i = 0; i < 32; i++) {
        lhs[i] = (uint8_t)(0xf0u ^ (i * 13u));
        rhs[i] = (uint8_t)(0x33u + i * 7u);
        expect_and[i] = lhs[i] & rhs[i];
        expect_andn[i] = (uint8_t)(~lhs[i] & rhs[i]);
        expect_or[i] = lhs[i] | rhs[i];
        expect_xor[i] = lhs[i] ^ rhs[i];
    }
    test_set_ymm_bytes(ctx, 1, lhs);
    test_set_ymm_bytes(ctx, 2, rhs);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_and));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect_andn));
    ASSERT(test_ymm_bytes_equal(ctx, 4, expect_or));
    ASSERT(test_ymm_bytes_equal(ctx, 5, expect_xor));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc5, 0xf5, 0x54, 0x05, 0x00, 0x00, 0x00, 0x00},
        {0}
    };
    for (unsigned i = 0; i < 32; i++) mem_case.mem[i] = (uint8_t)(0xa5u + i * 3u);
    base = (uint64_t)(uintptr_t)&mem_case;
    dec = hb_decoder_create(HB_ARCH_X64, mem_case.code, sizeof(mem_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ...
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    for (unsigned i = 0; i < 32; i++) expect_and[i] = lhs[i] & mem_case.mem[i];
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_and));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vex_xmm[] = {0xc5, 0xf1, 0x54, 0xc2}; /* vandpd xmm0, xmm1, xmm2 */
    uint8_t legacy_xmm[] = {0x66, 0x0f, 0x54, 0xc2}; /* andpd xmm0, xmm2 */
    hb_context_t* vex_ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    hb_context_t* legacy_ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    ASSERT(vex_ctx != NULL && legacy_ctx != NULL);
    vex_ctx->memory = hb_memory_create(0);
    ...
    vex_ctx->pc = (uint64_t)(uintptr_t)vex_xmm;
    legacy_ctx->pc = (uint64_t)(uintptr_t)legacy_xmm;
    memcpy(vex_ctx->regs.x64.xmm[1], lhs, 16);
    memcpy(vex_ctx->regs.x64.xmm[2], rhs, 16);
    memcpy(legacy_ctx->regs.x64.xmm[0], lhs, 16);
    memcpy(legacy_ctx->regs.x64.xmm[2], rhs, 16);
    hb_decoder_t* vex_dec = hb_decoder_create(HB_ARCH_X64, vex_xmm, sizeof(vex_xmm), vex_ctx->pc);
    hb_decoder_t* leg_dec = hb_decoder_create(HB_ARCH_X64, legacy_xmm, sizeof(legacy_xmm), legacy_ctx->pc);
    hb_ir_func_t* vex_func = NULL;
    hb_ir_func_t* leg_func = NULL;
    ...
    hb_decoder_destroy(vex_dec);
    hb_decoder_destroy(leg_dec);
    ASSERT(hb_runtime_run(vex_ctx, vex_func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(hb_runtime_run(legacy_ctx, leg_func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(memcmp(vex_ctx->regs.x64.xmm[0], legacy_ctx->regs.x64.xmm[0], 16) == 0);
    hb_context_destroy(vex_ctx);
    hb_context_destroy(legacy_ctx);
...
TEST(interp_x64_avx2_0f38_compare_minmax_mul_family_semantics) {
    hb_decoded_t d;
    hb_exec_result_t run_out;

    uint8_t wide_ops[] = {
        0xc4, 0xe2, 0x75, 0x29, 0xc2, /* vpcmpeqq ymm0, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x37, 0xda, /* vpcmpgtq ymm3, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x40, 0xe2, /* vpmulld  ymm4, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x28, 0xea, /* vpmuldq  ymm5, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x39, 0xf2, /* vpminsd  ymm6, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x3f, 0xfa  /* vpmaxud  ymm7, ymm1, ymm2 */
    };
    ASSERT(hb_decode_x64(wide_ops, 5, (uint64_t)(uintptr_t)wide_ops, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PCMPEQQ && d.op1.size == 32);
    ASSERT(hb_decode_x64(wide_ops + 5, 5, (uint64_t)(uintptr_t)(wide_ops + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PCMPGTQ && d.op2.reg == HB_REG_XMM1 && d.op3.reg == HB_REG_XMM2);
    uint64_t base = (uint64_t)(uintptr_t)wide_ops;
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, wide_ops, sizeof(wide_ops), base);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)wide_ops, sizeof(wide_ops),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;

    int32_t lhs_d[8] = {10, -20, 3000, -4, 200000, 5, 100000, -7};
    int32_t rhs_d[8] = {10, 30, -2, -5, 200000, 5, 50000, 9};
    test_set_ymm_bytes(ctx, 1, lhs_d);
    test_set_ymm_bytes(ctx, 2, rhs_d);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);

    uint64_t expect_pcmpeqq[4] = {0, 0, UINT64_MAX, 0};
    uint64_t expect_pcmpgtq[4] = {0, UINT64_MAX, 0, 0};
    uint32_t expect_pmulld[8];
    int64_t expect_pmuldq[4];
    int32_t expect_pminsd[8];
    uint32_t expect_pmaxud[8];
    for (unsigned i = 0; i < 8; i++) {
        expect_pmulld[i] = (uint32_t)((int64_t)lhs_d[i] * (int64_t)rhs_d[i]);
        expect_pminsd[i] = lhs_d[i] < rhs_d[i] ? lhs_d[i] : rhs_d[i];
        uint32_t a = (uint32_t)lhs_d[i], b = (uint32_t)rhs_d[i];
        expect_pmaxud[i] = a > b ? a : b;
    }
    for (unsigned i = 0; i < 4; i++) {
        expect_pmuldq[i] = (int64_t)lhs_d[i * 2] * (int64_t)rhs_d[i * 2];
    }
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_pcmpeqq));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect_pcmpgtq));
    ASSERT(test_ymm_bytes_equal(ctx, 4, expect_pmulld));
    ASSERT(test_ymm_bytes_equal(ctx, 5, expect_pmuldq));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expect_pminsd));
    ASSERT(test_ymm_bytes_equal(ctx, 7, expect_pmaxud));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t narrow_ops[] = {
        0xc4, 0xe2, 0x75, 0x38, 0xc2, /* vpminsb ymm0, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x3c, 0xda, /* vpmaxsb ymm3, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x3a, 0xe2, /* vpminuw ymm4, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x3e, 0xea, /* vpmaxuw ymm5, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x3b, 0xf2, /* vpminud ymm6, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x3d, 0xfa  /* vpmaxsd ymm7, ymm1, ymm2 */
    };
    ASSERT(hb_decode_x64(narrow_ops, 5, (uint64_t)(uintptr_t)narrow_ops, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PMINSB && d.op1.size == 32);
    base = (uint64_t)(uintptr_t)narrow_ops;
    dec = hb_decoder_create(HB_ARCH_X64, narrow_ops, sizeof(narrow_ops), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)narrow_ops, sizeof(narrow_ops),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;
    uint8_t lhs_b[32], rhs_b[32], expect_pminsb[32], expect_pmaxsb[32];
    uint16_t expect_pminuw[16], expect_pmaxuw[16];
    uint32_t expect_pminud[8];
    int32_t expect_pmaxsd[8];
    for (unsigned i = 0; i < 32; i++) {
        lhs_b[i] = (uint8_t)(i * 17u + 3u);
        rhs_b[i] = (uint8_t)(240u - i * 11u);
        int8_t as = (int8_t)lhs_b[i], bs = (int8_t)rhs_b[i];
        expect_pminsb[i] = as < bs ? lhs_b[i] : rhs_b[i];
        expect_pmaxsb[i] = as > bs ? lhs_b[i] : rhs_b[i];
    }
    for (unsigned i = 0; i < 16; i++) {
        uint16_t a, b;
        memcpy(&a, lhs_b + i * 2, sizeof(a));
        memcpy(&b, rhs_b + i * 2, sizeof(b));
        expect_pminuw[i] = a < b ? a : b;
        expect_pmaxuw[i] = a > b ? a : b;
    }
    for (unsigned i = 0; i < 8; i++) {
        uint32_t au, bu;
        int32_t as, bs;
        memcpy(&au, lhs_b + i * 4, sizeof(au));
        memcpy(&bu, rhs_b + i * 4, sizeof(bu));
        memcpy(&as, lhs_b + i * 4, sizeof(as));
        memcpy(&bs, rhs_b + i * 4, sizeof(bs));
        expect_pminud[i] = au < bu ? au : bu;
        expect_pmaxsd[i] = as > bs ? as : bs;
    }
    test_set_ymm_bytes(ctx, 1, lhs_b);
    test_set_ymm_bytes(ctx, 2, rhs_b);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_pminsb));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect_pmaxsb));
    ASSERT(test_ymm_bytes_equal(ctx, 4, expect_pminuw));
    ASSERT(test_ymm_bytes_equal(ctx, 5, expect_pmaxuw));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expect_pminud));
    ASSERT(test_ymm_bytes_equal(ctx, 7, expect_pmaxsd));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
    ...
    struct {
        ...
        {0xc4, 0xe2, 0x75, 0x40, 0x05, 0x00, 0x00, 0x00, 0x00},
        {2, -3, 4, -5, 6, -7, 8, -9}
    };
    base = (uint64_t)(uintptr_t)&mem_case;
    int32_t mem_disp = (int32_t)((uint64_t)(uintptr_t)mem_case.mem -
                                 (base + sizeof(mem_case.code)));
    memcpy(mem_case.code + 5, &mem_disp, sizeof(mem_disp));
    dec = hb_decoder_create(HB_ARCH_X64, mem_case.code, sizeof(mem_case.code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    ...
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint32_t expect_mem_mulld[8];
    for (unsigned i = 0; i < 8; i++)
        expect_mem_mulld[i] = (uint32_t)((int64_t)lhs_d[i] * (int64_t)mem_case.mem[i]);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_mem_mulld));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vex_xmm[] = {0xc4, 0xe2, 0x71, 0x40, 0xc2}; /* vpmulld xmm0, xmm1, xmm2 */
    uint8_t legacy_xmm[] = {0x66, 0x0f, 0x38, 0x40, 0xc2}; /* pmulld xmm0, xmm2 */
    hb_context_t* vex_ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    hb_context_t* legacy_ctx = hb_context_create(HB_ARCH_X64, HB_BACKEND_INTERP);
    ASSERT(vex_ctx != NULL && legacy_ctx != NULL);
    vex_ctx->memory = hb_memory_create(0);
    ...
    vex_ctx->pc = (uint64_t)(uintptr_t)vex_xmm;
    legacy_ctx->pc = (uint64_t)(uintptr_t)legacy_xmm;
    memcpy(vex_ctx->regs.x64.xmm[1], lhs_d, 16);
    memcpy(vex_ctx->regs.x64.xmm[2], rhs_d, 16);
    memcpy(legacy_ctx->regs.x64.xmm[0], lhs_d, 16);
    memcpy(legacy_ctx->regs.x64.xmm[2], rhs_d, 16);
    hb_decoder_t* vex_dec = hb_decoder_create(HB_ARCH_X64, vex_xmm, sizeof(vex_xmm), vex_ctx->pc);
    hb_decoder_t* leg_dec = hb_decoder_create(HB_ARCH_X64, legacy_xmm, sizeof(legacy_xmm), legacy_ctx->pc);
    hb_ir_func_t* vex_func = NULL;
    hb_ir_func_t* leg_func = NULL;
...
TEST(interp_x64_avx2_broadcast_family_semantics) {
    hb_decoded_t d;
    hb_exec_result_t run_out;
    uint8_t reg_code[] = {
        0xc4, 0xe2, 0x79, 0x78, 0xc1, /* vpbroadcastb xmm0, xmm1 */
    ...
    ASSERT(hb_decode_x64(reg_code, 5, (uint64_t)(uintptr_t)reg_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPBROADCASTB && d.op1.reg == HB_REG_XMM0 && d.op1.size == 16);
    ASSERT(d.op2.reg == HB_REG_XMM1);
    ASSERT(hb_decode_x64(reg_code + 5, 5, (uint64_t)(uintptr_t)(reg_code + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPBROADCASTW && d.op1.reg == HB_REG_XMM2 && d.op1.size == 32);
    ASSERT(hb_decode_x64(reg_code + 10, 5, (uint64_t)(uintptr_t)(reg_code + 10), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPBROADCASTD && d.op1.reg == HB_REG_XMM4 && d.op1.size == 32);
    ASSERT(hb_decode_x64(reg_code + 15, 5, (uint64_t)(uintptr_t)(reg_code + 15), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPBROADCASTQ && d.op1.reg == HB_REG_XMM6 && d.op1.size == 32);
    ...
    uint8_t expected_xmm0[16], expected_ymm2[32], expected_ymm4[32], expected_ymm6[32];
    memset(expected_xmm0, 0x9a, sizeof(expected_xmm0));
    for (unsigned i = 0; i < 16; i++) {
        uint16_t w = 0x1234u;
        memcpy(expected_ymm2 + i * 2, &w, sizeof(w));
    }
    for (unsigned i = 0; i < 8; i++) {
        uint32_t v = 0x11223344u;
        memcpy(expected_ymm4 + i * 4, &v, sizeof(v));
    }
    for (unsigned i = 0; i < 4; i++) {
        uint64_t v = 0x8877665544332211ULL;
        memcpy(expected_ymm6 + i * 8, &v, sizeof(v));
    }
    ctx->regs.x64.xmm[1][0] = 0x9a;
    ctx->regs.x64.xmm[3][0] = 0x1234;
    ctx->regs.x64.xmm[5][0] = 0x11223344u;
    ctx->regs.x64.xmm[7][0] = 0x8877665544332211ULL;
    memset(ctx->ymm_hi[0], 0xff, sizeof(ctx->ymm_hi[0]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_xmm0, sizeof(expected_xmm0)) == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    ASSERT(test_ymm_bytes_equal(ctx, 2, expected_ymm2));
    ASSERT(test_ymm_bytes_equal(ctx, 4, expected_ymm4));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expected_ymm6));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t mem_code[] = {
        0xc4, 0xe2, 0x7d, 0x58, 0x48, 0x20, /* vpbroadcastd ymm1, [rax+0x20] */
        0xc4, 0xe2, 0x7d, 0x5a, 0x10        /* vbroadcasti128 ymm2, [rax] */
    };
    ASSERT(hb_decode_x64(mem_code, 6, (uint64_t)(uintptr_t)mem_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPBROADCASTD && d.op2.is_mem && d.op2.size == 4);
    ASSERT(hb_decode_x64(mem_code + 6, 5, (uint64_t)(uintptr_t)(mem_code + 6), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VBROADCASTI128 && d.op1.size == 32 && d.op2.is_mem && d.op2.size == 16);
    base = (uint64_t)(uintptr_t)mem_code;
    dec = hb_decoder_create(HB_ARCH_X64, mem_code, sizeof(mem_code), base);
    func = NULL;
    ASSERT(dec != NULL);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)mem_code, sizeof(mem_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint64_t data_base = 0x72000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    uint8_t data[0x40] = {0};
    for (unsigned i = 0; i < 16; i++) data[i] = (uint8_t)(0xe0u + i);
    uint32_t mem_dword = 0xa5b6c7d8u;
    memcpy(data + 0x20, &mem_dword, sizeof(mem_dword));
    ASSERT(hb_memory_write(ctx->memory, data_base, data, sizeof(data)) == HB_OK);
    ctx->pc = base;
    ctx->regs.x64.rax = data_base;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint8_t expect_mem_ymm1[32], expect_mem_ymm2[32];
    for (unsigned i = 0; i < 8; i++) memcpy(expect_mem_ymm1 + i * 4, &mem_dword, sizeof(mem_dword));
    memcpy(expect_mem_ymm2, data, 16);
    memcpy(expect_mem_ymm2 + 16, data, 16);
    ASSERT(test_ymm_bytes_equal(ctx, 1, expect_mem_ymm1));
    ASSERT(test_ymm_bytes_equal(ctx, 2, expect_mem_ymm2));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_avx2_lane_control_family_semantics) {
    hb_decoded_t d;
    uint8_t code[] = {
        0xc4, 0xe3, 0x75, 0x02, 0xc2, 0xaa, /* vpblendd ymm0, ymm1, ymm2, 0xaa */
        0xc4, 0xe3, 0xfd, 0x00, 0xdc, 0x1b, /* vpermq ymm3, ymm4, 0x1b */
        0xc4, 0xe3, 0xfd, 0x01, 0xee, 0x4e, /* vpermpd ymm5, ymm6, 0x4e */
        0xc4, 0xe3, 0x75, 0x38, 0xfa, 0x01, /* vinserti128 ymm7, ymm1, xmm2, 1 */
        0xc4, 0xe3, 0x7d, 0x39, 0xfe, 0x01, /* vextracti128 xmm6, ymm7, 1 */
        0xc4, 0xe3, 0x75, 0x46, 0xe2, 0x31  /* vperm2i128 ymm4, ymm1, ymm2, 0x31 */
    };
    ASSERT(hb_decode_x64(code, 6, (uint64_t)(uintptr_t)code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPBLENDD && d.has_imm8 && d.imm8 == 0xaa);
    ASSERT(d.op1.size == 32 && d.op2.size == 32 && d.op3.size == 32);
    ASSERT(hb_decode_x64(code + 18, 6, (uint64_t)(uintptr_t)(code + 18), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VINSERTI128 && d.op1.size == 32 && d.op2.size == 32 && d.op3.size == 16);
    ASSERT(hb_decode_x64(code + 24, 6, (uint64_t)(uintptr_t)(code + 24), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VEXTRACTI128 && d.op1.size == 16 && d.op2.size == 32);

    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, code, sizeof(code), (uint64_t)(uintptr_t)code);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    uint8_t r1[32], r2[32], r4[32], r6[32];
    for (unsigned i = 0; i < 32; i++) {
        r1[i] = (uint8_t)(0x10u + i);
        r2[i] = (uint8_t)(0x80u + i);
        r4[i] = (uint8_t)(0x40u + i);
        r6[i] = (uint8_t)(0xc0u + i);
    }
    test_set_ymm_bytes(ctx, 1, r1);
    test_set_ymm_bytes(ctx, 2, r2);
    test_set_ymm_bytes(ctx, 4, r4);
    test_set_ymm_bytes(ctx, 6, r6);

    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);

    uint8_t expect0[32], expect3[32], expect4[32], expect5[32], expect6[32], expect7[32];
    for (unsigned lane = 0; lane < 8; lane++) {
        const uint8_t* src = ((0xaau >> lane) & 1u) ? r2 : r1;
        memcpy(expect0 + lane * 4, src + lane * 4, 4);
    }
    for (unsigned lane = 0; lane < 4; lane++) memcpy(expect3 + lane * 8, r4 + (3u - lane) * 8, 8);
    unsigned pd_sel[4] = {2, 3, 0, 1};
    for (unsigned lane = 0; lane < 4; lane++) memcpy(expect5 + lane * 8, r6 + pd_sel[lane] * 8, 8);
    memcpy(expect7, r1, 16);
    memcpy(expect7 + 16, r2, 16);
    memset(expect6, 0, sizeof(expect6));
    memcpy(expect6, r2, 16);
    memcpy(expect4, r1 + 16, 16);
    memcpy(expect4 + 16, r2 + 16, 16);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect0));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect3));
    ASSERT(test_ymm_bytes_equal(ctx, 4, expect4));
    ASSERT(test_ymm_bytes_equal(ctx, 5, expect5));
    ASSERT(memcmp(ctx->regs.x64.xmm[6], expect6, 16) == 0);
    ASSERT(ctx->ymm_hi[6][0] == 0 && ctx->ymm_hi[6][1] == 0);
    ASSERT(test_ymm_bytes_equal(ctx, 7, expect7));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t xmm_code[] = {0xc4, 0xe3, 0x71, 0x02, 0xc2, 0x0a}; /* vpblendd xmm0, xmm1, xmm2, 0x0a */
    ASSERT(hb_decode_x64(xmm_code, sizeof(xmm_code), (uint64_t)(uintptr_t)xmm_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPBLENDD && d.op1.size == 16);
    dec = hb_decoder_create(HB_ARCH_X64, xmm_code, sizeof(xmm_code), (uint64_t)(uintptr_t)xmm_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    for (unsigned lane = 0; lane < 4; lane++) {
        const uint8_t* src = ((0x0au >> lane) & 1u) ? r2 : r1;
        memcpy(expect0 + lane * 4, src + lane * 4, 4);
    }
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expect0, 16) == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t mem_code[] = {
        0xc4, 0xe3, 0x75, 0x18, 0x00, 0x01,       /* vinsertf128 ymm0, ymm1, [rax], 1 */
        0xc4, 0xe3, 0x7d, 0x19, 0x40, 0x20, 0x01  /* vextractf128 [rax+0x20], ymm0, 1 */
    };
    dec = hb_decoder_create(HB_ARCH_X64, mem_code, sizeof(mem_code), (uint64_t)(uintptr_t)mem_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)mem_code, sizeof(mem_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint64_t data_base = 0x73000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base, r2, 16) == HB_OK);
    test_set_ymm_bytes(ctx, 1, r1);
    ctx->regs.x64.rax = data_base;
    ctx->pc = (uint64_t)(uintptr_t)mem_code;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    memcpy(expect0, r1, 16);
    memcpy(expect0 + 16, r2, 16);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect0));
    uint8_t got_mem[16] = {0};
    ASSERT(hb_memory_read(ctx->memory, data_base + 0x20, got_mem, sizeof(got_mem)) == HB_OK);
    ASSERT(memcmp(got_mem, r2, sizeof(got_mem)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_avx2_variable_shift_family_semantics) {
    uint8_t code[] = {
        0xc4, 0xe2, 0x71, 0x47, 0xc2, /* vpsllvd xmm0, xmm1, xmm2 */
        0xc4, 0xe2, 0x75, 0x45, 0xda, /* vpsrlvd ymm3, ymm1, ymm2 */
        0xc4, 0xe2, 0xcd, 0x47, 0xec, /* vpsllvq ymm5, ymm6, ymm4 */
        0xc4, 0xe2, 0xc5, 0x45, 0xf4, /* vpsrlvq ymm6, ymm7, ymm4 */
        0xc4, 0xe2, 0x75, 0x46, 0x20  /* vpsravd ymm4, ymm1, [rax] */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(code, 5, (uint64_t)(uintptr_t)code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPSLLVD && d.op1.size == 16 && d.op2.size == 16 && d.op3.size == 16);
    ASSERT(hb_decode_x64(code + 5, 5, (uint64_t)(uintptr_t)(code + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPSRLVD && d.op1.size == 32 && d.op2.size == 32 && d.op3.size == 32);
    ASSERT(hb_decode_x64(code + 20, 5, (uint64_t)(uintptr_t)(code + 20), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPSRAVD && d.op3.is_mem && d.op3.size == 32);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code, sizeof(code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint64_t data_base = 0x74000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)code;
    ctx->regs.x64.rax = data_base;

    uint32_t dvals[8] = {1, 0x80000000u, 0xffffffffu, 0x12345678u,
                         0x7fffffffu, 0x00008000u, 0xdeadbeefu, 0x40000000u};
    uint32_t dcounts[8] = {0, 1, 4, 31, 32, 33, 8, 2};
    uint32_t ar_counts[8] = {1, 31, 32, 4, 0, 17, 40, 2};
    uint64_t qvals_a[4] = {1, 0x8000000000000000ULL, 0xffffffffffffffffULL, 0x0123456789abcdefULL};
    uint64_t qvals_b[4] = {0x8000000000000000ULL, 0xffffffffffffffffULL, 0x100000000ULL, 0x80ULL};
    uint64_t qcounts_b[4] = {1, 4, 64, 7};
    uint8_t r1[32], r2[32], r4[32], r6[32], r7[32];
    memcpy(r1, dvals, sizeof(r1));
    memcpy(r2, dcounts, sizeof(r2));
    memcpy(r4, qcounts_b, sizeof(r4));
    memcpy(r6, qvals_a, sizeof(r6));
    memcpy(r7, qvals_b, sizeof(r7));
    test_set_ymm_bytes(ctx, 1, r1);
    test_set_ymm_bytes(ctx, 2, r2);
    test_set_ymm_bytes(ctx, 4, r4);
    test_set_ymm_bytes(ctx, 6, r6);
    test_set_ymm_bytes(ctx, 7, r7);
    ASSERT(hb_memory_write(ctx->memory, data_base, ar_counts, sizeof(ar_counts)) == HB_OK);
    memset(ctx->ymm_hi[0], 0xff, sizeof(ctx->ymm_hi[0]));
    ...
    uint32_t exp0_d[4], exp3_d[8], exp4_d[8];
    for (unsigned i = 0; i < 4; i++) exp0_d[i] = dcounts[i] >= 32 ? 0 : (uint32_t)(dvals[i] << dcounts[i]);
    for (unsigned i = 0; i < 8; i++) exp3_d[i] = dcounts[i] >= 32 ? 0 : (uint32_t)(dvals[i] >> dcounts[i]);
    for (unsigned i = 0; i < 8; i++) {
        int32_t a = (int32_t)dvals[i];
        exp4_d[i] = ar_counts[i] >= 32 ? (uint32_t)(a < 0 ? -1 : 0) : (uint32_t)(a >> ar_counts[i]);
    }
    uint64_t exp5_q[4], exp6_q[4];
    for (unsigned i = 0; i < 4; i++) exp5_q[i] = qcounts_b[i] >= 64 ? 0 : (uint64_t)(qvals_a[i] << qcounts_b[i]);
    for (unsigned i = 0; i < 4; i++) exp6_q[i] = qcounts_b[i] >= 64 ? 0 : (uint64_t)(qvals_b[i] >> qcounts_b[i]);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], exp0_d, sizeof(exp0_d)) == 0);
    ASSERT(ctx->ymm_hi[0][0] == 0 && ctx->ymm_hi[0][1] == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[3], exp3_d, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[3], ((uint8_t*)exp3_d) + 16, 16) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[4], exp4_d, 16) == 0);
    ASSERT(memcmp(ctx->ymm_hi[4], ((uint8_t*)exp4_d) + 16, 16) == 0);
    ASSERT(test_ymm_bytes_equal(ctx, 5, exp5_q));
    ASSERT(test_ymm_bytes_equal(ctx, 6, exp6_q));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_avx2_permute_dword_family_semantics) {
    uint8_t code[] = {
        0xc4, 0xe2, 0x75, 0x36, 0xc2, /* vpermd ymm0, ymm1, ymm2 */
        0xc4, 0xe2, 0x75, 0x16, 0x18  /* vpermps ymm3, ymm1, [rax] */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(code, 5, (uint64_t)(uintptr_t)code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPERMD && d.op1.size == 32 && d.op2.size == 32 && d.op3.size == 32);
    ASSERT(hb_decode_x64(code + 5, 5, (uint64_t)(uintptr_t)(code + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPERMPS && d.op3.is_mem && d.op3.size == 32);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code, sizeof(code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint64_t data_base = 0x76000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)code;
    ctx->regs.x64.rax = data_base;

    uint32_t idx[8] = {7, 0, 5, 2, 8, 11, 1, 4};
    uint32_t src_reg[8] = {0x100, 0x101, 0x102, 0x103, 0x104, 0x105, 0x106, 0x107};
    uint32_t src_mem[8] = {0x400, 0x401, 0x402, 0x403, 0x404, 0x405, 0x406, 0x407};
    test_set_ymm_bytes(ctx, 1, idx);
    test_set_ymm_bytes(ctx, 2, src_reg);
    ASSERT(hb_memory_write(ctx->memory, data_base, src_mem, sizeof(src_mem)) == HB_OK);

    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint32_t exp0[8], exp3[8];
    for (unsigned i = 0; i < 8; i++) {
        exp0[i] = src_reg[idx[i] & 7u];
        exp3[i] = src_mem[idx[i] & 7u];
    }
    ASSERT(test_ymm_bytes_equal(ctx, 0, exp0));
    ASSERT(test_ymm_bytes_equal(ctx, 3, exp3));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_avx_maskmov_family_semantics) {
    uint8_t code[] = {
        0xc4, 0xe2, 0x6d, 0x2c, 0x08,                         /* vmaskmovps ymm1, ymm2, [rax] */
        0xc4, 0xe2, 0x49, 0x2d, 0x68, 0x20,                   /* vmaskmovpd xmm5, xmm6, [rax+0x20] */
        0xc4, 0xe2, 0xdd, 0x8c, 0x58, 0x40,                   /* vpmaskmovq ymm3, ymm4, [rax+0x40] */
        0xc4, 0xe2, 0xdd, 0x8e, 0x98, 0x80, 0x00, 0x00, 0x00  /* vpmaskmovq [rax+0x80], ymm4, ymm3 */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(code, 5, (uint64_t)(uintptr_t)code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VMASKMOVPS && d.op1.size == 32 && d.op2.size == 32 && d.op3.is_mem);
    ASSERT(hb_decode_x64(code + 5, 6, (uint64_t)(uintptr_t)(code + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VMASKMOVPD && d.op1.size == 16 && d.op2.size == 16 && d.op3.is_mem);
    ASSERT(hb_decode_x64(code + 11, 6, (uint64_t)(uintptr_t)(code + 11), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPMASKMOVQ && d.op1.size == 32 && d.op2.size == 32 && d.op3.is_mem);
    ASSERT(hb_decode_x64(code + 17, 9, (uint64_t)(uintptr_t)(code + 17), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPMASKMOVQ && d.op1.is_mem && d.op2.size == 32 && d.op3.size == 32);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code, sizeof(code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint64_t data_base = 0x77000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)code;
    ctx->regs.x64.rax = data_base;

    uint32_t mem_ps[8] = {0x100, 0x101, 0x102, 0x103, 0x104, 0x105, 0x106, 0x107};
    uint64_t mem_pd[2] = {0x200, 0x201};
    uint64_t mem_q[4] = {0x300, 0x301, 0x302, 0x303};
    uint64_t untouched[4] = {0xaaaa, 0xbbbb, 0xcccc, 0xdddd};
    ASSERT(hb_memory_write(ctx->memory, data_base, mem_ps, sizeof(mem_ps)) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base + 0x20, mem_pd, sizeof(mem_pd)) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base + 0x40, mem_q, sizeof(mem_q)) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base + 0x80, untouched, sizeof(untouched)) == HB_OK);

    uint32_t mask_ps[8] = {0x80000000u, 0, 0x80000000u, 0, 0, 0x80000000u, 0, 0x80000000u};
    uint64_t mask_q[4] = {0x8000000000000000ULL, 0, 0x8000000000000000ULL, 0};
    uint64_t mask_pd[4] = {0, 0x8000000000000000ULL, 0, 0};
    test_set_ymm_bytes(ctx, 2, mask_ps);
    test_set_ymm_bytes(ctx, 4, mask_q);
    test_set_ymm_bytes(ctx, 6, mask_pd);
    memset(ctx->ymm_hi[5], 0xff, sizeof(ctx->ymm_hi[5]));
    ...
    uint32_t exp_ps[8] = {0};
    for (unsigned i = 0; i < 8; i++) if (mask_ps[i] & 0x80000000u) exp_ps[i] = mem_ps[i];
    uint64_t exp_pd[2] = {0, mem_pd[1]};
    uint64_t exp_q[4] = {mem_q[0], 0, mem_q[2], 0};
    uint64_t exp_mem[4] = {mem_q[0], untouched[1], mem_q[2], untouched[3]};
    ASSERT(test_ymm_bytes_equal(ctx, 1, exp_ps));
    ASSERT(memcmp(ctx->regs.x64.xmm[5], exp_pd, sizeof(exp_pd)) == 0);
    ASSERT(ctx->ymm_hi[5][0] == 0 && ctx->ymm_hi[5][1] == 0);
    ASSERT(test_ymm_bytes_equal(ctx, 3, exp_q));
    uint64_t got_mem[4] = {0};
    ASSERT(hb_memory_read(ctx->memory, data_base + 0x80, got_mem, sizeof(got_mem)) == HB_OK);
    ASSERT(memcmp(got_mem, exp_mem, sizeof(exp_mem)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_avx2_gather_family_semantics) {
    uint8_t code[] = {
        0xc4, 0xe2, 0x5d, 0x90, 0x1c, 0x90, /* vpgatherdd ymm3, [rax+ymm2*4], ymm4 */
        0xc4, 0xe2, 0xc5, 0x91, 0x2c, 0xf0  /* vpgatherqq ymm5, [rax+ymm6*8], ymm7 */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(code, 6, (uint64_t)(uintptr_t)code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPGATHERDD && d.op1.size == 32 && d.op2.size == 32);
    ASSERT(d.op3.is_mem && d.op3.mem.vsib && d.op3.mem.vsib_index_size == 4 && d.op3.mem.vsib_elem_size == 4);
    ASSERT(hb_decode_x64(code + 6, 6, (uint64_t)(uintptr_t)(code + 6), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPGATHERQQ && d.op3.mem.vsib && d.op3.mem.vsib_index_size == 8 && d.op3.mem.vsib_elem_size == 8);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code, sizeof(code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint64_t data_base = 0x78000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x200, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)code;
    ctx->regs.x64.rax = data_base;
    ...
    for (unsigned i = 0; i < 16; i++) {
        data_d[i] = 0x1000u + i;
        data_q[i] = 0x8000000000000000ULL + i;
    }
    ASSERT(hb_memory_write(ctx->memory, data_base, data_d, sizeof(data_d)) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base + 0x80, data_q, sizeof(data_q)) == HB_OK);

    int32_t idx_d[8] = {0, 3, 5, 7, 2, 4, 6, 1};
    int64_t idx_q[4] = {16, 18, 20, 22};
    uint32_t mask_d[8] = {0x80000000u, 0, 0x80000000u, 0, 0x80000000u, 0, 0, 0x80000000u};
    uint64_t mask_q[4] = {0x8000000000000000ULL, 0, 0x8000000000000000ULL, 0};
    uint32_t dst_d[8] = {0xdead0000u, 0xdead0001u, 0xdead0002u, 0xdead0003u,
                         0xdead0004u, 0xdead0005u, 0xdead0006u, 0xdead0007u};
    uint64_t dst_q[4] = {0xbeef0000u, 0xbeef0001u, 0xbeef0002u, 0xbeef0003u};
    test_set_ymm_bytes(ctx, 2, idx_d);
    test_set_ymm_bytes(ctx, 3, dst_d);
    test_set_ymm_bytes(ctx, 4, mask_d);
    test_set_ymm_bytes(ctx, 5, dst_q);
    test_set_ymm_bytes(ctx, 6, idx_q);
    test_set_ymm_bytes(ctx, 7, mask_q);
    ...
    uint32_t exp_d[8];
    uint32_t exp_mask_d[8];
    memcpy(exp_d, dst_d, sizeof(exp_d));
    memcpy(exp_mask_d, mask_d, sizeof(exp_mask_d));
    for (unsigned i = 0; i < 8; i++) {
        if (mask_d[i] & 0x80000000u) {
            exp_d[i] = data_d[idx_d[i]];
            exp_mask_d[i] = 0;
        }
    }
    uint64_t exp_q[4];
    uint64_t exp_mask_q[4];
    memcpy(exp_q, dst_q, sizeof(exp_q));
    memcpy(exp_mask_q, mask_q, sizeof(exp_mask_q));
    for (unsigned i = 0; i < 4; i++) {
        if (mask_q[i] & 0x8000000000000000ULL) {
            exp_q[i] = data_q[idx_q[i] - 16];
            exp_mask_q[i] = 0;
        }
    }
    ASSERT(test_ymm_bytes_equal(ctx, 3, exp_d));
    ASSERT(test_ymm_bytes_equal(ctx, 4, exp_mask_d));
    ASSERT(test_ymm_bytes_equal(ctx, 5, exp_q));
    ASSERT(test_ymm_bytes_equal(ctx, 7, exp_mask_q));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_0f3a_pclmul_aeskeygenassist_semantics) {
    uint8_t legacy_code[] = {
        0x66, 0x0f, 0x3a, 0x44, 0xc3, 0x10, /* pclmulqdq xmm0, xmm3, 0x10 */
        0x66, 0x0f, 0x3a, 0xdf, 0xd1, 0x1b  /* aeskeygenassist xmm2, xmm1, 0x1b */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(legacy_code, 6, (uint64_t)(uintptr_t)legacy_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PCLMULQDQ && d.op3.is_imm && d.op3.imm == 0x10);
    ASSERT(hb_decode_x64(legacy_code + 6, 6, (uint64_t)(uintptr_t)(legacy_code + 6), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_AESKEYGENASSIST && d.op3.is_imm && d.op3.imm == 0x1b);

    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, legacy_code, sizeof(legacy_code),
                                          (uint64_t)(uintptr_t)legacy_code);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)legacy_code, sizeof(legacy_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)legacy_code;

    uint8_t aes_src[16] = {
        0xaa, 0xbb, 0xcc, 0xdd, 0x00, 0x01, 0x02, 0x03,
        0x05, 0xff, 0x11, 0x22, 0x10, 0x20, 0x30, 0x40
    };
    memset(ctx->regs.x64.xmm[0], 0, 16);
    ctx->regs.x64.xmm[0][0] = 0x02;
    memcpy(ctx->regs.x64.xmm[1], aes_src, 16);
    memset(ctx->regs.x64.xmm[3], 0, 16);
    ((uint8_t*)ctx->regs.x64.xmm[3])[8] = 0x03;

    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint8_t expect_pcl[16] = {0};
    expect_pcl[0] = 0x06;
    uint32_t expect_aes[4] = {
        0x7b777c63u, 0x637b7767u, 0x0904b7cau, 0xca0904acu
    };
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expect_pcl, sizeof(expect_pcl)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[2], expect_aes, sizeof(expect_aes)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vex_code[] = {
        0xc4, 0xe3, 0x59, 0x44, 0x58, 0x20, 0x11, /* vpclmulqdq xmm3, xmm4, [rax+0x20], 0x11 */
        0xc4, 0xe3, 0x79, 0xdf, 0x28, 0x1b  /* vaeskeygenassist xmm5, [rax], 0x1b */
    };
    ASSERT(hb_decode_x64(vex_code, 7, (uint64_t)(uintptr_t)vex_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VPCLMULQDQ && d.has_imm8 && d.imm8 == 0x11);
    ASSERT(hb_decode_x64(vex_code + 7, 6, (uint64_t)(uintptr_t)(vex_code + 7), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_AESKEYGENASSIST && d.op3.is_imm && d.op3.imm == 0x1b);

    dec = hb_decoder_create(HB_ARCH_X64, vex_code, sizeof(vex_code), (uint64_t)(uintptr_t)vex_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)vex_code, sizeof(vex_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint64_t data_base = 0x75000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base, aes_src, 16) == HB_OK);
    uint8_t pcl_src[16] = {0};
    pcl_src[8] = 0x05;
    ASSERT(hb_memory_write(ctx->memory, data_base + 0x20, pcl_src, sizeof(pcl_src)) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)vex_code;
    ctx->regs.x64.rax = data_base;
    memset(ctx->regs.x64.xmm[4], 0, 16);
    ((uint8_t*)ctx->regs.x64.xmm[4])[8] = 0x04;
    memset(ctx->ymm_hi[3], 0xff, sizeof(ctx->ymm_hi[3]));
    memset(ctx->ymm_hi[5], 0xff, sizeof(ctx->ymm_hi[5]));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    memset(expect_pcl, 0, sizeof(expect_pcl));
    expect_pcl[0] = 0x14;
    ASSERT(memcmp(ctx->regs.x64.xmm[3], expect_pcl, sizeof(expect_pcl)) == 0);
    ASSERT(ctx->ymm_hi[3][0] == 0 && ctx->ymm_hi[3][1] == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[5], expect_aes, sizeof(expect_aes)) == 0);
    ASSERT(ctx->ymm_hi[5][0] == 0 && ctx->ymm_hi[5][1] == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_aes_round_family_semantics) {
    ...
    static const uint8_t expect_r1[16] = {
        0x89,0xd8,0x10,0xe8,0x85,0x5a,0xce,0x68,
        0x2d,0x18,0x43,0xd8,0xcb,0x12,0x8f,0xe4
    };
    static const uint8_t expect_r8[16] = {
        0xf4,0xdf,0x52,0x42,0x4d,0x0a,0xf9,0x89,
        0x8b,0xaf,0x73,0x1a,0xf4,0xdc,0xa2,0x5e
    };
    static const uint8_t expect_dk1[16] = {
        0x8c,0x56,0xdf,0xf0,0x82,0x5d,0xd3,0xf9,
        0x80,0x5a,0xd3,0xfc,0x86,0x59,0xd7,0xfd
    };
    static const uint8_t expect_declast[16] = {
        0xf7,0xd7,0x3d,0x77,0xbb,0x1c,0x4d,0x2d,
        0xde,0x04,0x07,0xcd,0x9d,0xab,0x00,0xe8
    };
    static const uint8_t aes_dk9[16] = {
    ...
    uint8_t legacy_code[] = {
        ...
        0x66,0x0f,0x38,0xde,0x38, /* aesdec xmm7, [rax] */
        0x66,0x0f,0x38,0xdf,0xc6  /* aesdeclast xmm0, xmm6 */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(legacy_code + 0, 5, (uint64_t)(uintptr_t)(legacy_code + 0), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_AESIMC);
    ASSERT(hb_decode_x64(legacy_code + 5, 5, (uint64_t)(uintptr_t)(legacy_code + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_AESENC);
    ASSERT(hb_decode_x64(legacy_code + 10, 5, (uint64_t)(uintptr_t)(legacy_code + 10), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_AESENCLAST);
    ASSERT(hb_decode_x64(legacy_code + 15, 5, (uint64_t)(uintptr_t)(legacy_code + 15), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_AESDEC && d.op2.is_mem);
    ASSERT(hb_decode_x64(legacy_code + 20, 5, (uint64_t)(uintptr_t)(legacy_code + 20), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_AESDECLAST);
    ...
    uint64_t data_base = 0x76000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base, aes_dk9, sizeof(aes_dk9)) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)legacy_code;
    ctx->regs.x64.rax = data_base;
    memcpy(ctx->regs.x64.xmm[0], aes_ct, 16);
    memcpy(ctx->regs.x64.xmm[2], aes_rk1, 16);
    memcpy(ctx->regs.x64.xmm[3], aes_init, 16);
    memcpy(ctx->regs.x64.xmm[4], aes_rk1, 16);
    memcpy(ctx->regs.x64.xmm[5], aes_r9, 16);
    memcpy(ctx->regs.x64.xmm[6], aes_rk10, 16);
    memcpy(ctx->regs.x64.xmm[7], aes_r9, 16);
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(memcmp(ctx->regs.x64.xmm[1], expect_dk1, 16) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[3], expect_r1, 16) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[5], aes_ct, 16) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[7], expect_r8, 16) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expect_declast, 16) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vex_code[] = {
        0xc4,0xe2,0x75,0xdc,0xc2,      /* vaesenc ymm0, ymm1, ymm2 */
        0xc4,0xe2,0x5d,0xdd,0xdd,      /* vaesenclast ymm3, ymm4, ymm5 */
        0xc4,0xe2,0x45,0xde,0x30,      /* vaesdec ymm6, ymm7, [rax] */
        0xc4,0x62,0x35,0xdf,0x40,0x20  /* vaesdeclast ymm8, ymm9, [rax+0x20] */
    };
    ASSERT(hb_decode_x64(vex_code + 0, 5, (uint64_t)(uintptr_t)(vex_code + 0), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VAESENC && d.op1.size == 32);
    ASSERT(hb_decode_x64(vex_code + 5, 5, (uint64_t)(uintptr_t)(vex_code + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VAESENCLAST && d.op1.size == 32);
    ASSERT(hb_decode_x64(vex_code + 10, 5, (uint64_t)(uintptr_t)(vex_code + 10), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VAESDEC && d.op3.is_mem && d.op3.size == 32);
    ASSERT(hb_decode_x64(vex_code + 15, 6, (uint64_t)(uintptr_t)(vex_code + 15), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VAESDECLAST && d.op1.reg == HB_REG_XMM8 && d.op2.reg == HB_REG_XMM9);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)vex_code, sizeof(vex_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    uint8_t dk9_ymm[32], rk10_ymm[32], init_ymm[32], rk1_ymm[32], r9_ymm[32], ct_ymm[32];
    uint8_t expect_r1_ymm[32], expect_r8_ymm[32], expect_declast_ymm[32];
    memcpy(dk9_ymm, aes_dk9, 16); memcpy(dk9_ymm + 16, aes_dk9, 16);
    memcpy(rk10_ymm, aes_rk10, 16); memcpy(rk10_ymm + 16, aes_rk10, 16);
    memcpy(init_ymm, aes_init, 16); memcpy(init_ymm + 16, aes_init, 16);
    memcpy(rk1_ymm, aes_rk1, 16); memcpy(rk1_ymm + 16, aes_rk1, 16);
    memcpy(r9_ymm, aes_r9, 16); memcpy(r9_ymm + 16, aes_r9, 16);
    memcpy(ct_ymm, aes_ct, 16); memcpy(ct_ymm + 16, aes_ct, 16);
    memcpy(expect_r1_ymm, expect_r1, 16); memcpy(expect_r1_ymm + 16, expect_r1, 16);
    memcpy(expect_r8_ymm, expect_r8, 16); memcpy(expect_r8_ymm + 16, expect_r8, 16);
    memcpy(expect_declast_ymm, expect_declast, 16); memcpy(expect_declast_ymm + 16, expect_declast, 16);
    ASSERT(hb_memory_write(ctx->memory, data_base, dk9_ymm, sizeof(dk9_ymm)) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base + 0x20, rk10_ymm, sizeof(rk10_ymm)) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)vex_code;
    ctx->regs.x64.rax = data_base;
    test_set_ymm_bytes(ctx, 1, init_ymm);
    test_set_ymm_bytes(ctx, 2, rk1_ymm);
    test_set_ymm_bytes(ctx, 4, r9_ymm);
    test_set_ymm_bytes(ctx, 5, rk10_ymm);
    test_set_ymm_bytes(ctx, 7, r9_ymm);
    test_set_ymm_bytes(ctx, 9, ct_ymm);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect_r1_ymm));
    ASSERT(test_ymm_bytes_equal(ctx, 3, ct_ymm));
    ASSERT(test_ymm_bytes_equal(ctx, 6, expect_r8_ymm));
    ASSERT(test_ymm_bytes_equal(ctx, 8, expect_declast_ymm));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t evex_code[] = {0x62,0xf2,0x75,0x48,0xdc,0xc2}; /* vaesenc zmm0, zmm1, zmm2 */
    ASSERT(hb_decode_x64(evex_code, sizeof(evex_code), (uint64_t)(uintptr_t)evex_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VAESENC && d.op1.size == 64);
    dec = hb_decoder_create(HB_ARCH_X64, evex_code, sizeof(evex_code), (uint64_t)(uintptr_t)evex_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)evex_code, sizeof(evex_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint8_t init_zmm[64], rk1_zmm[64], expect_r1_zmm[64];
    for (unsigned i = 0; i < 4; i++) {
        memcpy(init_zmm + i * 16, aes_init, 16);
        memcpy(rk1_zmm + i * 16, aes_rk1, 16);
        memcpy(expect_r1_zmm + i * 16, expect_r1, 16);
    }
    ctx->pc = (uint64_t)(uintptr_t)evex_code;
    test_set_zmm_bytes(ctx, 1, init_zmm);
    test_set_zmm_bytes(ctx, 2, rk1_zmm);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(test_zmm_bytes_equal(ctx, 0, expect_r1_zmm));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_gfni_gf2p8mulb_family_semantics) {
    static const uint8_t lhs16[16] = {
        0x57,0x02,0x80,0xff,0x13,0x00,0x01,0xab,
        0xcd,0xee,0x42,0x99,0x7f,0x10,0x20,0x33
    };
    static const uint8_t rhs16[16] = {
        0x83,0x80,0x02,0xff,0x11,0x55,0x00,0x10,
        0x02,0x03,0x42,0x99,0x7f,0x10,0x20,0x33
    };
    static const uint8_t expect16[16] = {
        0xc1,0x1b,0x1b,0x13,0x38,0x00,0x00,0x5e,
        0x81,0x29,0xaf,0xc0,0x89,0x1b,0x6c,0x72
    };
    static const uint8_t expect32[32] = {
        0xc1,0x1b,0x1b,0x13,0x38,0x00,0x00,0x5e,
        0x81,0x29,0xaf,0xc0,0x89,0x1b,0x6c,0x72,
        0x72,0x6c,0x1b,0x89,0xc0,0xaf,0x29,0x81,
        0x5e,0x00,0x00,0x38,0x13,0x1b,0x1b,0xc1
    };
    uint8_t lhs32[32], rhs32[32];
    memcpy(lhs32, lhs16, 16);
    for (unsigned i = 0; i < 16; i++) lhs32[16 + i] = lhs16[15 - i];
    memcpy(rhs32, rhs16, 16);
    for (unsigned i = 0; i < 16; i++) rhs32[16 + i] = rhs16[15 - i];

    uint8_t legacy_code[] = {0x66,0x0f,0x38,0xcf,0xc1}; /* gf2p8mulb xmm0, xmm1 */
    hb_decoded_t d;
    ASSERT(hb_decode_x64(legacy_code, sizeof(legacy_code), (uint64_t)(uintptr_t)legacy_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_GF2P8MULB);
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, legacy_code, sizeof(legacy_code),
                                          (uint64_t)(uintptr_t)legacy_code);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)legacy_code, sizeof(legacy_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)legacy_code;
    memcpy(ctx->regs.x64.xmm[0], lhs16, 16);
    memcpy(ctx->regs.x64.xmm[1], rhs16, 16);
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expect16, 16) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t vex_code[] = {
        0xc4,0xe2,0x75,0xcf,0xc2, /* vgf2p8mulb ymm0, ymm1, ymm2 */
        0xc4,0xe2,0x75,0xcf,0x18  /* vgf2p8mulb ymm3, ymm1, [rax] */
    };
    ASSERT(hb_decode_x64(vex_code, 5, (uint64_t)(uintptr_t)vex_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VGF2P8MULB && d.op1.size == 32);
    ASSERT(hb_decode_x64(vex_code + 5, 5, (uint64_t)(uintptr_t)(vex_code + 5), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VGF2P8MULB && d.op3.is_mem && d.op3.size == 32);
    dec = hb_decoder_create(HB_ARCH_X64, vex_code, sizeof(vex_code), (uint64_t)(uintptr_t)vex_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    ...
    uint64_t data_base = 0x77000000ULL;
    ASSERT(hb_memory_map_private(ctx->memory, data_base, 0x100, HB_PERM_READ | HB_PERM_WRITE) == HB_OK);
    ASSERT(hb_memory_write(ctx->memory, data_base, rhs32, sizeof(rhs32)) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)vex_code;
    ctx->regs.x64.rax = data_base;
    test_set_ymm_bytes(ctx, 1, lhs32);
    test_set_ymm_bytes(ctx, 2, rhs32);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(test_ymm_bytes_equal(ctx, 0, expect32));
    ASSERT(test_ymm_bytes_equal(ctx, 3, expect32));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t evex_code[] = {0x62,0xf2,0x75,0x48,0xcf,0xc2}; /* vgf2p8mulb zmm0, zmm1, zmm2 */
    ASSERT(hb_decode_x64(evex_code, sizeof(evex_code), (uint64_t)(uintptr_t)evex_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_VGF2P8MULB && d.op1.size == 64);
    dec = hb_decoder_create(HB_ARCH_X64, evex_code, sizeof(evex_code), (uint64_t)(uintptr_t)evex_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    ...
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)evex_code, sizeof(evex_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    uint8_t lhs64[64], rhs64[64], expect64[64];
    memcpy(lhs64, lhs32, 32); memcpy(lhs64 + 32, lhs32, 32);
    memcpy(rhs64, rhs32, 32); memcpy(rhs64 + 32, rhs32, 32);
    memcpy(expect64, expect32, 32); memcpy(expect64 + 32, expect32, 32);
    ctx->pc = (uint64_t)(uintptr_t)evex_code;
    test_set_zmm_bytes(ctx, 1, lhs64);
    test_set_zmm_bytes(ctx, 2, rhs64);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(test_zmm_bytes_equal(ctx, 0, expect64));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_evex_zmm_unmasked_core_semantics) {
    uint8_t code[] = {
        0x62,0xf1,0x74,0x48,0x58,0xc2, /* vaddps zmm0, zmm1, zmm2 */
        0x62,0xf1,0x7d,0x48,0x6f,0xd8  /* vmovdqa32 zmm3, zmm0 */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(code, 6, (uint64_t)(uintptr_t)code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_ADDPS && d.op1.size == 64 && d.op2.reg == HB_REG_XMM1);
    ASSERT(hb_decode_x64(code + 6, 6, (uint64_t)(uintptr_t)(code + 6), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_SSE_MOV && d.op1.size == 64 && d.op2.reg == HB_REG_XMM0);
    ...
    float lhs[16], rhs[16], expect[16];
    for (unsigned i = 0; i < 16; i++) {
        lhs[i] = (float)i + 1.0f;
        rhs[i] = (float)(i * 2) + 0.5f;
        expect[i] = lhs[i] + rhs[i];
    }
    test_set_zmm_bytes(ctx, 1, lhs);
    test_set_zmm_bytes(ctx, 2, rhs);
    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(test_zmm_bytes_equal(ctx, 0, expect));
    ASSERT(test_zmm_bytes_equal(ctx, 3, expect));
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_sse42_pcmpxstr_family_semantics) {
    hb_decoded_t d;
    hb_exec_result_t out;

    uint8_t istr_i_code[] = {0x66, 0x0f, 0x3a, 0x63, 0xca, 0x00}; /* pcmpistri xmm1, xmm2, 0 */
    ASSERT(hb_decode_x64(istr_i_code, sizeof(istr_i_code), (uint64_t)(uintptr_t)istr_i_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PCMPISTRI && d.op3.is_imm && d.op3.imm == 0);
    hb_decoder_t* dec = hb_decoder_create(HB_ARCH_X64, istr_i_code, sizeof(istr_i_code), (uint64_t)(uintptr_t)istr_i_code);
    hb_ir_func_t* func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)istr_i_code, sizeof(istr_i_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)istr_i_code;
    memcpy(ctx->regs.x64.xmm[1], "abc", 4);
    memcpy(ctx->regs.x64.xmm[2], "zzb", 4);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x64.rcx == 2);
    ASSERT(ctx->flags.cf && ctx->flags.zf && ctx->flags.sf && !ctx->flags.of);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t istr_m_code[] = {0x66, 0x0f, 0x3a, 0x62, 0xca, 0x48}; /* pcmpistrm xmm1, xmm2, 0x48 */
    dec = hb_decoder_create(HB_ARCH_X64, istr_m_code, sizeof(istr_m_code), (uint64_t)(uintptr_t)istr_m_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)istr_m_code, sizeof(istr_m_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)istr_m_code;
    memcpy(ctx->regs.x64.xmm[1], "abX", 4);
    memcpy(ctx->regs.x64.xmm[2], "abY", 4);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint8_t exp_mask[16] = {0xff, 0xff};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], exp_mask, sizeof(exp_mask)) == 0);
    ASSERT(ctx->flags.cf && ctx->flags.zf && ctx->flags.sf && ctx->flags.of);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t estr_code[] = {
        0x66, 0x0f, 0x3a, 0x61, 0xca, 0x08, /* pcmpestri xmm1, xmm2, 8 */
        0x66, 0x0f, 0x3a, 0x60, 0xca, 0x48  /* pcmpestrm xmm1, xmm2, 0x48 */
    };
    ASSERT(hb_decode_x64(estr_code, 6, (uint64_t)(uintptr_t)estr_code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PCMPESTRI);
    ASSERT(hb_decode_x64(estr_code + 6, 6, (uint64_t)(uintptr_t)(estr_code + 6), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PCMPESTRM);
    dec = hb_decoder_create(HB_ARCH_X64, estr_code, sizeof(estr_code), (uint64_t)(uintptr_t)estr_code);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)estr_code, sizeof(estr_code),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = (uint64_t)(uintptr_t)estr_code;
    ctx->regs.x64.rax = 4;
    ctx->regs.x64.rdx = 4;
    memcpy(ctx->regs.x64.xmm[1], "test", 4);
    memcpy(ctx->regs.x64.xmm[2], "tent", 4);
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    uint8_t exp_estr_mask[16] = {0xff, 0xff, 0x00, 0xff};
    ASSERT(ctx->regs.x64.rcx == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[0], exp_estr_mask, sizeof(exp_estr_mask)) == 0);
    ASSERT(ctx->flags.cf && ctx->flags.zf && ctx->flags.sf && ctx->flags.of);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_sse2_remaining_packed_vector_semantics) {
    uint8_t code[] = {
        0x66, 0x0f, 0xd8, 0xc1, /* psubusb xmm0, xmm1 */
        0x66, 0x0f, 0xe9, 0xd3, /* psubsw  xmm2, xmm3 */
        0x66, 0x0f, 0xda, 0xe5, /* pminub  xmm4, xmm5 */
        0x66, 0x0f, 0xee, 0xf7  /* pmaxsw  xmm6, xmm7 */
    };
    hb_decoded_t d;
    ASSERT(hb_decode_x64(code, 4, (uint64_t)(uintptr_t)code, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PSUBUSB);
    ASSERT(hb_decode_x64(code + 12, 4, (uint64_t)(uintptr_t)(code + 12), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PMAXSW);
    ...
    uint8_t psubusb_lhs[16] = {0, 5, 200, 255, 100, 50, 1, 2, 9, 80, 81, 82, 200, 201, 202, 203};
    uint8_t psubusb_rhs[16] = {1, 3, 100, 255, 150, 49, 2, 1, 10, 70, 90, 82, 100, 210, 1, 255};
    int16_t psubsw_lhs[8] = {0, 100, -100, 30000, -30000, 32767, -32768, 1234};
    int16_t psubsw_rhs[8] = {1, -100, 100, -10000, 10000, -1, 1, 2345};
    uint8_t pminub_lhs[16] = {10, 250, 1, 99, 44, 45, 46, 47, 200, 1, 2, 3, 4, 5, 6, 7};
    uint8_t pminub_rhs[16] = {9, 251, 2, 98, 50, 40, 46, 48, 199, 2, 1, 4, 3, 6, 5, 8};
    int16_t pmaxsw_lhs[8] = {-10, 10, 1000, -1000, 32760, -32760, 7, -7};
    int16_t pmaxsw_rhs[8] = {-20, 20, 900, -900, 32767, -32768, -8, 8};
    memcpy(ctx->regs.x64.xmm[0], psubusb_lhs, sizeof(psubusb_lhs));
    memcpy(ctx->regs.x64.xmm[1], psubusb_rhs, sizeof(psubusb_rhs));
    memcpy(ctx->regs.x64.xmm[2], psubsw_lhs, sizeof(psubsw_lhs));
    memcpy(ctx->regs.x64.xmm[3], psubsw_rhs, sizeof(psubsw_rhs));
    memcpy(ctx->regs.x64.xmm[4], pminub_lhs, sizeof(pminub_lhs));
    memcpy(ctx->regs.x64.xmm[5], pminub_rhs, sizeof(pminub_rhs));
    memcpy(ctx->regs.x64.xmm[6], pmaxsw_lhs, sizeof(pmaxsw_lhs));
    memcpy(ctx->regs.x64.xmm[7], pmaxsw_rhs, sizeof(pmaxsw_rhs));

    hb_exec_result_t run_out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint8_t expected_psubusb[16] = {0, 2, 100, 0, 0, 1, 0, 1, 0, 10, 0, 0, 100, 0, 201, 0};
    int16_t expected_psubsw[8] = {-1, 200, -200, 32767, -32768, 32767, -32768, -1111};
    uint8_t expected_pminub[16] = {9, 250, 1, 98, 44, 40, 46, 47, 199, 1, 1, 3, 3, 5, 5, 7};
    int16_t expected_pmaxsw[8] = {-10, 20, 1000, -900, 32767, -32760, 7, 8};
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_psubusb, sizeof(expected_psubusb)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[2], expected_psubsw, sizeof(expected_psubsw)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[4], expected_pminub, sizeof(expected_pminub)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[6], expected_pmaxsw, sizeof(expected_pmaxsw)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);

    uint8_t code2[] = {
        0x66, 0x0f, 0xf4, 0xc1, /* pmuludq xmm0, xmm1 */
        0x66, 0x0f, 0xf6, 0xd3, /* psadbw  xmm2, xmm3 */
        0x66, 0x0f, 0xd1, 0xe5, /* psrlw   xmm4, xmm5 */
        0x66, 0x0f, 0xe2, 0xf7  /* psrad   xmm6, xmm7 */
    };
    ASSERT(hb_decode_x64(code2, 4, (uint64_t)(uintptr_t)code2, &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PMULUDQ);
    ASSERT(hb_decode_x64(code2 + 8, 4, (uint64_t)(uintptr_t)(code2 + 8), &d) == HB_OK);
    ASSERT(d.opcode == HB_INS_PSRLW && !d.op2.is_imm);
    base = (uint64_t)(uintptr_t)code2;
    dec = hb_decoder_create(HB_ARCH_X64, code2, sizeof(code2), base);
    func = NULL;
    ASSERT(dec != NULL);
    ASSERT(hb_lift_func_x64(dec, &func) == HB_OK);
    hb_decoder_destroy(dec);
    ...
    ctx->memory = hb_memory_create(0);
    ASSERT(ctx->memory != NULL);
    ASSERT(hb_memory_map(ctx->memory, (hb_gva_t)(uintptr_t)code2, sizeof(code2),
                         HB_PERM_READ | HB_PERM_EXEC) == HB_OK);
    ctx->pc = base;

    uint32_t mul_lhs[4] = {2, 0x11111111u, 0xffffffffu, 0x22222222u};
    uint32_t mul_rhs[4] = {3, 0x33333333u, 2, 0x44444444u};
    uint8_t sad_lhs[16], sad_rhs[16];
    uint16_t psrlw_lhs[8] = {0xffff, 0x8000, 0x0008, 0x00f0, 0x1234, 0xaaaa, 0x0001, 0x0100};
    uint64_t psrlw_count = 3;
    int32_t psrad_lhs[4] = {-16, 0x70000000, 16, -1};
    uint64_t psrad_count = 4;
    for (int i = 0; i < 16; i++) {
        sad_lhs[i] = (uint8_t)i;
        sad_rhs[i] = (uint8_t)(15 - i);
    }
    memcpy(ctx->regs.x64.xmm[0], mul_lhs, sizeof(mul_lhs));
    memcpy(ctx->regs.x64.xmm[1], mul_rhs, sizeof(mul_rhs));
    memcpy(ctx->regs.x64.xmm[2], sad_lhs, sizeof(sad_lhs));
    memcpy(ctx->regs.x64.xmm[3], sad_rhs, sizeof(sad_rhs));
    memcpy(ctx->regs.x64.xmm[4], psrlw_lhs, sizeof(psrlw_lhs));
    memcpy(ctx->regs.x64.xmm[5], &psrlw_count, sizeof(psrlw_count));
    memcpy(ctx->regs.x64.xmm[6], psrad_lhs, sizeof(psrad_lhs));
    memcpy(ctx->regs.x64.xmm[7], &psrad_count, sizeof(psrad_count));
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &run_out) == HB_OK);
    ASSERT(run_out.result == HB_OK);
    uint64_t expected_mul[2] = {6, 0x00000001fffffffeULL};
    uint64_t expected_sad[2] = {0, 0};
    uint16_t expected_psrlw[8];
    int32_t expected_psrad[4] = {-1, 0x07000000, 1, -1};
    for (int i = 0; i < 8; i++) {
        int diff0 = (int)sad_lhs[i] - (int)sad_rhs[i];
        int diff1 = (int)sad_lhs[i + 8] - (int)sad_rhs[i + 8];
        expected_sad[0] += (uint64_t)(diff0 < 0 ? -diff0 : diff0);
        expected_sad[1] += (uint64_t)(diff1 < 0 ? -diff1 : diff1);
        expected_psrlw[i] = (uint16_t)(psrlw_lhs[i] >> 3);
    }
    ASSERT(memcmp(ctx->regs.x64.xmm[0], expected_mul, sizeof(expected_mul)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[2], expected_sad, sizeof(expected_sad)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[4], expected_psrlw, sizeof(expected_psrlw)) == 0);
    ASSERT(memcmp(ctx->regs.x64.xmm[6], expected_psrad, sizeof(expected_psrad)) == 0);
    hb_context_destroy(ctx);
    hb_ir_func_destroy(func);
...
TEST(interp_x64_cpuid_vendor_and_leaf1) {
    ...
    ASSERT((uint32_t)ctx->regs.x64.rcx & (1u << 28)); /* AVX */
    ASSERT((uint32_t)ctx->regs.x64.rcx & (1u << 29)); /* F16C */
...
TEST(interp_x64_cpuid_leaf7_hides_unverified_simd_features) {
    uint8_t code[] = {
    ...
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(((uint32_t)ctx->regs.x64.rbx & (1u << 5)) == 0);  /* AVX2 */
    ASSERT(((uint32_t)ctx->regs.x64.rbx & (1u << 16)) == 0); /* AVX512F */
    ASSERT(((uint32_t)ctx->regs.x64.rbx & (1u << 29)) == 0); /* SHA */
    ASSERT(((uint32_t)ctx->regs.x64.rcx & (1u << 8)) == 0);  /* GFNI */
...
TEST(interp_x64_xgetbv_leaf0) {
    ...
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT((uint32_t)ctx->regs.x64.rax == 0x7U);
...
TEST(decode_x86_bound_arpl_family) {
    ...
    /* ARPL ax, cx — r/m16 destination, r16 source. */
    uint8_t arpl[]  = {0x63, 0xc8};
    ASSERT(legacy_decode(bound, sizeof(bound), HB_INS_BOUND));
    ASSERT(legacy_decode(arpl,  sizeof(arpl),  HB_INS_ARPL));
    tests_passed++;
}

TEST(interp_x86_arpl_ax_cx_operand_order) {
    const uint32_t code_base = 0x00426000u;
    uint8_t code[] = {0x63, 0xc8};
    ...
    ctx->regs.x86.eip = code_base;
    ctx->regs.x86.eax = 0x70001000u;
    ctx->regs.x86.ecx = 0x00000003u;
    ctx->flags.zf = false;

    hb_exec_result_t out;
    ASSERT(hb_runtime_run(ctx, func, HB_BACKEND_INTERP, &out) == HB_OK);
    ASSERT(out.result == HB_OK);
    ASSERT(ctx->regs.x86.eax == 0x70001003u);
    ASSERT(ctx->regs.x86.ecx == 0x00000003u);
    ASSERT(ctx->flags.zf == true);
    ASSERT(ctx->regs.x86.eip == code_base + sizeof(code));
...
TEST(decode_x86_segreg_load_family) {
    /* Far pointer loads from the deterministic oracle slot at [esi]. */
    uint8_t les[] = {0xc4, 0x06};
    uint8_t lds[] = {0xc5, 0x06};
    uint8_t lfs[] = {0x0f, 0xb4, 0x06};
    uint8_t lgs[] = {0x0f, 0xb5, 0x06};
...
int main(int argc, char** argv) {
    ...
    if (argc == 3 && strcmp(argv[1], "--fast-family") == 0) {
        if (!strcmp(argv[2], "rep_movs") || !strcmp(argv[2], "string_ops")) {
            ...
            test_interp_x86_addr16_lea_bx_si();
            ...
            test_interp_x64_lodsq_overwrites_rax();
            test_interp_x64_lodsw_preserves_rax_high();
            test_interp_x64_rep_lodsb_direction_flag_backward();
            test_interp_x86_lodsb_preserves_eax_high();
            printf("lods_exit\n");
        ...
        if (!strcmp(argv[2], "x87")) {
            ...
            test_x87_fxam_class_condition_codes();
            test_x87_fprem_condition_codes();
            ...
            test_interp_x86_x87_fcmov_condition_family();
            printf("%d passed, %d failed\n", tests_passed, tests_failed);
            return tests_failed ? 1 : 0;
        }
        if (!strcmp(argv[2], "phase1_core")) {
            ...
            test_jit_x64_native_arith_rmw_dead_flags_test_jcc();
            ...
            test_interp_x64_vex_avx_and_sse41_packed_vector_semantics();
            test_interp_x64_avx2_packed_integer_256_family_semantics();
            test_interp_x64_avx2_unpack_pack_family_semantics();
            test_interp_x64_vex_movdqa_movdqu_load_store_semantics();
            test_interp_x64_vex_packed_mov_float_load_store_semantics();
            test_interp_x64_vex_packed_float_pd_and_sqrt_semantics();
            test_interp_x64_vex_scalar_sqrt_semantics();
            test_interp_x64_vex_scalar_mov_arith_and_upper_zero_semantics();
            test_interp_x64_vex128_upper_zero_audit();
            test_interp_x64_vex_bitwise_pd_family_semantics();
            test_interp_x64_avx2_0f38_compare_minmax_mul_family_semantics();
            test_interp_x64_avx2_broadcast_family_semantics();
            test_interp_x64_avx2_lane_control_family_semantics();
            test_interp_x64_avx2_variable_shift_family_semantics();
            test_interp_x64_avx2_permute_dword_family_semantics();
            test_interp_x64_avx_maskmov_family_semantics();
            test_interp_x64_avx2_gather_family_semantics();
            test_interp_x64_fma3_f16c_semantics();
            test_interp_x64_aes_round_family_semantics();
            test_interp_x64_gfni_gf2p8mulb_family_semantics();
            test_interp_x64_evex_zmm_unmasked_core_semantics();
            test_interp_x64_sse42_pcmpxstr_family_semantics();
            test_interp_x64_0f3a_pclmul_aeskeygenassist_semantics();
            test_interp_x64_sse2_remaining_packed_vector_semantics();
            printf("sse_exit\n");
    ...
    test_interp_x86_unicorn_diff_regressions_rcl_rcr();
    test_interp_x86_unicorn_diff_regressions_sse2();
    ...
    test_interp_x86_arpl_ax_cx_operand_order();
    ...
    test_jit_x64_xfg_dispatch_call_shortcuts_guard_jmp_rax();
    test_jit_x64_xfg_dispatch_call_fallback_preserves_normal_target();
    ...
    test_jit_x64_helper_null_qword_scan_loop_promotes_cache_triplet();
    test_jit_x64_helper_i32_less_tiebreaker_comparator_promotes_triplet();
    ...
    test_jit_x64_native_mem_reg_flags_jcc_pair();
    test_jit_x64_native_flags_setcc_sequence_family();
    ...
    test_interp_x64_unicorn_diff_regressions_scalar_flags();
