...
for ((run = 1; run <= SMOKE_REPEAT_COUNT; run++)); do
  RUN_LOG="$LOG"
  SMOKE_DIAG_HOST_PATH="$LOG_DIR/dx11-headless-${ARCH}.diag.log"
  if [[ "$SMOKE_REPEAT_COUNT" -gt 1 ]]; then
    RUN_LOG="$LOG_DIR/dx11-headless-${ARCH}-run${run}.log"
    SMOKE_DIAG_HOST_PATH="$LOG_DIR/dx11-headless-${ARCH}-run${run}.diag.log"
  fi
  rm -f "$SMOKE_DIAG_HOST_PATH"
  SMOKE_DIAG_WINE_PATH="Z:${SMOKE_DIAG_HOST_PATH//\//\\}"
  ...
  (
    env -i \
      ...
      DXMT_LOG_PATH="${DXMT_LOG_PATH:-$LOG_DIR}" \
      DXMT_LOG_LEVEL="${DXMT_LOG_LEVEL:-info}" \
      WINEMETAL_TRACE_HWND="${WINEMETAL_TRACE_HWND:-0}" \
      DXMT_SMOKE_DIAG_PATH="$SMOKE_DIAG_WINE_PATH" \
  ...
  echo "diag=$SMOKE_DIAG_HOST_PATH"
  echo "run_exit_code=$run_rc"
  grep -E "$SUMMARY_FOCUS_PATTERN" "$RUN_LOG" || true
  grep -E "$SUMMARY_PATTERN" "$RUN_LOG" | tail -700 || true
  if [[ -f "$SMOKE_DIAG_HOST_PATH" ]]; then
    grep -E "DXMT_SMOKE_DIAG|CreateSwapChainForHwnd|window_create|hr=|swapchain" "$SMOKE_DIAG_HOST_PATH" || true
  fi
  if [[ "$run_rc" -ne 0 ]]; then
    SMOKE_RC="$run_rc"
    break
