...
CG_SWIFT_TARGET="${CG_SWIFT_TARGET:-$(uname -m)-apple-macosx15.0}"
CG_SWIFT=(swift -target "$CG_SWIFT_TARGET")
...
for _ in $(seq 1 "$LIVE_POLL_ATTEMPTS"); do
  ...
  if "${CG_SWIFT[@]}" "$PROJECT_ROOT/tools/cg_window_probe.swift" dxmt >"$PROBE_JSON.tmp" 2>"$PROBE_JSON.err"; then
...
if [[ -n "$window_id" ]]; then
  ...
  if "${CG_SWIFT[@]}" "$PROJECT_ROOT/tools/cg_window_capture.swift" "$window_id" "$CAPTURE_BMP" >"$CAPTURE_JSON" 2>"$CAPTURE_JSON.err"; then
    capture_status="PASS"
  else
    capture_status="FAIL"
  fi
