...
# ── Прогон ────────────────────────────────────────────────────────────────────────────────────
# ПРИБОРЫ ТРАССЫ СВОПЧЕЙНА — ПО ЯВНОЙ ПРОСЬБЕ: MR_FRAMES=1.
#
# ОБВИНЕНИЕ СНЯТО. Часом раньше я записал здесь, что приборы сами поднимают шторм отказов:
# floor26 без них дал 20 480 отказов, с ними — 125 миллионов. Сравнение было ложным по двум
# причинам сразу.
#   1. MACRUNNER_DXMT_FRAME_DUMP лежит в наборе САМОГО ПРОФИЛЯ (final-child.json), то есть
#      дамп кадров был включён ВО ВСЕХ прогонах. «Без приборов» не существовало.
#   2. Прогон 202954 шёл с swaptrace=0 и всё равно набрал 53 миллиона отказов на одну страницу.
# Настоящий разделитель — не приборы, а ПРОДВИЖЕНИЕ: шторм начинается на +590 с, когда игрок
# выбрал язык и игра пошла грузить следующую сцену. Прогон 170156 просто закончился раньше,
# на 615 с, и до шторма не дожил.
#
# Гейт оставлен: трасса свопчейна многословна, и включать её стоит осознанно. Но цена её —
# строки в журнале, а не остановка прогона.
...
if [ -z "$D" ] || [ ! -f "$D/run.log" ]; then
  # Не приписывать вину префлайту вслепую: сюда попадаем и когда упал сам ярлык (07.08 так и
  # было — bash 3.2 на пустом массиве). Сказать «отказала проверка», когда она не запускалась,
  # значит отправить искать причину не туда.
  echo "${R}Прогон не начался.${N}"
  if [ "$rc" = 0 ]; then
    echo "Преполётная проверка отказала — причина в выводе выше."
  else
    echo "Цепочка запуска завершилась с кодом $rc. Если выше есть строка про сам"
    echo "mr-launch.sh или mr-profile.sh — сломан скрипт, а не проверка."
  fi
