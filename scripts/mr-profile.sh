...
PY
  ...
  # ЗАПЕЧАТАН = ЛЕЖИТ ВНЕ РАБОЧЕГО ДЕРЕВА. Первая редакция признавала запечатанным только дист
  # внутри папки профиля, и этаж (floor26 в Documents/MacRunner) под правило не попал: префлайт
  # отказал ему за то, что он старше исходников, — то есть за то, ради чего этаж и хранят.
  # Правильный признак не «где именно лежит», а «собран ли он из текущего дерева»: всё, что вне
  # engine/wine, пересобрать нельзя по определению, и требовать от него свежести бессмысленно.
  case "$dist" in
    "$ROOT"/engine/wine/*) ;;                 # рабочее дерево — свежесть требуем
    /*) export PREFLIGHT_SEALED_OK=1 ;;       # профиль, этаж, снапшот — запечатано
    ?*) case "$dist" in engine/wine/*) ;; *) export PREFLIGHT_SEALED_OK=1 ;; esac ;;
  esac
