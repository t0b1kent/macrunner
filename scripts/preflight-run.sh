...
for kv in "${ENVS[@]}"; do
  ...
  if ! printf '%s' "$DIST_STRINGS" | grep -qFs -- "$name"; then
    if [ "${PREFLIGHT_SEALED_OK:-0}" = 1 ]; then
      # У запечатанного этажа гейта может не быть просто потому, что этаж СТАРШЕ гейта.
      # floor26 от 06.08 не знает MACRUNNER_HB_RESERVE_HOLES, появившегося позже, — это не
      # «собрано, но не задеплоено», а «переключатель на этой сборке инертен». Молчать нельзя
      # (05.08 так вскрылось, что важный гейт исчез из кода), но и запрещать прогон незачем.
      note "эталон: $name в носителях нет — на этой сборке переключатель ИНЕРТЕН"
    else
