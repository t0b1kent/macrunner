#!/usr/bin/env python3
"""Unicorn-backed x86-64 single-instruction oracle for HyperBridge fuzzing."""
...
def build_initial_state(seed: int) -> InitialState:
    rng = [seed & MASK64]
...
def set_initial_state(uc: Uc, state: InitialState, code: bytes) -> None:
    ...
    for name, reg in zip(GPR_NAMES, GPR_REGS):
...
def snapshot(uc: Uc) -> dict[str, Any]:
    regs = {name: hex64(uc.reg_read(reg)) for name, reg in zip(GPR_NAMES, GPR_REGS)}
...
def run_case(seed: int, code_hex: str) -> dict[str, Any]:
    code = bytes.fromhex(code_hex)
    state = build_initial_state(seed)
    uc = Uc(UC_ARCH_X86, UC_MODE_64)
    try:
        set_initial_state(uc, state, code)
...
def diff_interpreter(row: dict[str, Any], oracle: dict[str, Any],
                     defined_flags: set[str] | frozenset[str] | None = None) -> list[str]:
